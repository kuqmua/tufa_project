        ...
        match &value.0 {
            ...
            None => {
                ...
                Ok(format!("
                      (
                    	  select jsonb_agg(
                    		  jsonb_build_object('id', elem -> 'id')||
                    		  jsonb_build_object('field_0', 'null'::jsonb)
                      	  )
                    	  from
                    	  jsonb_array_elements(column_157) as elem
                      )::jsonb as column_157
                "))
                //
                // Ok(format!("'[]'::jsonb"))
...
impl postgresql_crud::tests::PostgresqlTypeTestCases for OptionVecOfAnimalWithIdAsNullableArrayOfNotNullJsonbObjectWithId {
    type Element = Self;
    fn test_cases(read_only_ids: &<Self::Element as postgresql_crud::PostgresqlType>::ReadOnlyIds) -> std::vec::Vec<std::vec::Vec<<Self::Element as postgresql_crud::PostgresqlType>::ReadInner>> {
        println!("UUUUU {read_only_ids:#?}");
        // UUUUU OptionVecOfAnimalWithIdAsNullableArrayOfNotNullJsonbObjectWithIdReadOnlyIds(
        //     Some(
        //         VecOfAnimalWithIdAsNotNullArrayOfNotNullJsonbObjectWithIdReadOnlyIds(
        //             [
        //                 AnimalWithIdAsNotNullJsonbObjectWithIdReadOnlyIds {
        //                     id: 58ef626d-b868-4bc9-8eb4-874400bd08ed,
        //                     field_0: None,
        //                 },
        //             ],
        //         ),
        ...
        acc.push(None);
        let f = vec![acc];//here this case fails
        println!("RRRRRRR {f:#?}");
        f
    }
