        ...
        std::thread::Builder::new()
            ...
            .spawn(|| {
                tokio::runtime::Builder::new_multi_thread().worker_threads(num_cpus::get()).enable_all().build().expect("error 38823c21-1879-449c-9b60-ce7293709959").block_on(async {
                    ...
                    //
                    println!("######");
                    let common_read_only_ids_returned_from_create_many = match super::Example::try_create_many(
                        &url,
...
impl Example {
    pub async fn create_many(app_state: axum::extract::State<std::sync::Arc<dyn postgresql_crud::CombinationOfAppStateLogicTraits>>, request: axum::extract::Request) -> axum::response::Response {
        ...
        let query_string = postgresql_crud::generate_create_many_query_string(
            &Example::table_name(),
            "primary_key_column,column_154,column_155,column_156,column_157",
            {
...
impl Example {
    pub async fn try_create_many(endpoint_location: &std::primitive::str, parameters: ExampleCreateManyParameters) -> Result<
        std::vec::Vec<
        // <postgresql_crud::postgresql_type::SqlxTypesUuidUuidAsNotNullUuidV4InitializedByPostgresql as postgresql_crud::PostgresqlTypePrimaryKey>::PrimaryKey
            ExampleReadOnlyIds
        >,//here
        ExampleTryCreateManyErrorNamed
    > {
        let payload = {
        ...
        let future = reqwest::Client::new().post(&url).header(&postgresql_crud::CommitSnakeCase.to_string(), git_info::PROJECT_GIT_INFO.commit).header(reqwest::header::CONTENT_TYPE, "application/json").body(payload).send();
