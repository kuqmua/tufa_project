        ...
        std::thread::Builder::new()
            ...
            .spawn(|| {
                tokio::runtime::Builder::new_multi_thread().worker_threads(num_cpus::get()).enable_all().build().expect("error 38823c21-1879-449c-9b60-ce7293709959").block_on(async {
                    ...
                    ident_create_default = super :: ExampleCreate
                    {
                        column_156 : < < crate :: repositories_types :: server ::
                        routes :: api :: example ::
                        OptionAnimalAsNullableJsonbObject as postgresql_crud ::
                    ...
                    let column_156_test_cases = <crate::repositories_types::server::routes::api::example::OptionAnimalAsNullableJsonbObject as postgresql_crud::tests::PostgresqlTypeTestCases<
                        <crate::repositories_types::server::routes::api::example::OptionAnimalAsNullableJsonbObject as postgresql_crud::PostgresqlType>::ReadInner,
                    >>::test_cases();
                    for index in 0..column_156_test_cases.len() {
                        let column_156_some_value_update = match column_156_test_cases.get(index) {
                            Some(value) =>
                            Some(postgresql_crud :: Value {
                                value : < crate :: repositories_types :: server :: routes ::
                                api :: example :: OptionAnimalAsNullableJsonbObject as
                        ...
                        let column_156_some_value_read = match
                        column_156_test_cases.get(index)
                        {
                            Some(value) =>
                            Some(postgresql_crud :: Value
                            {
                                ...
                                example :: OptionAnimalAsNullableJsonbObject as
                        ...
                        default_but_option_is_always_some_and_vec_always_contains_one_element()),
                        super :: ExampleSelect ::
                        Column156(< < crate :: repositories_types :: server ::
                        routes :: api :: example ::
                        OptionAnimalAsNullableJsonbObject as postgresql_crud ::
                        ...
                        assert_eq!(
                            {
                                let left = super::ExampleRead {
                                    ...
                                    column_156: column_156_some_value_read
                                };
                                println!("left {left:#?}");
                                left
                            },
...
impl postgresql_crud::tests::PostgresqlTypeTestCases<AnimalAsNotNullJsonbObjectReadInner> for AnimalAsNotNullJsonbObject {
    ...
    fn test_cases() -> std::vec::Vec<<Self::Element as postgresql_crud::PostgresqlType>::ReadInner> {
        ...
        println!("generate_postgresql_json_object_type ident AnimalAsNotNullJsonbObject test cases len {}", acc.len());
        acc
    }
...
impl OptionAnimalAsNullableJsonbObjectSelect {
    fn select_query_part_postgresql_type(&self, column: &std::primitive::str) -> std::string::String {
        format!("case when jsonb_typeof({column}) = 'null' then null else ({}) end", {
...
impl OptionAnimalAsNullableJsonbObjectUpdate {
    fn update_query_part_postgresql_type(&self, jsonb_set_accumulator: &std::primitive::str, jsonb_set_target: &std::primitive::str, jsonb_set_path: &std::primitive::str, increment: &mut std::primitive::u64) -> Result<std::string::String, postgresql_crud::QueryPartErrorNamed> {
        let f = match &self.0 {
            ...
            None => match increment.checked_add(1) {
                Some(value) => {
                    println!("here jsonb_set_path{jsonb_set_path}##");
                    *increment = value;
                    Ok(format!(
                        // "${increment}"
...
impl postgresql_crud::tests::PostgresqlTypeTestCases<OptionAnimalAsNullableJsonbObjectReadInner> for OptionAnimalAsNullableJsonbObject {
    type Element = Self;
    fn test_cases() -> std::vec::Vec<<Self::Element as postgresql_crud::PostgresqlType>::ReadInner> {
        let mut acc: std::vec::Vec<<Self::Element as postgresql_crud::PostgresqlType>::ReadInner> = vec![];
        acc.push(None);
        acc
    }
