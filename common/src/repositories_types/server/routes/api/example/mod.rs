            ...
            .spawn(|| {
                tokio::runtime::Builder::new_multi_thread().worker_threads(num_cpus::get()).enable_all().build().expect("error 38823c21-1879-449c-9b60-ce7293709959").block_on(async {
                    ...
                    for index in 0..column_154_test_cases.len() {
                        let column_154_some_value_update = match
                        column_154_test_cases.get(index)
                        {
                            Some(value) =>
                        ...
                        let column_154_some_value_read = match
                        column_154_test_cases.get(index)
                        {
                            Some(value) =>
                            Some(postgresql_crud :: Value
...
impl ExampleUpdate {
    ...
    fn select_only_updated_ids_query_part(&self, increment: &mut std::primitive::u64) -> Result<std::string::String, postgresql_crud::QueryPartErrorNamed> {
        ...
        acc.push_str(&match <postgresql_crud::postgresql_type::SqlxTypesUuidUuidAsNotNullUuidV4InitializedByPostgresql as postgresql_crud::PostgresqlType>::select_only_updated_ids_query_part(
        ...
        if let Some(value) = &self.column_154 {
            acc.push_str(&match <crate::repositories_types::server::routes::api::example::AnimalAsNotNullJsonbObject as postgresql_crud::PostgresqlType>::select_only_updated_ids_query_part(
                &value.value,
                "column_154",
                increment,
                false,
