    ...
    fn read_from_previous_read_unwraped_merged_with_update(read: <Self::Element as postgresql_crud::PostgresqlJsonType>::Read, option_update: std::option::Option<<Self::Element as postgresql_crud::PostgresqlJsonType>::Update>) -> <Self::Element as postgresql_crud::PostgresqlJsonType>::Read {
        ...
        match option_update {
            Some(value) => match (read.0, value.0) {
                (Some(read_value), Some(update_value)) => OptionAnimalAsNullableJsonbObjectRead(Some(
                    <AnimalAsNotNullJsonbObject as postgresql_crud::PostgresqlJsonTypeTestCases>::read_from_previous_read_unwraped_merged_with_update(
                        read_value,
                        Some(update_value)
                    )
                )),
                (Some(read_value), None) => OptionAnimalAsNullableJsonbObjectRead(Some(read_value)),
                (None, Some(update_value)) => {
                    let mut field_0 = None;
                    let mut field_1 = None;
                    for element in update_value.0.into_vec() {
                        match element {
                            AnimalAsNotNullJsonbObjectUpdateElement::Field0(value) => {
                                field_0 = Some(value.value);
                            }
