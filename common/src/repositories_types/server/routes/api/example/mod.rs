...
pub type PostgresqlJsonTypeObjectAnimalOptionToUpdateTryGeneratePostgresqlJsonTypeErrorNamed = PostgresqlJsonTypeAnimalOptionToUpdateTryGeneratePostgresqlJsonTypeErrorNamed;
pub type PostgresqlJsonTypeObjectAnimalOptionToUpdateTryGeneratePostgresqlJsonTypeErrorNamedWithSerializeDeserialize = PostgresqlJsonTypeAnimalOptionToUpdateTryGeneratePostgresqlJsonTypeErrorNamedWithSerializeDeserialize;
impl postgresql_crud::PostgresqlJsonType for ObjectAnimal {
    ...
    fn generate_postgresql_json_type_to_read(
        ...
        let column_name_and_maybe_field_getter_field_ident = format!("{column_name_and_maybe_field_getter_handle}'{field_ident}'");
        ...
        for element in &postgresql_json_type_self_field_reader.0 {
            ...
            let h = match element
            {
                ...
                postgresql_crud :: postgresql_json_type :: StdPrimitiveI8 as
                postgresql_crud :: PostgresqlJsonType > ::
                generate_postgresql_json_type_to_read(value,
                "std_primitive_i8", &
                column_name_and_maybe_field_getter_field_ident, &
                column_name_and_maybe_field_getter_for_error_message_field_ident),
                
                AnimalFieldToReadWithoutId ::
                StdOptionOptionStdPrimitiveI8(value) => < postgresql_crud ::
                postgresql_json_type :: StdOptionOptionStdPrimitiveI8 as
                postgresql_crud :: PostgresqlJsonType > ::
                generate_postgresql_json_type_to_read(value,
        ...
        let _ = acc.pop();
        let _ = acc.pop();
        let f = format!("jsonb_build_object('{field_ident}', jsonb_build_object('value',{acc}))");
        println!("{f}");
        f
    }
    ...
    fn try_generate_postgresql_json_type_to_update(
        ...
        postgresql_json_type_self_option_to_update.try_generate_postgresql_json_type_to_update(jsonb_set_accumulator, jsonb_set_target, jsonb_set_path, increment)
    }
