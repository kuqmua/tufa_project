...
DynArcCombinationOfAppStateLogicTraits, >,
    ...
    let parameters = UpdateManyParameters {
        payload: match axum::Json::<UpdateManyPayloadWithSerializeDeserialize>::from_bytes(
            ...
            Ok(axum::Json(value)) => match UpdateManyPayload::try_from(value) {
                Ok(value) => value,
                Err(error) => {
    ...
    let
    query_string =
    "update dogs as t set std_primitive_bool_as_postgresql_bool = data.std_primitive_bool_as_postgresql_bool, std_primitive_i16_as_postgresql_small_int = data.std_primitive_i16_as_postgresql_small_int, std_primitive_i32_as_postgresql_int = data.std_primitive_i32_as_postgresql_int, sqlx_types_uuid_uuid_as_postgresql_uuid = data.sqlx_types_uuid_uuid_as_postgresql_uuid from (select * from unnest($1, $2, $3, $4, $5)) as data(std_primitive_bool_as_postgresql_bool, std_primitive_i16_as_postgresql_small_int, std_primitive_i32_as_postgresql_int, std_primitive_i64_as_postgresql_big_serial_not_null_primary_key, sqlx_types_uuid_uuid_as_postgresql_uuid) where t.std_primitive_i64_as_postgresql_big_serial_not_null_primary_key = data.std_primitive_i64_as_postgresql_big_serial_not_null_primary_key returning data.std_primitive_i64_as_postgresql_big_serial_not_null_primary_key";
    ...
    let binded_query = {
        ...
        let (
            ...
            |mut acc, element| {
                ...
                acc.4.push(element.sqlx_types_uuid_uuid_as_postgresql_uuid);
                acc
            },
        );
        query = query.bind(
        ...
        query = query.bind(
            sqlx_types_uuid_uuid_as_postgresql_uuid_vec
                .into_iter()
                .map(|element| element.into_inner())
                .collect::<std::vec::Vec<std::option::Option<sqlx::types::uuid::Uuid>>>(),
        );
        query
    };
        ...
        let results_vec =
            {
                ...
                if let Some(error) = option_error {
                    match postgres_transaction.rollback().await {
                        ...
                        QueryAndRollbackFailed
                        {
                            query_and_rollback_failed : error, rollback :
                            rollback_error, code_occurence : error_occurence_lib ::
                            code_occurence :: CodeOccurence ::
                            new(file! ().to_owned(), line! (), column! (),
                            Some(error_occurence_lib :: code_occurence :: MacroOccurence
                            {
