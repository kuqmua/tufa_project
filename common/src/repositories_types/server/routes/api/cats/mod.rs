        ...
        let sqlx_types_uuid_uuid_as_postgresql_uuid_not_null_primary_key =
            match value.sqlx_types_uuid_uuid_as_postgresql_uuid_not_null_primary_key {
                Some(value) => {
                        ...
                        match postgresql_crud::SqlxTypesUuidUuid::try_from(element) {
                            Ok(value) => {
                                if acc.contains(&value) {
                                    not_unique_primary_key_option = Some(value);
                                    break;
                                } else {
                                    acc.push(value);
                                }
                    ...
                    if let Some(not_unique_primary_key) = not_unique_primary_key_option {
                        return Err(Self::Error::NotUniquePrimaryKey {
...
#[derive(Debug, thiserror :: Error, error_occurence_lib :: ErrorOccurence)]
pub enum ReadManyPayloadWithSerializeDeserializeTryFromReadManyPayloadErrorNamed {
    NotUniqueColumn {
        #[eo_to_std_string_string_serialize_deserialize]
        not_unique_column: DogColumn,
        code_occurence: error_occurence_lib::code_occurence::CodeOccurence,
...
impl std::convert::TryFrom<ReadManyPayload> for ReadManyPayloadWithSerializeDeserialize {
    type Error = ReadManyPayloadWithSerializeDeserializeTryFromReadManyPayloadErrorNamed;
    ...
    let parameters = ReadManyParameters {
        payload: match axum::Json::<ReadManyPayloadWithSerializeDeserialize>::from_bytes(
            ...
            Ok(axum::Json(value)) => {
                ...
                {
                    let mut acc = std::vec::Vec::new();
                    for not_unique_primary_key in
                        sqlx_types_uuid_uuid_as_postgresql_uuid_not_null_primary_key
                    {
                        if !acc.contains(&not_unique_primary_key) {
                            acc.push(&not_unique_primary_key);
                        } else {
...
pub async fn try_read_many(
    ...
    let payload = {
        let value = match ReadManyPayloadWithSerializeDeserialize::try_from(parameters.payload) {
            Ok(value) => value,
            Err(error) => {
                return
