...
DynArcCombinationOfAppStateLogicTraits, >,
                    ...
                    for element in &value.0 {
                        if !acc.contains(
                            ...
                            NotUniquePrimaryKey
                            {
                                ...
                                element.sqlx_types_uuid_uuid_as_postgresql_uuid_not_null_primary_key.clone(),
                                code_occurence : error_occurence_lib :: code_occurence ::
                                CodeOccurence ::
                                new(file! ().to_owned(), line! (), column! (),
                                Some(error_occurence_lib :: code_occurence :: MacroOccurence
    ...
    let query_string = {
            ...
            if is_std_primitive_bool_as_postgresql_bool_update_exist {
                ...
                for element in &parameters.payload.0 {
                    if let Some(value) = &element.std_primitive_bool_as_postgresql_bool {
                        acc.push_str(& format!
                        ("when sqlx_types_uuid_uuid_as_postgresql_uuid_not_null_primary_key = {} then {} ",
        ...
        let _ = query.pop();
        query.push_str(& format!
        (" where sqlx_types_uuid_uuid_as_postgresql_uuid_not_null_primary_key in ({}) returning sqlx_types_uuid_uuid_as_postgresql_uuid_not_null_primary_key;",
        {
            ...
            while let Some(row) = match {
                ...
                .await
                {
                    Ok(value) => match value {
                        Some(value) => match primary_key_try_from_sqlx_row(&value) {
                            Ok(value) => Some(value),
                            Err(error) => {
                                drop(rows);
