...
pub struct UpdateManyPayloadElement {
    ...
    pub sqlx_types_time_time_as_postgresql_time_not_null: postgresql_crud::SqlxTypesTimeTime,
}
#[derive(Debug, utoipa :: ToSchema)]
    ...
    > for UpdateManyPayloadTryFromUpdateManyPayloadWithSerializeDeserializeErrorNamed
{
    ...
    UpdateManyPayloadElementTryFromUpdateManyPayloadElementWithSerializeDeserializeErrorNamed,
        ...
        {
            UpdateManyPayloadElementTryFromUpdateManyPayloadElementWithSerializeDeserializeErrorNamed
            :: Id { sqlx_types_uuid_uuid, code_occurence, } => Self :: Id
            { sqlx_types_uuid_uuid, code_occurence, },
            UpdateManyPayloadElementTryFromUpdateManyPayloadElementWithSerializeDeserializeErrorNamed
            :: SqlxTypesTimeTimeAsPostgresqlTimeNotNull
            { sqlx_types_time_time, code_occurence, } => Self ::
            SqlxTypesTimeTimeAsPostgresqlTimeNotNull
            { sqlx_types_time_time, code_occurence, },
        }
    }
}
#[derive(Debug, thiserror :: Error, error_occurence_lib :: ErrorOccurence)]
...
pub async fn update_many<'a>(
    ...
    println!("{:#?}", parameters);
    {
        ...
        let binded_query = {
            let query_string = {
                "update dogs as t set sqlx_types_time_time_as_postgresql_time_not_null = data.sqlx_types_time_time_as_postgresql_time_not_null from (select * from unnest($1, $2)) as data(id, sqlx_types_time_time_as_postgresql_time_not_null) where t.id = data.id returning data.id"
            };
            println!("{}", query_string);
            let mut query = sqlx::query::<sqlx::Postgres>(&query_string);
            let current_vec_len = parameters.payload.0.len();
            let (id_vec, sqlx_types_time_time_as_postgresql_time_not_null_vec) =
                parameters.payload.0.into_iter().fold(
                    ...
                    |mut acc, element| {
                        ...
                        acc.1
                            .push(element.sqlx_types_time_time_as_postgresql_time_not_null);
                        acc
                    },
                );
            query = query.bind(
            ...
            query = query.bind(
                sqlx_types_time_time_as_postgresql_time_not_null_vec
                    .into_iter()
                    .map(|element| element.into_inner())
                    .collect::<std::vec::Vec<sqlx::types::time::Time>>(),
            );
            query
        };
