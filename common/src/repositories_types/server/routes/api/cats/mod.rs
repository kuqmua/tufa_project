...
DynArcCombinationOfAppStateLogicTraits, >,
    ...
    println!("{:#?}", parameters);
    //
    let expected_primary_keys = parameters.payload.sqlx_types_uuid_uuid_as_postgresql_uuid_not_null_primary_key.clone();
    //
                    ...
                    Ok(value) => match value {
                        Some(value) => match sqlx::Row::try_get::<sqlx::types::uuid::Uuid, &std::primitive::str>(
                            ...
                            Ok(value) => {
                                ...
                                Some(postgresql_crud::SqlxTypesUuidUuid(value))
                            },
                            Err(error) => {
                                drop(rows);
