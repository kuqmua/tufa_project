...
fn main() {
  {
    let mut arr = ARRAY.lock().expect("ab1cc416-8e8b-40f5-aa1f-b2f5b3f471cb");
    for _ in 0..10 {
      arr.push(1);
    }
  }
  println!("Called push {} times", ARRAY.lock().expect("dc16e55f-a200-4a04-b396-4a892545e420").len());
}
 ```
...
instead of 
something.lock().expect("c64fe78e-e494-4142-9036-4c79a96931a0");
or match some none
...
mod test {
    ...
    fn test_positive() {
        ...
        assert!(x.is_ok());
        assert_eq!(PositiveNonzeroInteger::from_str("42"), Ok(x.expect("fd29d73e-f7a4-49f3-9bd7-5792a48a6140")));
    }
}
```
...
pub fn reboot_kernel() {
    File::create("/proc/sysrq-trigger")
        .expect("48a571e6-de2c-4012-be31-5e7e2bb90c50")
        .write_all(b"b")
        .expect("2e535916-81a6-4fe2-9936-8fa79a89c1ef");
}
```
