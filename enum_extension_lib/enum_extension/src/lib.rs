...
pub fn enum_extension(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let variants = match syn_derive_input.data {
        syn::Data::Enum(enum_item) => enum_item.variants.into_iter().map(|element_f0467eb6| {
            let variant_ident = element_f0467eb6.ident;
            match element_f0467eb6.fields {
    ...
    let generated = quote::quote! {
        impl #ident {
            ...
            pub fn into_vec() -> Vec<Self> {
                ...
                for element_976865e3 in {
                    use enum_extension_lib::IntoEnumIterator;
                    Self::iter()
                } {
                    self_vec.push(element_976865e3);
                }
                self_vec
            }
            pub fn into_string_name_and_variant_hashmap() -> std::collections::HashMap<String, Self> {
                ...
                for element_dfeecd17 in {
                    use enum_extension_lib::IntoEnumIterator;
                    Self::iter()
                } {
                    variants_hashmap.insert(format!("{}", element_dfeecd17), element_dfeecd17);
                }
                variants_hashmap
            }
            pub fn into_string_name_and_variant_tuple_vec() -> Vec<(String, Self)> {
                let mut variants_vec = Vec::with_capacity(Self::get_length());
                for element_538a15df in {
                    use enum_extension_lib::IntoEnumIterator;
                    Self::iter()
                } {
                    variants_vec.push((format!("{}", element_538a15df), element_538a15df));
                }
                variants_vec
            }
