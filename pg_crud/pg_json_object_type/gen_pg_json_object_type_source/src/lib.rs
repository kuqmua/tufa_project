...
use naming::{
    ...
    ReadOnlyIdsToOptVReadDefaultOptSomeVecOneElSc, ReadOnlyIdsToTwoDimalVecReadInnerSc, ReadSc,
...
pub fn gen_pg_json_object_type(input_ts: Ts2) -> Ts2 {
    ...
    .map(|(i, el)| {
        ...
        let wrap_into_v_init_ts = |ts: &dyn ToTokens|{
            quote!{#import_path_v_ts { #ValueSc: #ts }}
        };
                ...
                let ts = match &pattern {
                    ...
                    Pattern::Arr => match &is_nullable {
                        ...
                        IsNullable::True => {
                            ...
                            quote! {Self(#ValueSc.map(#ident_arr_not_null_with_id_postfix_ucc::new))}
                        }
                    },
                };
                    ...
                    let ts = match &pattern {
                        Pattern::Stdrt => match &is_nullable {
                            ...
                            IsNullable::True => quote! {
                                Self(#ValueSc.map(#ident_stdrt_not_null_as_pg_json_type_select_ts::new))
                            },
                        },
                        Pattern::Arr => match &is_nullable {
                            IsNullable::False => {
            ...
            IsNullable::False => {
                ...
                let gen_where_filter_query_part_fields_content_stdrt_not_null_ts = |is_stdrt_with_id: &IsStdrtWithId| {
                    let query_part_vrts_ts = get_vec_syn_field(is_stdrt_with_id).iter().map(|el0| {
                        ...
                        let format_ts = dq_ts(&format!("{{column}}->'{fi_str}'"));
                        quote! {
                            Self::#fi_ucc_ts(value) => #import_path::PgTypeWhereFilter::#QueryPartSc(
                                value,
                                incr,
                                &format!(#format_ts),
                                is_need_to_add_operator,
                    ...
                    match &pattern {
                        Pattern::Stdrt => match &is_nullable {
                            IsNullable::False => gen_impl_pg_type_where_filter_for_ident_ts(
                                ...
                                &{
                                    let fields_ts = gen_where_filter_query_bind_fields_content_stdrt_not_null_ts(&is_stdrt_with_id_false);
                                    quote!{
                                        match self {
                                            #fields_ts,
                        ...
                        Pattern::Arr => gen_impl_pg_type_where_filter_for_ident_ts(
                            &{
                                let el_filters_ts = vec_syn_field_with_id.iter().map(|el0| {
                                    ...
                                    quote! {
                                        Self::#el_fi_ucc(#ValueSc) => gen_el_query(
                                            #ValueSc.get_operator(),
                                            #ValueSc,
                                            #fi_dq_ts
                                        )
                                    }
                ...
                let mb_ident_with_id_stdrt_not_null_where_ts = if is_stdrt_not_null {
                    ...
                    let impl_pg_crud_pg_type_pg_type_where_filter_for_ident_with_id_stdrt_not_null_where_ts = gen_impl_pg_type_where_filter_ts(
                        ...
                        IsQueryBindMutable::False,
                        &{
                            let fields_ts = gen_where_filter_query_bind_fields_content_stdrt_not_null_ts(&is_stdrt_with_id_true);
                            quote!{
                                match self {
                                    #fields_ts,
                                    Self::#EqualUcc(#ValueSc) => pg_crud::PgTypeWhereFilter::query_bind(#ValueSc, #QuerySc),//todo mb reuse? vrt generation
                                }
                            }
                        },
                    );
                    ...
                    match &pattern {
                        ...
                        Pattern::Arr => match &is_nullable {
                            ...
                            IsNullable::True => gen_pub_new_ts(
                                ...
                                &quote! {Self(#ValueSc.map(#ident_with_id_arr_not_null_as_pg_json_type_read_ts::new))},
                            ),
                        },
                    }
                };
        ...
        let gen_fields_read_only_ids_into_opt_v_read_inner_ts = |is_stdrt_with_id: &IsStdrtWithId, params_ts: &dyn ToTokens|{
            ...
            let ts = get_vec_syn_field(is_stdrt_with_id).iter().map(|el0| {
                ...
                let ft_as_pg_json_type_test_cases_ts = gen_type_as_pg_json_type_test_cases_ts(&ft);
                let value_ts = wrap_into_v_init_ts(&{
                ...
                quote! {
                    #fi: #ft_as_pg_json_type_test_cases_ts::read_only_ids_into_opt_v_read_inner(
                        #params_ts.0.#ValueSc.#fi
                    ).map_or_else(|| Some(#value_ts), Some)
                }
            });
            let value_ts = wrap_into_v_init_ts(&quote!{ #ident_ts_6ad36393 { #(#ts),* } });
            quote!{Some(#value_ts)}
        };
            ...
            let ident_read_only_ids_ts = StructOrEnumDeriveTsStreamBuilder::new()
                ...
                .build_struct(
                    ...
                    &match &pattern {
                        Pattern::Stdrt => match &is_nullable {
                            IsNullable::False => {
                                let value_ident_read_only_ids_handle_ucc_ts = wrap_into_v_decl_ts(&ident_read_only_ids_handle_ucc);
                                quote! {(#value_ident_read_only_ids_handle_ucc_ts);}
                            },
                            IsNullable::True => {
                                let value_opt_ident_read_only_ids_stdrt_not_null_ts = wrap_into_v_decl_ts(&gen_opt_type_decl_ts(
                                    &ident_stdrt_not_null_read_only_ids_ucc
                                ));
                                quote! {
                                    (#value_opt_ident_read_only_ids_stdrt_not_null_ts);
                                }
                            }
                        },
                        Pattern::Arr => match &is_nullable {
                            IsNullable::False => {
                                let value_vec_ident_with_id_stdrt_not_null_read_only_ids_ts = wrap_into_v_decl_ts(&gen_vec_tokens_decl_ts(
                                    &ident_with_id_stdrt_not_null_read_only_ids_ucc
                                ));
                                quote! {
                                    (#value_vec_ident_with_id_stdrt_not_null_read_only_ids_ts);
                                }
                            },
                            IsNullable::True => {
                                let value_opt_ident_with_id_read_only_ids_arr_not_null_ts = wrap_into_v_decl_ts(&gen_opt_type_decl_ts(
                                    &SelfReadOnlyIdsUcc::from_tokens(&gen_ident_ucc(&IdentPattern::ArrNotNullWithId))
                                ));
                                quote! {(#value_opt_ident_with_id_read_only_ids_arr_not_null_ts);}
                            }
                        },
                    }
                );
                ...
                let ident_with_id_stdrt_not_null_read_only_ids_ts = {
                    ...
                    let ident_with_id_stdrt_not_null_read_only_ids_ts = StructOrEnumDeriveTsStreamBuilder::new()
                        ...
                        .build_struct(
                            ...
                            &Ts2::new(),
                            &{
                                let value_ident_with_id_stdrt_not_null_read_only_ids_handle_ts = wrap_into_v_decl_ts(
                                    &ident_with_id_stdrt_not_null_read_only_ids_handle_ucc
                                );
                                quote!{(pub #value_ident_with_id_stdrt_not_null_read_only_ids_handle_ts);}
                            }
                        );
                    quote! {
                    ...
                    .build_enum(
                        ...
                        &{
                            let vrts_ts = vec_syn_field.iter().map(|el0| {
                                ...
                                let value_ft_as_json_type_update_ts = wrap_into_v_decl_ts(
                                    &gen_type_as_pg_json_type_update_ts(&el0.type0)
                                );
                                quote! {
                                    #[serde(rename(serialize = #fi_dq_ts, deserialize = #fi_dq_ts))]
                                    #vrt_ident_ucc_ts(#value_ft_as_json_type_update_ts)
                                }
                            });
                            quote!{{#(#vrts_ts),*}}
                        }
                        ...
                        Pattern::Stdrt => match &is_nullable {
                            IsNullable::False => {
                                let match_vrts_ts = vec_syn_field.iter().map(|el0| {
                                    ...
                                    quote! {
                                        #ident_stdrt_not_null_update_for_query_el_ucc::#fi_ucc(#ValueSc) => {
                        ...
                        Pattern::Arr => match &is_nullable {
                            IsNullable::False => {
                                let match_vrts_ts = vec_syn_field.iter().map(|el0| {
                                    ...
                                    quote! {
                                        #ident_stdrt_not_null_update_for_query_el_ucc::#fi_ucc(#ValueSc) => match #ft_as_pg_json_type_ts::#SelectOnlyUpdatedIdsQueryPartSc(
                                            &#ValueSc.#ValueSc,
                                            #fi_dq_ts,
                                            "elem",
                                            #IncrSc
                    ...
                    .build_enum(
                        ...
                        &{
                            let vrts_ts = vec_syn_field.iter().map(|el0| {
                                ...
                                let vrt_ident_ucc_ts = ToTokensToUccTs::case_or_panic(&fi);
                                let fi_dq_ts = gen_fi_dq_ts(el0);
                                let value_ft_as_json_type_update_for_query_ts = wrap_into_v_decl_ts(&gen_type_as_pg_json_type_update_for_query_ts(&el0.type0));
                                quote! {
                                    #[serde(rename(serialize = #fi_dq_ts, deserialize = #fi_dq_ts))]
                                    #vrt_ident_ucc_ts(#value_ft_as_json_type_update_for_query_ts)
                                }
                            });
                            quote!{{#(#vrts_ts),*}}
                        }
                    ...
                    &{
                        let vrts_ts = vec_syn_field.iter().map(|el0| {
                            ...
                            let value_init_ts = gen_import_path_v_init_ts(&{
                                ...
                                quote!{
                                    #ft_as_json_type_update_for_query_ts::from(v_121f1c54.#ValueSc)
                                }
                            });
                            quote! {
                                #ident_stdrt_not_null_update_el_ucc::#vrt_ident_ucc_ts(v_121f1c54) => #SelfUcc::#vrt_ident_ucc_ts(#value_init_ts)
                            }
                        });
                        quote!{
                            match #ValueSc {
        ...
        let (impl_pg_crud_pg_json_type_for_ident_ts, mb_impl_pg_crud_pg_types_pg_type_pg_type_ts) = {
            ...
            let impl_pg_crud_pg_json_type_for_ident_ts = gen_impl_pg_json_type_ts(
                ...
                &match &pattern {
                            ...
                            match #VSc.#SelectQueryPartSc(
                                ...
                                Ok(v_156121ad) => Ok(
                                    if is_pg_type {
                                        ...
                                        format!("jsonb_build_object('{fi}',jsonb_build_object('value',{v_156121ad}))")
                                    }
                                ),
                                Err(#ErSc) => Err(#ErSc)
                            }
                        ...
                        IsNullable::True => {
                            ...
                            quote! {
                                ...
                                match #ident_stdrt_not_null_as_pg_json_type_ts::#SelectQueryPartSc(
                                    ...
                                    Ok(v_1f8de96a) => Ok(
                                        format!("jsonb_build_object('{fi}',jsonb_build_object('value',case when jsonb_typeof({column_field_fi}) = 'null' then 'null'::jsonb else ({v_1f8de96a}) end))")
                                    ),
                                    Err(#ErSc) => Err(#ErSc)
                                }
                            }
                    ...
                    Pattern::Arr => match &is_nullable {
                        IsNullable::False => {
                            ...
                            let format_ts = dq_ts(&format!(
                                "jsonb_build_object('{{fi}}',jsonb_build_object('value',case when (jsonb_array_length({{column_field}}->'{{fi}}') = 0) then '[]'::jsonb else (select jsonb_agg(({{{ident_with_id_stdrt_not_null_select_sc}}})) from jsonb_array_elements((select {{column_field}}->'{{fi}}')) with ordinality where ordinality between {{dim1_start}} and {{dim1_end}}) end ))"
                            ));
                            quote! {
                                let #ident_with_id_stdrt_not_null_select_sc = {
                        ...
                        IsNullable::True => {
                            let format_ts = dq_ts(
                                &"case when jsonb_typeof({column_field}->'{fi}') = 'null' then jsonb_build_object('{fi}',jsonb_build_object('value','null'::jsonb)) else ({v_d7bbd03c}) end"
                            );
                            let ident_with_id_arr_not_null_as_pg_json_type_select_as_default_but_opt_is_some_ts = gen_ident_as_default_but_opt_is_some_ts(
                                &ident_with_id_arr_not_null_as_pg_json_type_select_ts
                            );
                        ...
                        let ts_a5fb2689 = {
                            let ts = {
                                let acc_push_ts = get_vec_syn_field(match &pattern {
                                    ...
                                    let fi = &el0.ident;
                                    let format_ts = dq_ts(&format!("jsonb_build_object('{fi}',{{}})||"));
                                    let ft_as_pg_json_type_ts = gen_type_as_pg_json_type_ts(&el0.type0);
                                    let ts = match &pattern {
                                        Pattern::Stdrt => {
                    ...
                    let gen_impl_into_inner_for_ident_read_or_ident_with_id_stdrt_not_null_read_ts = |is_stdrt_with_id: &IsStdrtWithId| {
                        ...
                        let ts = get_vec_syn_field(is_stdrt_with_id).iter().map(|el0| {
                            ...
                            let ts = wrap_into_v_init_ts(&{
                                ...
                                let params_ts = quote!{v_6e5af985.#ValueSc};
                ...
                &match &pattern {
                    Pattern::Stdrt => match &is_nullable {
                        IsNullable::False => {
                            let update_query_bind_vrts_ts = vec_syn_field.iter().map(|el0| {
                                let vrt_ident_ucc_ts = AsRefStrToUccTs::case_or_panic(&el0.ident.to_string());
                                ...
                                quote! {
                                    #ident_update_for_query_el_ucc::#vrt_ident_ucc_ts(v_b27f5b09) => {
                                        match #ft_as_crud_pg_json_type_from_field_ts::#UpdateQueryBindSc(
                                            v_b27f5b09.#ValueSc,
                                            #QuerySc
                                        ) {
                                            Ok(v_a4870bad) => {
                        ...
                        IsNullable::True => quote! {
                            match #VSc.0 {
                                Some(v_269a0d34) => #ident_stdrt_not_null_as_pg_json_type_ts::update_query_bind(
                                    v_269a0d34,
                                    #QuerySc
                                ),
                    ...
                    Pattern::Arr => match &is_nullable {
                        IsNullable::False => quote! {
                            for el in #VSc.#UpdateSc.into_vec() {
                                ...
                                match #ident_stdrt_not_null_as_pg_json_type_ts::update_query_bind(
                                    el.#FieldsSc,
                                    #QuerySc
                                ) {
                                    Ok(v_2073f07a) => {
                        ...
                        IsNullable::True => quote! {
                            match #VSc.0 {
                                Some(v_a2156b3e) => #ident_arr_not_null_as_pg_json_type_ts::update_query_bind(
                                    v_a2156b3e,
                                    #QuerySc
                                ),
                                None => if let Err(#ErSc) = #QuerySc.try_bind(sqlx::types::Json(#self_as_pg_json_type_update_ts::new(None))) {
                ...
                &quote!{
                    match #VSc.#SelectOnlyUpdatedIdsQueryPartSc(
                        ...
                        Ok(v_e137951b) => Ok(format!("'{fi}',jsonb_build_object('value',{v_e137951b}),")),
                        Err(#ErSc) => Err(#ErSc)
                    }
                },
                ...
                &match &pattern {
                    Pattern::Stdrt => match &is_nullable {
                            ...
                            let match_ts = vec_syn_field.iter().map(|el0| {
                                ...
                                quote! {
                                    #ident_stdrt_not_null_update_for_query_el_ucc::#fi_ucc(v_b79c2851) => {
                                        match #ft_as_pg_json_type_ts::#SelectOnlyUpdatedIdsQueryBindSc(
                                            &v_b79c2851.#ValueSc,
                                            #QuerySc
                                        ) {
                                            Ok(v_e8914f75) => {
                        ...
                        IsNullable::True => quote!{
                            if let Some(v_6334d77d) = &#VSc.0 {
                                match #ident_stdrt_not_null_as_pg_json_type_ts::#SelectOnlyUpdatedIdsQueryBindSc(v_6334d77d, #QuerySc) {
                                    Ok(v_0bd3ba6f) => {
                                        #QuerySc = v_0bd3ba6f;
                                    },
                                    Err(#ErSc) => {
                    ...
                    Pattern::Arr => match &is_nullable {
                        IsNullable::False => {
                            let select_only_created_ids_query_bind_ts = vec_syn_field_with_id.iter().map(|el0| {
                                let fi = &el0.ident;
                                let ft_as_pg_json_type_ts = gen_type_as_pg_json_type_ts(&el0.type0);
                                quote! {
                                    match #ft_as_pg_json_type_ts::#SelectOnlyCreatedIdsQueryBindSc(
                                        &el_5fba4c1f.#fi,
                                        #QuerySc
                                    ) {
                                        Ok(v_cb81ec2c) => {
                            ...
                            quote!{
                                for el in #VSc.#UpdateSc.to_vec() {
                                    ...
                                    match #ident_stdrt_not_null_as_pg_json_type_ts::#SelectOnlyUpdatedIdsQueryBindSc(
                                        &el.fields,
                                        #QuerySc
                                    ) {
                                        Ok(v_4b52fa4f) => {
                        ...
                        IsNullable::True => quote!{
                            if let Some(v_107e6639) = &#VSc.0 {
                                match #ident_arr_not_null_as_pg_json_type_ts::#SelectOnlyUpdatedIdsQueryBindSc(v_107e6639, #QuerySc) {
                                    Ok(v_ecf1b8de) => {
                                        #QuerySc = v_ecf1b8de;
                                    },
                                    Err(#ErSc) => {
                ...
                &match &pattern {
                    ...
                    Pattern::Arr => match &is_nullable {
                        IsNullable::False => {
                            ...
                            quote!{
                                Ok(format!(
                                    "'{fi}',jsonb_build_object('value',(select jsonb_agg({}) from jsonb_array_elements({}) as elem where elem->>'id' in ({}))),",
                                    {
                    ...
                    Pattern::Arr => match &is_nullable {
                        IsNullable::False => {
                            let ts = vec_syn_field_with_id.iter().map(|el0| {
                                ...
                                let ft_as_pg_json_type_ts = gen_type_as_pg_json_type_ts(&el0.type0);
                                quote! {
                                    match #ft_as_pg_json_type_ts::#SelectOnlyCreatedIdsQueryBindSc(&el_9bdcd847.#fi, #QuerySc) {
                                        Ok(v_ade27463) => {
                                            #QuerySc = v_ade27463;
                                        }
                                        Err(#ErSc) => {
        ...
        let impl_pg_json_type_test_cases_for_ident_ts = {
                ...
                Pattern::Stdrt => match &is_nullable {
                    IsNullable::False => {
                        ...
                        let ts = vec_syn_field.iter().map(|el0| {
                            ...
                            let value_ts = wrap_into_v_init_ts(&quote!{el_2720df8a});
                            quote! {
                    ...
                    IsNullable::False => {
                        let ts = vec_syn_field.iter().map(|el0| {
                            ...
                            let fields_ts = vec_syn_field.iter().map(|el_value| {
                                ...
                                if fi == fi_dd46e0cb {
                                    let value_ts = wrap_into_v_init_ts(&quote!{el_18d1f553});
                                    quote! {
                            ...
                            let ft_as_pg_json_type_test_cases_ts = gen_type_as_pg_json_type_test_cases_ts(&el0.type0);
                            let value_ts = wrap_into_v_init_ts(&quote!{el.0.#ValueSc.#IdSc.0.#ValueSc});
                            quote! {
            ...
            let read_only_ids_into_opt_v_read_inner_ts = match &pattern {
                Pattern::Stdrt => match &is_nullable {
                    ...
                    IsNullable::True => {
                        let value_ts = wrap_into_v_init_ts(&quote!{
                            #VSc.0.#ValueSc.and_then(|v_5d7e3961| match #ident_stdrt_not_null_as_pg_json_type_test_cases_ts::read_only_ids_into_opt_v_read_inner(
                                v_5d7e3961
                            ) {
                ...
                Pattern::Arr => match &is_nullable {
                    IsNullable::False => {
                        let value_ts = wrap_into_v_init_ts(&{
                            ...
                            quote!{
                                #VSc.0.#ValueSc.into_iter().fold(Vec::new(), |mut acc_cf4743b1, el_6603f209| {
                    ...
                    IsNullable::True => {
                        let value_ts = wrap_into_v_init_ts(&quote!{
                            #VSc.0.#ValueSc.and_then(|v_f816032d| match #ident_arr_not_null_as_pg_json_type_test_cases_ts::#ReadOnlyIdsIntoOptVReadInnerSc(
                                v_f816032d
                            ) {
            ...
            let read_only_ids_to_opt_v_read_default_opt_some_vec_one_el_ts = {
                let value_init_ts = gen_import_path_v_init_ts(&match &pattern {
                    Pattern::Stdrt => match &is_nullable {
                        IsNullable::False => {
                            let params_ts = vec_syn_field.iter().map(|el0| {
                                let fi = &el0.ident;
                        ...
                        IsNullable::True => quote! {
                            #ident_read_ucc::new(
                                #VSc.0.#ValueSc.as_ref().and_then(|v_dfa7815e| match #ident_stdrt_not_null_as_pg_json_type_test_cases_ts::read_only_ids_to_opt_v_read_default_opt_some_vec_one_el(
                                    v_dfa7815e
                                ) {
                    ...
                    Pattern::Arr => match &is_nullable {
                        IsNullable::False => {
                            ...
                            quote! {
                                #ident_read_ucc::new({
                                    let mut acc_5f587d35 = #VSc.0.#ValueSc.clone().into_iter().map(|el_629b1544|{
                        ...
                        IsNullable::True => quote! {
                            #ident_read_ucc::new(
                                #VSc.0.#ValueSc.as_ref().and_then(|v_16ab4136| match #ident_arr_not_null_as_pg_json_type_test_cases_ts::read_only_ids_to_opt_v_read_default_opt_some_vec_one_el(
                                    v_16ab4136
                                ) {
