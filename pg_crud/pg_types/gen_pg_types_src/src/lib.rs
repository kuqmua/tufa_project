    ...
    .map(|(i, el)| {
        ...
        let (ser_derive_or_impl, de_derive_or_impl) = if matches!(&is_stdrt_nn, IsStdrtNn::True) {
            ...
            (
                ...
                },
                {
                    ...
                    let serde_deserializer_de_struct_visitor_ts = {
                        quote! {
                    ...
                    let serde_deserializer_de_newtype_struct_ts = quote! {
                    ...
                    let enum_field_two_ts = {
                        let fields_ts = {
                            let fields_ts = param_nbr_two.get_vec_from_i_starting_with_zero().into_iter().map(&gen_field_i_ts);
                            quote! {#(#fields_ts),*}
                        };
                        quote! {
                    ...
                    let match_origin_try_new_for_de_two_ts = {
                        let fields_ts = gen_vec_field_i_ts(2);
                        quote! {
                    ...
                    let origin_new_for_de_two_ts = {
                        let fields_ts = gen_vec_field_i_ts(2);
                        quote! {
                            Ok(#ident_stdrt_nn_origin_ucc::new_for_deserialize(#fields_ts))
                        }
                    };
                    let (fn_visit_newtype_struct_uuid_ts, fn_visit_newtype_struct_mac_address_ts) = {
                    ...
                    let (
                        ...
                        (
                            ...
                            gen_ts(&{
                                ...
                                quote! {
                                    #fields_init_ts
                                    #match_origin_try_new_for_de_two_ts
                                }
                            }),
                            gen_ts(&{
                    ...
                    let fn_visit_u64_two_ts = {
                        let fields_ts = {
                            param_nbr_two.get_vec_from_i_starting_with_zero().into_iter().map(|el0| {
                    ...
                    let fn_visit_str_v_start_end_ts = {
                        let fields_ts = start_end_display_plus_to_tokens_arr.iter().enumerate().map(|(i_e1c5acfd, el0)| {
                    ...
                    let fn_visit_bytes_start_end_ts = {
                        let fields_ts = start_end_display_plus_to_tokens_arr.iter().enumerate().map(|(i_545c3b1e, el0)| {
                    ...
                    let (
                        ...
                        let match_field_init_start_end_ts = {
                            let fields_init_ts = start_end_display_plus_to_tokens_arr.iter().enumerate().map(|(i_e1adef1a, el0)| {
                    ...
                    let const_fields_start_end_ts = {
                        let field_names_ts = start_end_display_plus_to_tokens_arr.iter().map(|el0| dq_ts(&el0));
                        quote! {
                            #[doc(hidden)]
                    ...
                    let impl_serde_de_visitor_for_field_visitor_ts_f4d8cc33 = {
                        let ts = quote! {
        ...
        let ident_origin_ts = {
            ...
            let impl_ident_origin_ts = {
                ...
                let mb_fn_new_or_try_new_for_de_token = match &pg_type_pattern {
                    PgTypePattern::Stdrt => match &is_nl {
                                ...
                                PgTypeImplNewForDeserializeOrTryNewForDe::TryNewForDe(pg_type_impl_try_new_for_deserialize) => {
                                    let params_ts = {
                                        ...
                                        match &pg_type_impl_try_new_for_deserialize {
                                            ...
                                            PgTypeImplTryNewForDe::SqlxTypesUuidUuidAsUuidV4InitByPg => todo!(),
                                        }
                                    };
                                    let ts = {
                    ...
                    PgType::SqlxPgTypesPgIntervalAsInterval => quote!{
                        impl From<(i32,i32,i64)> for #ident_origin_ucc {
                            fn from(v: (i32,i32,i64)) -> Self {
                                Self::new_for_deserialize(v.0, v.1, v.2)
                            }
                        }
                    },
                    PgType::SqlxTypesChronoNaiveDateTimeAsTimestamp | PgType::SqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTz => quote!{//todo reuse naming
                        impl From<(SqlxTypesChronoNaiveDateAsNnDateOrigin,SqlxTypesChronoNaiveTimeAsNnTimeOrigin)> for #ident_origin_ucc {
                            fn from(v: (SqlxTypesChronoNaiveDateAsNnDateOrigin,SqlxTypesChronoNaiveTimeAsNnTimeOrigin)) -> Self {
                                Self::new_for_deserialize(v.0, v.1)
                            }
                        }
                    },
                }
                        ...
                        impl TryFrom<String> for #ident_origin_ucc {
                            ...
                            fn try_from(v: String) -> Result<Self, Self::Error> {
                                match uuid::Uuid::try_parse(&v) {
                                    Ok(v) => Self(v),
                                    Err(er) => Err(er)//here todo maybe cast to String
                                }
                            }
                        }
                    },
            ...
            quote! {
                ...
                #mb_pub_enum_ident_stdrt_nn_origin_try_new_for_de_er_ts
        ...
        let ident_tt_ts = {
            ...
            let impl_sqlx_type_for_ident_tt_ts = gen_impl_sqlx_type_for_ident_ts(&ident_tt_ucc, &ident_origin_ucc);
            let impl_sqlx_encode_sqlx_pg_for_ident_tt_ts = gen_impl_sqlx_encode_sqlx_pg_for_ident_ts(&ident_tt_ucc, &quote! {#SelfSc.0});
            let impl_sqlx_decode_sqlx_pg_for_ident_tt_ts = gen_impl_sqlx_decode_sqlx_pg_for_ident_ts(&ident_tt_ucc, &ident_origin_ucc, &quote! {Ok(Self(v))});
            //todo rewrite as dependency of PgType trait?
            let impl_pg_type_equal_oprtr_for_ident_tt_ts = impl_pg_type_equal_oprtr_for_ident_ts(
                &import,
                &ident_tt_ucc,
                //todo
                &{
            ...
            quote! {
                ...
                #impl_pg_type_equal_oprtr_for_ident_tt_ts
            }
        };
                            ...
                            <#ident_stdrt_nn_ucc as #import::PgTypeTestCases>::pg_type_opt_vec_wh_greater_than_test().map(
                                ...
                                #import::NotEmptyUniqueVec::try_new(
                                    ...
                                    .map(|el_504739e6| #import::PgTypeGreaterThanTest {
                                        ...
                                        greater_than: #ident_tt_ucc(#ident_origin_ucc(Some(el_504739e6.greater_than.0))),
                                    })
                                    .collect()
                                ).expect("63ce5df3")
                            )
