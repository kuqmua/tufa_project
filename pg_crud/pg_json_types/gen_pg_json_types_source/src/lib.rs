...
use naming::{
    ...
    PgJsonTypeUcc, QuerySc, ReadInnerUcc, ReadOnlyIdsAndCreateIntoReadSc,
    ReadOnlyIdsAndCreateIntoVecWhereEqualUsingFieldsSc, ReadOnlyIdsAndCreateIntoWhereEqualSc,
    ReadOnlyIdsSc, ReadOnlyIdsToTwoDimsVecReadInnerSc, ReadSc, SelfSc, SelfUcc, StringUcc,
...
pub fn gen_pg_json_types(input_ts: &Ts2) -> Ts2 {
    ...
    .map(|(i, el)| {
        ...
        let is_stdrt_not_null_uuid = if matches!((&is_nullable, &pattern, &pg_json_type), (IsNullable::False, Pattern::Stdrt, PgJsonType::UuidUuidAsJsonbString)) {
        ...
        let gen_ident_ts = |is_nullable_ddf79d44: &IsNullable, pattern_2c09ee59: &Pattern| {
            ...
            let (rust_part, pg_part) = match &pattern_2c09ee59 {
                Pattern::Stdrt => (rust_type_name.to_string(), pg_json_type_name.to_string()),
        ...
        let ident_read_inner_stdrt_not_null_al_ts = {
        ...
        let gen_pub_new_or_fn_new_ts = |const_new_ts: &dyn ToTokens, new_ts: &dyn ToTokens|match mb_const_fn {
            ConstFn::False => gen_pub_fn_new_v_ident_read_inner_content_ts(
                &new_ts
            ),
            ConstFn::True => gen_pub_const_fn_new_v_ident_read_inner_content_ts(
                &const_new_ts
            ),
        };
        let pub_new_or_const_new_self_ident_origin_new_v_ts = gen_pub_new_or_fn_new_ts(
        ...
        |quote! {
            match #import::NotEmptyUniqueVec::try_new(#param_ts) {
                Ok(#ok_v_ts) => #ok_ts,
                Err(er) => match er {
                    #import::NotEmptyUniqueVecTryNewEr::IsEmpty {..} => #is_empty_ts,
                    #import::NotEmptyUniqueVecTryNewEr::NotUnique {..} => #not_unique_ts
                }
            }
        };
        ...
        let ident_where_ts = match &is_nullable {
            IsNullable::False => gen_pg_type_where_ts(
                ...
                &{
                    ...
                    match &pattern {
                        Pattern::Stdrt => {
                            let common_stdrt_pg_json_type_filters = {
                            ...
                            match &pg_json_type_specific {
                                PgJsonTypeSpecific::Nbr => {
                                    let mut vec = common_stdrt_pg_json_type_filters;
                                ...
                                PgJsonTypeSpecific::Bool => common_stdrt_pg_json_type_filters,
        ...
        let ident_read_only_ids_stdrt_not_null_ucc = SelfReadOnlyIdsUcc::from_tokens(&ident_stdrt_not_null_ucc);
        ...
        let mb_impl_pg_json_type_object_vec_el_id_for_ident_origin_ts = if matches!(&is_stdrt_not_null_uuid, IsStdrtNotNullUuid::True) {
            let (qb_string_as_pg_text_create_for_query_ts, qb_string_as_pg_text_update_for_query_ts) = {
                ...
                let gen_ts = |create_or_update_for_query: &CreateOrUpdateForQuery| {
                    ...
                    quote! {
                        fn #name_ts(
                            #VSc: <Self::PgJsonType as #import::PgJsonType>::#type_ts,
        ...
        let impl_pg_json_type_test_cases_for_ident_ts = {
            ...
            let opt_vec_create_ts = {
                let gen_some_acc_content_ts = |is_nullable_c964bb93: &IsNullable, ident_ts_dc0d5797: &dyn ToTokens| {
                    let (new_content_ts, mb_acc_push_none_ts) = match &is_nullable_c964bb93 {
                        ...
                        IsNullable::True => (quote! {Some(el_88131059.0.into())}, quote! {acc_50e99088.push(<Self as #import::PgJsonType>::Create::new(#NoneTs));}),
                    };
                    ...
                    let mb_acc_push_long_vec_ts = match &is_nullable {
                        IsNullable::False => quote! {
                            ...
                            if !acc_27624e5e.is_empty() {
                                acc_50e99088.push(<Self as #import::PgJsonType>::Create::new(acc_27624e5e));
                            }
                        },
                    ...
                    quote! {Some({
                        ...
                        if let Some(v_8de026a4) = <#ident_ts_dc0d5797 as #import::PgJsonTypeTestCases>::#OptVecCreateSc() {
                            for el_88131059 in v_8de026a4 #mb_dot_clone_ts {
                                acc_50e99088.push(<Self as #import::PgJsonType>::Create::new(#new_content_ts));
                            }
                ...
                let content_ts = match &pattern {
                    Pattern::Stdrt => match &is_nullable {
                        IsNullable::False => quote! {
                            Some(
                                #import::#stdrt_not_null_test_cases_vec_name_ts().into_iter().map(<Self as #import::PgJsonType>::Create::new).collect()
                            )
                        },
                        IsNullable::True => gen_some_acc_content_ts(is_nullable, &gen_ident_ts(&IsNullable::False, &Pattern::Stdrt)),
                    },
            ...
            let read_only_ids_to_two_dims_vec_read_inner_ts = {
                let (has_len_greater_than_one_ts, has_len_greater_than_one_for_for_ts) = {
                    ...
                    (
                        ...
                        gen_ts(&quote! {{
                            ...
                            for el_4a00ab02 in &read_only_ids_to_two_dims_vec_read_inner {
                ...
                let gen_acc_content_handle_ts = |ident_ts_416231d8: &dyn ToTokens, has_len_greater_than_one_content_ts: &dyn ToTokens| {
                    ...
                    let opt_extra_content_ts = {
                        ...
                        quote! {
                            let opt_extra = {
                                ...
                                for el_c4f9bf8f in &read_only_ids_to_two_dims_vec_read_inner {
                    ...
                    let acc_push_vec_content_ts = {
                        let content_ts = {
                            let inner_content_ts = quote! {{
                                ...
                                for el_640f58e8 in read_only_ids_to_two_dims_vec_read_inner {
                    ...
                    quote! {
                        ...
                        let read_only_ids_to_two_dims_vec_read_inner = <
                            #ident_ts_416231d8
                            as
                            #import::PgJsonTypeTestCases
                        >::#ReadOnlyIdsToTwoDimsVecReadInnerSc(
                ...
                let content_ts = match &pattern {
                    Pattern::Stdrt => match &is_nullable {
                        ...
                        IsNullable::True => quote! {
                            ...
                            for el_8f3646f9 in <#ident_stdrt_not_null_ucc as #import::PgJsonTypeTestCases>::#ReadOnlyIdsToTwoDimsVecReadInnerSc(&#ident_read_only_ids_stdrt_not_null_ucc(read_only_ids.0.clone())) {
            ...
            let read_only_ids_into_opt_v_read_inner_ts = {
                let content_ts = gen_v_init_ts0(&if matches!(&is_stdrt_not_null_uuid, IsStdrtNotNullUuid::True) {
                    ...
                    quote! {
                        <Self as #import::PgJsonType>::into_inner(
                            <
            ...
            let read_only_ids_and_create_into_opt_v_read_ts = {
                let ts = gen_v_init_ts0(&quote! {
                    <Self as #import::PgJsonTypeTestCases>::#ReadOnlyIdsAndCreateIntoReadSc(
                        #ReadOnlyIdsSc,
                        #CreateSc
                    )
                });
            ...
            let read_only_ids_and_create_into_vec_where_equal_using_fields_ts = quote! {
                #import::NotEmptyUniqueVec::try_new(vec![
                    <Self as #import::PgJsonTypeTestCases>::#ReadOnlyIdsAndCreateIntoWhereEqualSc(
                        #ReadOnlyIdsSc,
                        #CreateSc
                    )
                ]).expect("56eb9ad4")
            };
            let read_only_ids_and_create_into_vec_where_equal_to_json_field_ts = quote! {<Self as #import::PgJsonTypeTestCases>::#ReadOnlyIdsAndCreateIntoVecWhereEqualUsingFieldsSc(
                #ReadOnlyIdsSc,
                #CreateSc
            )};
            let (
                ...
                let gen_ts = |dim: &Dim| {
                    ...
                    let gen_dim_i_nbr_ts = |is_nullable_vec: &[&IsNullable]|{
                        ...
                        quote! {
                            Some(#import::NotEmptyUniqueVec::try_new({
            ...
            let read_only_ids_and_create_into_pg_json_type_opt_vec_where_greater_than_ts = if matches!(&pattern, Pattern::Stdrt) &&
            ...
            let read_only_ids_and_create_into_pg_json_type_opt_vec_where_between_ts = if matches!(&pattern, Pattern::Stdrt) &&
                matches!(&is_nullable, IsNullable::False)
            {
                let (
                            ...
                            match where_filters::Between::try_new(
                                ...
                                Ok(v_cdde02cc) => match pg_crud_common::NotEmptyUniqueVec::try_new(vec![
                                    ...
                                    Err(er) => match er {
                                        ...
                                        #import::NotEmptyUniqueVecTryNewEr::NotUnique {..} => panic!("5edabfcc")
                                    }
                                },
                                Err(er) => None
                            }
            ...
            let read_only_ids_and_create_into_pg_json_type_opt_vec_where_in_ts = if matches!(&pattern, Pattern::Stdrt) &&
                matches!(&is_nullable, IsNullable::False)
            {
                match &pg_json_type {
                    PgJsonType::I8AsJsonbNbr |
            ...
            let read_only_ids_and_create_into_pg_json_type_opt_vec_where_regex_ts = if matches!(&pattern, Pattern::Stdrt) &&
                matches!(&is_nullable, IsNullable::False)
            {
                match &pg_json_type {
                    PgJsonType::I8AsJsonbNbr |
            ...
            let read_only_ids_and_create_into_pg_json_type_opt_vec_where_contains_el_greater_than_ts = match &pattern {
                Pattern::ArrDim1 { dim1_is_nullable } => {
            ...
            let read_only_ids_and_create_into_pg_json_type_opt_vec_where_contains_el_regex_ts = match &pattern {
                Pattern::ArrDim1 { dim1_is_nullable } => {
                    if matches!((&is_nullable, &dim1_is_nullable), (
                        IsNullable::False,
                        IsNullable::False
