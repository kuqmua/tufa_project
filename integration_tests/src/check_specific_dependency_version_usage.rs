...
fn check_specific_dependency_version_usage() {
    ...
    let unspecified_dependencies = toml_table_workspace_members_map_vec.iter().fold(Vec::new(), |mut acc, member| {
        ...
        let cargo_toml_member_map = cargo_toml_member_content.parse::<toml::Table>().unwrap_or_else(|error| panic!("cannot parse::<toml::Table>() cargo_toml_member_content for {member} {file_error}\"{e}\""));
