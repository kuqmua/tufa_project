...
pub fn from_str(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let variants_token_stream = variant_idents.iter().map(|variant_ident| {
        let variant_ident_snake_case_token_stream = {
            ...
            variant_ident_snake_case_stringified.parse::<proc_macro2::TokenStream>().unwrap_or_else(|_| panic!("{variant_ident_snake_case_stringified} {}", constants::PARSE_PROC_MACRO2_TOKEN_STREAM_FAILED_MESSAGE))
        };
        quote::quote! {
    ...
    let error_variants_stringified = variant_idents.iter().fold(String::default(), |mut acc, variant_ident| {
        ...
        assert!(write!(acc, "\'{variant_ident_snake_case_stringified}\',").is_ok(), "error 491d4b30-5cf7-45db-88dc-d851e70cd0a9");
        acc
    });
