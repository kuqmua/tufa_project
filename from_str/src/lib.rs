...
pub fn from_str(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let data_enum = if let syn::Data::Enum(data_enum) = syn_derive_input.data {
        ...
        panic!("{proc_macro_name_ident_stringified} {} syn::Data::Enum", naming_conventions::SUPPORTS_ONLY_STRINGIFIED);
    };
    let variant_idents = data_enum
        ...
        .map(|variant| match variant.fields {
            ...
            syn::Fields::Named(_) | syn::Fields::Unnamed(_) => panic!("{proc_macro_name_ident_stringified} expected fields would be unit"),
        })
    ...
    let variants_token_stream = variant_idents.iter().map(|variant_ident| {
        let variant_ident_snake_case_token_stream = {
            ...
            variant_ident_snake_case_stringified
                ...
                .unwrap_or_else(|_| panic!("{proc_macro_name_ident_stringified} {variant_ident_snake_case_stringified} {}", macros_common::constants::PARSE_PROC_MACRO2_TOKEN_STREAM_FAILED_MESSAGE))
        };
        quote::quote! {
    ...
    let error_token_stream = {
        ...
        error_stringified
            .parse::<proc_macro2::TokenStream>()
            .unwrap_or_else(|_| panic!("{proc_macro_name_ident_stringified} {error_stringified} {}", macros_common::constants::PARSE_PROC_MACRO2_TOKEN_STREAM_FAILED_MESSAGE))
    };
