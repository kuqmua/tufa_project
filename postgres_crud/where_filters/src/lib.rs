...
gen_where_filters::gen_where_filters!({
    "postgres_types_content_write_into_gen_where_filters_postgres_types": "False",
    "postgres_json_types_content_write_into_gen_where_filters_postgres_json_types": "False",
...
impl postgres_crud_common::DefaultOptionSomeVecOneEl for EncodeFormat {
    fn default_option_some_vec_one_el() -> Self {
        Self::default()
    }
}

//difference between NotEmptyUniqueVec and PostgresJsonTypeNotEmptyUniqueVec only in postgres_crud_common::DefaultOptionSomeVecOneEl impl with different generic requirement and PostgresTypeWhereFilter
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, utoipa::ToSchema, schemars::JsonSchema)]
pub struct PostgresJsonTypeNotEmptyUniqueVec<T>(Vec<T>);
impl<T: PartialEq + Clone> PostgresJsonTypeNotEmptyUniqueVec<T> {
    #[must_use]
    ...
    pub fn try_new(
        value: Vec<T>,
    ) -> Result<Self, postgres_crud_common::NotEmptyUniqueVecTryNewErrorNamed<T>> {
        if value.is_empty() {
            return Err(
                postgres_crud_common::NotEmptyUniqueVecTryNewErrorNamed::IsEmpty {
...
impl<T: PartialEq + Clone + serde::Serialize> PostgresJsonTypeNotEmptyUniqueVec<T> {
            ...
            match postgres_crud_common::increment_checked_add_one_returning_increment(increment) {
                Ok(value) => {
                    if write!(acc_ecd78d3a, "${value},").is_err() {
                        return Err(postgres_crud_common::QueryPartErrorNamed::WriteIntoBuffer {
                            code_occurence: error_occurence_lib::code_occurence!(),
                        });
                    }
                }
...
const _: () = {
    ...
    impl<'de, T: std::fmt::Debug + PartialEq + Clone + _serde::Deserialize<'de>>
        _serde::Deserialize<'de> for PostgresJsonTypeNotEmptyUniqueVec<T>
    {
            ...
            __D: _serde::Deserializer<'de>,
        {
            ...
            {
                marker: _serde::__private228::PhantomData<PostgresJsonTypeNotEmptyUniqueVec<T>>,
            ...
            {
                type Value = PostgresJsonTypeNotEmptyUniqueVec<T>;
                fn expecting(
                    &self,
                    ...
                    _serde::__private228::Formatter::write_str(
                        __f,
                        "tuple struct PostgresJsonTypeNotEmptyUniqueVec",
                    )
                }
                #[inline]
                    ...
                    __E: _serde::Deserializer<'de>,
                {
                    ...
                    Ok(PostgresJsonTypeNotEmptyUniqueVec(__field0))
                }
                #[inline]
                    ...
                    __A: _serde::de::SeqAccess<'de>,
                {
                    let Some(__field0) = _serde::de::SeqAccess::next_element::<Vec<T>>(&mut __seq)?
                        ...
                        return Err(_serde::de::Error::invalid_length(
                            0usize,
                            &"tuple struct PostgresJsonTypeNotEmptyUniqueVec with 1 element",
                        ));
                    };
                    match PostgresJsonTypeNotEmptyUniqueVec::try_new(__field0) {
                        Ok(value) => Ok(value),
            ...
            _serde::Deserializer::deserialize_newtype_struct(
                __deserializer,
                "PostgresJsonTypeNotEmptyUniqueVec",
                __Visitor {
...
impl<T: postgres_crud_common::DefaultOptionSomeVecOneEl>
    postgres_crud_common::DefaultOptionSomeVecOneEl for PostgresJsonTypeNotEmptyUniqueVec<T>
{
    fn default_option_some_vec_one_el() -> Self {
        Self(vec![
...
impl<T> Default for PostgresJsonTypeNotEmptyUniqueVec<T> {
    fn default() -> Self {
        Self(Vec::default())
    }
}
impl<T> From<PostgresJsonTypeNotEmptyUniqueVec<T>> for Vec<T> {
    fn from(value: PostgresJsonTypeNotEmptyUniqueVec<T>) -> Self {
        value.0
    }
}
impl<'lifetime, T> postgres_crud_common::PostgresTypeWhereFilter<'lifetime>
    for PostgresJsonTypeNotEmptyUniqueVec<T>
where
...
impl postgres_crud_common::DefaultOptionSomeVecOneEl for RegexRegex {
    fn default_option_some_vec_one_el() -> Self {
...
impl postgres_crud_common::DefaultOptionSomeVecOneEl for RegularExpressionCase {
    fn default_option_some_vec_one_el() -> Self {
        Self::Sensitive
    }
}
...
impl<
    T: postgres_crud_common::DefaultOptionSomeVecOneEl
        + sqlx::Type<sqlx::Postgres>
        + for<'__> sqlx::Encode<'__, sqlx::Postgres>,
> postgres_crud_common::DefaultOptionSomeVecOneEl for Between<T>
{
    fn default_option_some_vec_one_el() -> Self {
        Self {
            start: postgres_crud_common::DefaultOptionSomeVecOneEl::default_option_some_vec_one_el(
            ),
            end: postgres_crud_common::DefaultOptionSomeVecOneEl::default_option_some_vec_one_el(),
        }
    }
}
impl<
...
> postgres_crud_common::PostgresTypeWhereFilter<'lifetime> for Between<T>
{
    fn query_bind(
        self,
        mut query: Query<'lifetime, sqlx::Postgres, PgArguments>,
...
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, utoipa::ToSchema, schemars::JsonSchema)]
pub struct PostgresTypeNotEmptyUniqueVec<T>(Vec<T>);
#[allow(clippy::arbitrary_source_item_ordering)]
impl<T: PartialEq + Clone> PostgresTypeNotEmptyUniqueVec<T> {
    pub fn try_new(
        value: Vec<T>,
    ) -> Result<Self, postgres_crud_common::NotEmptyUniqueVecTryNewErrorNamed<T>> {
        if value.is_empty() {
...
const _: () = {
    ...
    #[automatically_derived]
    impl<'de, T: std::fmt::Debug + PartialEq + Clone + _serde::Deserialize<'de>>
        _serde::Deserialize<'de> for PostgresTypeNotEmptyUniqueVec<T>
    {
        fn deserialize<__D>(__deserializer: __D) -> Result<Self, __D::Error>
        where
            __D: _serde::Deserializer<'de>,
        {
            ...
            where
                T: _serde::Deserialize<'de>,
            {
                marker: _serde::__private228::PhantomData<PostgresTypeNotEmptyUniqueVec<T>>,
                lifetime: _serde::__private228::PhantomData<&'de ()>,
            }
            #[automatically_derived]
            impl<'de, T: std::fmt::Debug + PartialEq + Clone + _serde::Deserialize<'de>>
                _serde::de::Visitor<'de> for __Visitor<'de, T>
            {
                type Value = PostgresTypeNotEmptyUniqueVec<T>;
                fn expecting(
                    &self,
                    __f: &mut _serde::__private228::Formatter<'_>,
                ) -> _serde::__private228::fmt::Result {
                    _serde::__private228::Formatter::write_str(
                        __f,
                        "tuple struct PostgresTypeNotEmptyUniqueVec",
                    )
                }
                #[inline]
                fn visit_newtype_struct<__E>(self, __e: __E) -> Result<Self::Value, __E::Error>
                ...
                {
                    let __field0: Vec<T> = <Vec<T> as _serde::Deserialize>::deserialize(__e)?;
                    Ok(PostgresTypeNotEmptyUniqueVec(__field0))
                }
                #[inline]
                fn visit_seq<__A>(self, mut __seq: __A) -> Result<Self::Value, __A::Error>
                where
                    __A: _serde::de::SeqAccess<'de>,
                {
                    let Some(__field0) = _serde::de::SeqAccess::next_element::<Vec<T>>(&mut __seq)?
                        ...
                        return Err(_serde::de::Error::invalid_length(
                            0usize,
                            &"tuple struct PostgresTypeNotEmptyUniqueVec with 1 element",
                        ));
                    };
                    match PostgresTypeNotEmptyUniqueVec::try_new(__field0) {
                        Ok(value) => Ok(value),
                        Err(error) => Err(_serde::de::Error::custom(format!("{error:?}"))),
                    }
                }
...
impl<T: postgres_crud_common::DefaultOptionSomeVecOneEl>
    postgres_crud_common::DefaultOptionSomeVecOneEl for PostgresTypeNotEmptyUniqueVec<T>
{
    fn default_option_some_vec_one_el() -> Self {
        Self(vec![
            postgres_crud_common::DefaultOptionSomeVecOneEl::default_option_some_vec_one_el(),
...
impl<T> Default for PostgresTypeNotEmptyUniqueVec<T> {
    fn default() -> Self {
        Self(Vec::default())
    }
}
impl<T> From<PostgresTypeNotEmptyUniqueVec<T>> for Vec<T> {
    fn from(value: PostgresTypeNotEmptyUniqueVec<T>) -> Self {
        value.0
    }
}
...
enum PostgresTypeOrPostgresJsonType {
    PostgresJsonType,
    PostgresType,
}
enum Variant {
...
> BoundedStdVecVec<T, LENGTH>
{
    ...
    pub fn postgres_json_type_query_part(
        &self,
        increment: &mut u64,
        column: &dyn Display,
        ...
        self.query_part(
            ...
            &PostgresTypeOrPostgresJsonType::PostgresJsonType,
            &Variant::Normal,
        )
    }
    pub fn postgres_json_type_query_part_minus_one(
        &self,
        increment: &mut u64,
        column: &dyn Display,
        is_need_to_add_logical_operator: bool,
    ...
    pub fn postgres_type_query_part(
        &self,
        increment: &mut u64,
        column: &dyn Display,
        is_need_to_add_logical_operator: bool,
    ...
    pub fn postgres_type_query_part_minus_one(
        &self,
        increment: &mut u64,
        column: &dyn Display,
        is_need_to_add_logical_operator: bool,
                ...
                Ok(value) => {
                    if write!(
                        ...
                        &match &postgres_type_or_postgres_json_type {
                            PostgresTypeOrPostgresJsonType::PostgresType => format!("[${value}]"),
                            PostgresTypeOrPostgresJsonType::PostgresJsonType => {
                                format!("->${value}")
                            }
                        }
...
impl<T: Clone + postgres_crud_common::DefaultOptionSomeVecOneEl, const LENGTH: usize>
    postgres_crud_common::DefaultOptionSomeVecOneEl for BoundedStdVecVec<T, LENGTH>
{
    fn default_option_some_vec_one_el() -> Self {
        Self(vec![<T as postgres_crud_common::DefaultOptionSomeVecOneEl>::default_option_some_vec_one_el(); LENGTH])
    }
}
////////////////
