...
pub fn try_from_env(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let try_from_env_token_stream = {
        let fields_initialization_token_stream = fields_named.iter().map(|element| {
            let element_ident = &element.ident.as_ref().expect(ident_in_none_stringified);
            let element_ident_quotes_screaming_snake_case_string = syn::LitStr::new(&naming_conventions::ToScreamingSnakeCaseStringified::new(&element_ident.to_string()), ident.span());
            let element_ident_upper_camel_case_token_stream = naming_conventions::ToUpperCamelCaseTokenStream::new_or_panic(&element_ident.to_string());
            let element_ident_wrapper_upper_camel_case_token_stream = naming_conventions::ToUpperCamelCaseTokenStream::new_or_panic(&element_ident.to_string());
            quote::quote! {
                let #element_ident = {
