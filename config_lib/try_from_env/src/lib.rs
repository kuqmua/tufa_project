...
pub fn try_from_env(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let ident_try_from_env_error_named_upper_camel_case = naming_conventions::parameter::SelfTryFromEnvErrorNamedUpperCamelCase::from_dyn_quote_to_tokens(&ident);
    ...
    let dotenv_upper_camel_case = naming_conventions::DotenvUpperCamelCase;
    let dotenv_snake_case = naming_conventions::DotenvSnakeCase;
    let std_env_var_error_upper_camel_case = naming_conventions::StdEnvVarErrorUpperCamelCase;
    let std_env_var_error_snake_case = naming_conventions::StdEnvVarErrorSnakeCase;
    let env_var_name_snake_case = naming_conventions::EnvVarNameSnakeCase;
    let try_from_std_env_var_ok_upper_camel_case = naming_conventions::TryFromStdEnvVarOkUpperCamelCase;
    let error_named_token_stream = {
        let variants_token_stream = fields_named.iter().map(|element| {
            ...
            let try_from_std_env_var_ok_self_error_named_upper_camel_case = naming_conventions::parameter::TryFromStdEnvVarOkSelfErrorNamedUpperCamelCase::from_dyn_quote_to_tokens(&element_ident);
            quote::quote! {
    ...
    let try_from_env_token_stream = {
        let fields_initialization_token_stream = fields_named.iter().map(|element| {
            let element_ident = &element.ident.as_ref().expect(ident_in_none_stringified);
            let element_ident_quotes_screaming_snake_case_string = syn::LitStr::new(&naming_conventions::ToTokensToScreamingSnakeCaseStringified::new(&element_ident), ident.span());
            let element_ident_upper_camel_case_token_stream = naming_conventions::ToTokensToUpperCamelCaseTokenStream::new_or_panic(&element_ident);
            let element_ident_wrapper_upper_camel_case_token_stream = naming_conventions::ToTokensToUpperCamelCaseTokenStream::new_or_panic(&element_ident);
            quote::quote! {
                let #element_ident = {
