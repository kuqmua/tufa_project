...
pub fn try_from_env(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let ident_try_from_env_error_named_upper_camel_case = naming::parameter::SelfTryFromEnvErrorNamedUpperCamelCase::from_dyn_quote_to_tokens(&ident);
    ...
    let error_named_token_stream = {
        let variants_token_stream = fields_named.iter().map(|element| {
            ...
            let try_from_std_env_var_ok_self_error_named_upper_camel_case = naming::parameter::TryFromStdEnvVarOkSelfErrorNamedUpperCamelCase::from_dyn_quote_to_tokens(&element_ident);
            quote::quote! {
