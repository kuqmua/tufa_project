...
pub fn compile_time_project_git_info(
    ...
    let _: std::primitive::usize =
        std::io::Read::read_to_string(&mut buf_reader, &mut git_logs_head_content).unwrap_or_else(
            |error| panic!("cannot read_to_string from {full_path} file, error: \"{error}\""),
        );
