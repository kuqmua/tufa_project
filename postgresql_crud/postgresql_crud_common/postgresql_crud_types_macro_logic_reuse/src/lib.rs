...
pub fn generate_postgresql_json_types(_input_token_stream: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    fn generate_postgresql_json_type_handle_token_stream(postgresql_json_type: &PostgresqlJsonType) -> proc_macro2::TokenStream {
        ...
        let postgresql_json_type_ident_where_element_token_stream = {
            ...
            #[derive(Debug)]
            enum MaybePostgresqlJsonTypeIdentWhereElementFilter {
                Some {
                    token_stream: proc_macro2::TokenStream,
                },
                None,
            }
            impl quote::ToTokens for MaybePostgresqlJsonTypeIdentWhereElementFilter {
                fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
                    match &self {
            ...
            if let MaybePostgresqlJsonTypeIdentWhereElementFilter::Some { token_stream } = maybe_postgresql_json_type_ident_where_element_position_equals {
                common_postgresql_json_type_vec_filters_variants.push(&position_equals);
            ...
            if let MaybePostgresqlJsonTypeIdentWhereElementFilter::Some { token_stream } = maybe_postgresql_json_type_ident_where_element_contains_all_elements_of_array {
                common_postgresql_json_type_vec_filters_variants.push(&contains_all_elements_of_array);
                common_postgresql_json_type_vec_filters_token_stream.push(token_stream);
            }
            ...
            let generate_postgresql_json_type_where_element_vec_bool_token_stream = || {
                let postgresql_json_type_ident_where_element_token_stream = generate_postgresql_type_tokens_where_element_and_postgresql_type_std_option_option_tokens_where_element_handle_token_stream(
                    &ident,
                    &{
                        let vec: std::vec::Vec<&dyn crate::filters::WhereOperatorName> = common_postgresql_json_type_vec_filters_variants.clone();
                        vec
                    },
                    &naming::parameter::PostgresqlJsonTypeSelfWhereElementUpperCamelCase::from_tokens(&ident),
                    &ShouldDeriveSchemarsJsonSchema::True,
            ...
            let generate_postgresql_json_type_where_element_vec_string_token_stream = || {
                let postgresql_json_type_ident_where_element_token_stream = generate_postgresql_type_tokens_where_element_and_postgresql_type_std_option_option_tokens_where_element_handle_token_stream(
                    ...
                    &{
                        ...
                        if let MaybePostgresqlJsonTypeIdentWhereElementFilter::Some { token_stream: _ } = maybe_postgresql_json_type_ident_where_element_position_case_insensitive_regular_expression {
                            vec.push(&position_case_insensitive_regular_expression);
                        }
                        vec
                    },
                    &naming::parameter::PostgresqlJsonTypeSelfWhereElementUpperCamelCase::from_tokens(&ident),
...
fn generate_serde_deserialize_double_quotes_token_stream(postgresql_type_ident_where_element_tokens_upper_camel_case: &dyn naming::StdFmtDisplayPlusQuoteToTokens, length: std::primitive::u64) -> (
    proc_macro2::TokenStream,
    proc_macro2::TokenStream,
    proc_macro2::TokenStream
) {
