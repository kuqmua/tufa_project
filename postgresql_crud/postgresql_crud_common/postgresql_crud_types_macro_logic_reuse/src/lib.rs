...
#[proc_macro_derive(GenerateImplSerdeDeserializeForIdentOptionsToRead)]
pub fn generate_impl_serde_deserialize_for_ident_options_to_read(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    proc_macro_common::panic_location::panic_location();
    let proc_macro_name_upper_camel_case = "GenerateImplSerdeDeserializeForIdentOptionsToRead";
    let syn_derive_input: syn::DeriveInput = syn::parse(input).unwrap_or_else(|error| panic!("{proc_macro_name_upper_camel_case} {}: {error}", proc_macro_common::constants::AST_PARSE_FAILED));
    let ident = &syn_derive_input.ident;
    let proc_macro_name_upper_camel_case_ident_stringified = format!("{proc_macro_name_upper_camel_case} {ident}");
    let data_struct = match syn_derive_input.data {
