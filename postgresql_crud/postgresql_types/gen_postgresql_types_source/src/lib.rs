use naming::{
    ...
    ExcludedUcc, GenPostgresqlTypesModSc, HourSc, IncludedEndCannotBeMaxUcc,
    ...
    NewSc, OptionUpdateSc, OptionVecCreateSc, PostgresqlTypeUcc, QuerySc,
...
#[must_use]
pub fn gen_postgresql_types(input_ts: &proc_macro2::TokenStream) -> proc_macro2::TokenStream {
    ...
    impl Display for PostgresqlTypeRange {
        fn fmt(&self, f: &mut Formatter<'_>) -> StdFmtResult {
            write!(
                f,
                "{}",
                SelfNotNullUcc::from_display(&PostgresqlType::from(self))
            )
        }
    }
    ...
    enum GenPostgresqlTypesConfigVariant {
        All,
    ...
    struct GenPostgresqlJsonTypesConfig {
        ...
        whole_content_write_into_gen_postgresql_types:
            macros_helpers::ShouldWriteTokenStreamIntoFile,
        variant: GenPostgresqlTypesConfigVariant,
    }
    #[allow(clippy::arbitrary_source_item_ordering)]
    #[derive(Debug)]
    ...
    let gen_postgresql_json_types_config =
        serde_json::from_str::<GenPostgresqlJsonTypesConfig>(&input_ts.to_string())
            .expect("80485f71-4e21-4166-94df-722326c36a29");
    let (columns_ts, postgresql_type_array) = {
        let acc_5464fefe = match gen_postgresql_json_types_config.variant {
            GenPostgresqlTypesConfigVariant::All => PostgresqlType::into_array().into_iter().fold(Vec::new(), |mut acc_4351207e, el_a897c529| {
            ...
            GenPostgresqlTypesConfigVariant::Concrete(value) => value,
        };
        {
    ...
    .fold(
        ...
        |mut acc_0562629e, el_758fe97f| {
            ...
            fn gen_postgresql_type_record_handle_vec(
                ...
                | gen_postgresql_type_record_handle_vec(current_postgresql_type_record_handle)
                .into_iter()
                .chain(once(postgresql_type_record_handle.clone()))
                .collect();
                //same pattern was in gen_postgresql_types 21.05.2025
                match (
                    ...
                    (NotNullOrNullable::Nullable, PostgresqlTypePattern::Standart) => {
                        gen_vec(PostgresqlTypeRecordHandle {
                            not_null_or_nullable: NotNullOrNullable::NotNull,
            ...
            for el_39ea25de in gen_postgresql_type_record_handle_vec(PostgresqlTypeRecordHandle {
    ...
    .map(|(index, element)| {
        ...
        let import_path_non_primary_key_postgresql_type_read_only_ids_ts = quote! {#import_path::NonPrimaryKeyPostgresqlTypeReadOnlyIds};
        ...
        let gen_import_path_value_initialization_ts = |content_ts: &dyn quote::ToTokens| postgresql_crud_macros_common::gen_value_initialization_ts(&import_path, &content_ts);
        ...
        | {
            ...
            let (rust_part, postgresql_part) = match &current_postgresql_type_pattern {
                ...
                PostgresqlTypePattern::ArrayDimension1 { dimension1_not_null_or_nullable } => {
                    ...
                    (format!("{VecOfUcc}{d1_rust}{rust_type_name}"), format!("{ArrayOfUcc}{d1}{postgresql_type_name}"))
                }
            };
            format!("{not_null_or_nullable_rust}{rust_part}{AsUcc}{current_not_null_or_nullable}{postgresql_part}")
        };
        let gen_ident_ts = |
            current_postgresql_type: &PostgresqlType,
            current_not_null_or_nullable: &NotNullOrNullable,
        ...
        let ident = &gen_ident_ts(postgresql_type, not_null_or_nullable, postgresql_type_pattern);
        let gen_ident_standart_not_null_ts = |current_postgresql_type: &PostgresqlType| gen_ident_ts(current_postgresql_type, &NotNullOrNullable::NotNull, &PostgresqlTypePattern::Standart);
        let ident_standart_not_null_ucc = gen_ident_standart_not_null_ts(postgresql_type);
        let ident_standart_nullable_ucc = gen_ident_ts(postgresql_type, &NotNullOrNullable::Nullable, &PostgresqlTypePattern::Standart);
        ...
        let gen_ts = |content_ts: &dyn quote::ToTokens, postgresql_type_or_postgresql_type_test_cases: &PostgresqlTypeOrPostgresqlTypeTestCases| {
            let trait_ts = match &postgresql_type_or_postgresql_type_test_cases {
                PostgresqlTypeOrPostgresqlTypeTestCases::PostgresqlType => quote! {PostgresqlType},
                PostgresqlTypeOrPostgresqlTypeTestCases::PostgresqlTypeTestCases => quote! {PostgresqlTypeTestCases},
            };
            quote! {<#content_ts as #import_path::#trait_ts>}
        };
        let gen_as_postgresql_type_ts = |content_ts: &dyn quote::ToTokens| gen_ts(&content_ts, &PostgresqlTypeOrPostgresqlTypeTestCases::PostgresqlType);
        let gen_as_postgresql_type_test_cases_ts = |content_ts: &dyn quote::ToTokens| gen_ts(&content_ts, &PostgresqlTypeOrPostgresqlTypeTestCases::PostgresqlTypeTestCases);
        let self_as_postgresql_type_ts = gen_as_postgresql_type_ts(&SelfUcc);
        let ident_standart_not_null_as_postgresql_type_ts = gen_as_postgresql_type_ts(&ident_standart_not_null_ucc);
        let ident_standart_nullable_as_postgresql_type_ts = gen_as_postgresql_type_ts(&ident_standart_nullable_ucc);
        let self_postgresql_type_as_postgresql_type_ts = gen_as_postgresql_type_ts(&quote! {Self::#PostgresqlTypeUcc});

        let ident_standart_not_null_as_postgresql_type_test_cases_ts = gen_as_postgresql_type_test_cases_ts(&ident_standart_not_null_ucc);
        let ident_standart_nullable_as_postgresql_type_test_cases_ts = gen_as_postgresql_type_test_cases_ts(&ident_standart_nullable_ucc);
        let ident_array_not_null_as_postgresql_type_test_cases_ts = gen_as_postgresql_type_test_cases_ts(&ident_array_not_null_ucc);
        let ident_array_nullable_as_postgresql_type_test_cases_ts = gen_as_postgresql_type_test_cases_ts(&ident_array_nullable_ucc);

        let gen_ident_standart_not_null_origin_ts = |current_postgresql_type: &PostgresqlType| SelfOriginUcc::from_tokens(
            &gen_ident_standart_not_null_ts(current_postgresql_type)
        );
        let ident_standart_not_null_origin_ucc = gen_ident_standart_not_null_origin_ts(postgresql_type);
        ...
        let sqlx_types_chrono_naive_date_as_not_null_date_origin_ucc = gen_ident_standart_not_null_origin_ts(&PostgresqlType::SqlxTypesChronoNaiveDateAsDate);
        let sqlx_types_chrono_naive_time_as_not_null_time_origin_ucc = gen_ident_standart_not_null_origin_ts(&PostgresqlType::SqlxTypesChronoNaiveTimeAsTime);
        let sqlx_types_chrono_naive_date_time_as_not_null_timestamp_origin_ucc = gen_ident_standart_not_null_origin_ts(&PostgresqlType::SqlxTypesChronoNaiveDateTimeAsTimestamp);
        let sqlx_types_chrono_date_time_sqlx_types_chrono_utc_as_not_null_timestamptz_origin_ucc = gen_ident_standart_not_null_origin_ts(&PostgresqlType::SqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTz);

        let gen_ident_standart_not_null_origin_try_new_error_named_ts = |current_postgresql_type: &PostgresqlType| SelfOriginTryNewErrorNamedUcc::from_tokens(
            &gen_ident_standart_not_null_ts(current_postgresql_type)
        );
        let sqlx_types_chrono_naive_date_as_not_null_date_origin_try_new_error_named_ucc = gen_ident_standart_not_null_origin_try_new_error_named_ts(&PostgresqlType::SqlxTypesChronoNaiveDateAsDate);
        let sqlx_types_chrono_naive_time_as_not_null_time_origin_try_new_error_named_ucc = gen_ident_standart_not_null_origin_try_new_error_named_ts(&PostgresqlType::SqlxTypesChronoNaiveTimeAsTime);
        let sqlx_types_chrono_naive_date_time_as_not_null_timestamp_origin_try_new_error_named_ucc = gen_ident_standart_not_null_origin_try_new_error_named_ts(&PostgresqlType::SqlxTypesChronoNaiveDateTimeAsTimestamp);
        let sqlx_types_chrono_date_time_sqlx_types_chrono_utc_as_not_null_timestamptz_origin_try_new_error_named_ucc = gen_ident_standart_not_null_origin_try_new_error_named_ts(&PostgresqlType::SqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTz);
        ...
        let gen_current_ident_origin_non_wrapping = |current_postgresql_type_pattern: &PostgresqlTypePattern, current_not_null_or_nullable: &NotNullOrNullable| SelfOriginUcc::from_tokens(&gen_ident_ts(postgresql_type, current_not_null_or_nullable, current_postgresql_type_pattern));
        let field_type_handle: &dyn quote::ToTokens = {
            let gen_current_ident_origin = |current_postgresql_type_pattern: &PostgresqlTypePattern, current_not_null_or_nullable: &NotNullOrNullable| {
                let value = gen_current_ident_origin_non_wrapping(current_postgresql_type_pattern, current_not_null_or_nullable);
                match &not_null_or_nullable {
                    NotNullOrNullable::NotNull => postgresql_crud_macros_common::gen_std_vec_vec_tokens_declaration_ts(&value),
                    NotNullOrNullable::Nullable => postgresql_crud_macros_common::gen_std_option_option_tokens_declaration_ts(&value),
                }
            };
            match &postgresql_type_pattern {
                PostgresqlTypePattern::Standart => match &not_null_or_nullable {
                    NotNullOrNullable::NotNull => &inner_type_standart_not_null_ts,
                    NotNullOrNullable::Nullable => &postgresql_crud_macros_common::gen_std_option_option_tokens_declaration_ts(&ident_standart_not_null_origin_ucc),
                },
                PostgresqlTypePattern::ArrayDimension1 { dimension1_not_null_or_nullable } => &{
                    ...
                    gen_current_ident_origin(current_postgresql_type_pattern, current_not_null_or_nullable)
                },
            }
        };
        ...
        let ident_inner_type_ts = match &element.postgresql_type_pattern {
            PostgresqlTypePattern::Standart => match &not_null_or_nullable {
                NotNullOrNullable::NotNull => &inner_type_standart_not_null_ts,
                NotNullOrNullable::Nullable => &postgresql_crud_macros_common::gen_std_option_option_tokens_declaration_ts(&inner_type_standart_not_null_ts),
            },
            PostgresqlTypePattern::ArrayDimension1 { dimension1_not_null_or_nullable } => &{
                let dimension1_type = dimension1_not_null_or_nullable.maybe_option_wrap(quote! {#inner_type_standart_not_null_ts});
                not_null_or_nullable.maybe_option_wrap(postgresql_crud_macros_common::gen_std_vec_vec_tokens_declaration_ts(&dimension1_type))
            },
        };
        ...
        let (serde_serialize_derive_or_impl, serde_deserialize_derive_or_impl) = if matches!(&is_standart_not_null, postgresql_crud_macros_common::IsStandartNotNull::True) {
            ...
            let serde_serialize_derive_or_impl = {
                ...
                let impl_serde_serialize_for_postgresql_type_not_null_tokens_serde_serialize_content_e5bb5640_d9fe_4ed3_9862_6943f8efee90_ts = gen_impl_serde_serialize_for_ident_standart_not_null_origin_tokens(&serde_serialize_content_e5bb5640_d9fe_4ed3_9862_6943f8efee90_ts);
                ...
                match &postgresql_type {
                    ...
                    PostgresqlType::SqlxPostgresTypesPgMoneyAsMoney => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_serialize_for_ident_standart_not_null_origin_tokens(&gen_serde_serialize_content_b5af560e_5f3f_4f23_9286_c72dd986a1b4(&quote! {.0}))),
                    PostgresqlType::SqlxTypesChronoNaiveTimeAsTime => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_serialize_for_ident_standart_not_null_origin_tokens(&{
                    ...
                    PostgresqlType::SqlxTypesTimeTimeAsTime => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_serialize_for_ident_standart_not_null_origin_tokens(&{
                    ...
                    PostgresqlType::SqlxPostgresTypesPgIntervalAsInterval => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_serialize_for_ident_standart_not_null_origin_tokens(&{
                    ...
                    PostgresqlType::SqlxTypesChronoNaiveDateTimeAsTimestamp => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_serialize_for_ident_standart_not_null_origin_tokens(&{
                        enum DateOrTime {
                            Date,
                            Time,
                        }
                    ...
                    PostgresqlType::SqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTz => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_serialize_for_ident_standart_not_null_origin_tokens(&{
                    ...
                    PostgresqlType::SqlxTypesMacAddressMacAddressAsMacAddr => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_serialize_for_ident_standart_not_null_origin_tokens(&gen_serde_serialize_content_b5af560e_5f3f_4f23_9286_c72dd986a1b4(&quote! {.bytes()}))),
                    ...
                    PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateAsDateRange => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_serialize_for_ident_standart_not_null_origin_start_end_range_tokens(&sqlx_types_chrono_naive_date_as_not_null_date_origin_ucc)),
                    PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateTimeAsTimestampRange => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_serialize_for_ident_standart_not_null_origin_start_end_range_tokens(&sqlx_types_chrono_naive_date_time_as_not_null_timestamp_origin_ucc)),
                    PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTzRange => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_serialize_for_ident_standart_not_null_origin_start_end_range_tokens(&sqlx_types_chrono_date_time_sqlx_types_chrono_utc_as_not_null_timestamptz_origin_ucc)),
                }
            };
            let serde_deserialize_derive_or_impl = {
                ...
                let struct_ident_double_quotes_ts = postgresql_crud_macros_common::gen_struct_ident_double_quotes_ts(&ident_origin_ucc);
                let tuple_struct_ident_double_quotes_ts = postgresql_crud_macros_common::gen_tuple_struct_ident_double_quotes_ts(&ident_origin_ucc);
                ...
                let gen_serde_private_ok_postgresql_type_ts = |content_ts: &dyn quote::ToTokens| gen_serde_private_ok_ts(&quote! {#ident_standart_not_null_origin_ucc(#content_ts)});
                ...
                let (fn_visit_newtype_struct_pg_money_ts, fn_visit_newtype_struct_uuid_ts, fn_visit_newtype_struct_mac_address_ts, fn_visit_newtype_struct_text_ts, fn_visit_newtype_struct_sqlx_types_chrono_naive_date_ts) = {
                    ...
                    (
                        gen_fn_visit_newtype_struct_ts(&std_primitive_i64_ts, &gen_serde_private_ok_postgresql_type_ts(&quote! {#inner_type_standart_not_null_ts(#field_0_value_ts)})),
                        gen_fn_visit_newtype_struct_ts(&std_string_string_ts, &gen_serde_private_ok_postgresql_type_ts(&match_uuid_uuid_field_type_try_parse_ts)),
                        gen_fn_visit_newtype_struct_ts(&array_std_primitive_u8_6_ts, &gen_serde_private_ok_postgresql_type_ts(&sqlx_types_mac_address_mac_address_field_type_new_field_0_value_ts)),
                ...
                let gen_fields_serde_de_seq_access_next_el_initialization_ts = |vec_ts: &[&dyn quote::ToTokens]| {
                    let error_message_ts = postgresql_crud_macros_common::gen_struct_ident_with_number_elements_double_quotes_ts(&ident_standart_not_null_origin_ucc, vec_ts.len());
                ...
                let (
                    ...
                    (
                        gen_fn_visit_seq_ts(&{
                            ...
                            let serde_private_ok_postgresql_type_ts = gen_serde_private_ok_postgresql_type_ts(&quote! {#inner_type_standart_not_null_ts(#field_0_value_ts)});
                            quote! {
                        ...
                        gen_fn_visit_seq_ts(&{
                            ...
                            let serde_private_ok_postgresql_type_ts = gen_serde_private_ok_postgresql_type_ts(&match_uuid_uuid_field_type_try_parse_ts);
                            quote! {
                                #fields_initialization_ts
                                #serde_private_ok_postgresql_type_ts
                            }
                        ...
                        gen_fn_visit_seq_ts(&{
                            ...
                            let serde_private_ok_postgresql_type_ts = gen_serde_private_ok_postgresql_type_ts(&sqlx_types_mac_address_mac_address_field_type_new_field_0_value_ts);
                            quote! {
                                #fields_initialization_ts
                                #serde_private_ok_postgresql_type_ts
                            }
                ...
                match &postgresql_type {
                    ...
                    | PostgresqlType::StdVecVecStdPrimitiveU8AsBytea
                    | PostgresqlType::SqlxTypesIpnetworkIpNetworkAsInet => postgresql_crud_macros_common::DeriveOrImpl::Derive,
                    PostgresqlType::SqlxPostgresTypesPgMoneyAsMoney => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_deserialize_for_tokens_ts(&quote! {
                        #struct_visitor_ts
                        #impl_serde_de_visitor_for_visitor_pg_money_ts
                        #serde_deserializer_deserialize_newtype_struct_ts
                    })),
                    PostgresqlType::StdStringStringAsText => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_deserialize_for_tokens_ts(&quote! {
                        #struct_visitor_ts
                        #impl_serde_de_visitor_for_visitor_std_string_string_ts
                        #serde_deserializer_deserialize_newtype_struct_ts
                    })),
                    PostgresqlType::SqlxTypesChronoNaiveTimeAsTime => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_deserialize_for_tokens_ts(&quote! {
                        #enum_field_four_ts
                        #impl_serde_de_visitor_for_field_visitor_ts_5a4f24ce_7a8e_4bcc_8f79_2494f79bcc08
                    ...
                    PostgresqlType::SqlxTypesTimeTimeAsTime => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_deserialize_for_tokens_ts(&quote! {
                        #enum_field_four_ts
                        #impl_serde_de_visitor_for_field_visitor_ts_9b240c3e_a4af_4da1_a2ab_f1bab44b1df6
                        #impl_serde_deserialize_for_field_ts
                        #struct_visitor_ts
                    ...
                    PostgresqlType::SqlxTypesChronoNaiveDateAsDate => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_deserialize_for_tokens_ts(&quote! {
                        #struct_visitor_ts
                        #impl_serde_de_visitor_for_visitor_sqlx_types_chrono_naive_date_ts
                        #serde_deserializer_deserialize_newtype_struct_ts
                    })),
                    PostgresqlType::SqlxTypesChronoNaiveDateTimeAsTimestamp => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_deserialize_for_tokens_ts(&quote! {
                        #enum_field_two_ts
                        #impl_serde_de_visitor_for_field_visitor_ts_dc439ca1_8af1_4c4c_ab49_4e4fb15a41d3
                        #impl_serde_deserialize_for_field_ts
                        #struct_visitor_ts
                    ...
                    PostgresqlType::SqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTz => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_deserialize_for_tokens_ts(&quote! {
                        #enum_field_two_ts
                        #impl_serde_de_visitor_for_field_visitor_ts_8c733fe0_c816_4a0e_bb13_4c2d0cd2ded6
                        #impl_serde_deserialize_for_field_ts
                        #struct_visitor_ts
                    ...
                    PostgresqlType::SqlxTypesMacAddressMacAddressAsMacAddr => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_deserialize_for_tokens_ts(&quote! {
                        #struct_visitor_ts
                        #impl_serde_de_visitor_for_visitor_mac_address_mac_address_ts
                        #serde_deserializer_deserialize_newtype_struct_ts
                    })),
                    PostgresqlType::SqlxPostgresTypesPgIntervalAsInterval => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_deserialize_for_tokens_ts(&quote! {
                        #enum_field_three_ts
                        #impl_serde_de_visitor_for_field_visitor_ts_f702a411_b02b_4c90_aa7f_962a698612e7
                        #impl_serde_deserialize_for_field_ts
                        #struct_visitor_ts
                    ...
                    PostgresqlType::SqlxPostgresTypesPgRangeStdPrimitiveI32AsInt4Range => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_deserialize_for_tokens_ts(&quote! {
                        #enum_field_two_ts
                        #impl_serde_de_visitor_for_field_visitor_ts_f4d8cc33_bf35_4c13_a745_341364a68df6
                        #impl_serde_deserialize_for_field_ts
                        #struct_visitor_ts
                    ...
                    PostgresqlType::SqlxPostgresTypesPgRangeStdPrimitiveI64AsInt8Range => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_deserialize_for_tokens_ts(&quote! {
                        #enum_field_two_ts
                        #impl_serde_de_visitor_for_field_visitor_ts_f4d8cc33_bf35_4c13_a745_341364a68df6
                        #impl_serde_deserialize_for_field_ts
                        #struct_visitor_ts
                    ...
                    PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateAsDateRange => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_deserialize_for_tokens_ts(&quote! {
                        #enum_field_two_ts
                        #impl_serde_de_visitor_for_field_visitor_ts_f4d8cc33_bf35_4c13_a745_341364a68df6
                        #impl_serde_deserialize_for_field_ts
                        #struct_visitor_ts
                    ...
                    PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateTimeAsTimestampRange => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_deserialize_for_tokens_ts(&quote! {
                        #enum_field_two_ts
                        #impl_serde_de_visitor_for_field_visitor_ts_f4d8cc33_bf35_4c13_a745_341364a68df6
                        #impl_serde_deserialize_for_field_ts
                        #struct_visitor_ts
                    ...
                    PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTzRange => postgresql_crud_macros_common::DeriveOrImpl::Impl(gen_impl_serde_deserialize_for_tokens_ts(&quote! {
                        #enum_field_two_ts
                        #impl_serde_de_visitor_for_field_visitor_ts_f4d8cc33_bf35_4c13_a745_341364a68df6
                        #impl_serde_deserialize_for_field_ts
                        #struct_visitor_ts
        ...
        let ident_origin_ts = {
                    ...
                    .build_enum(
                        ...
                        &{
                            ...
                            let content_ts: &dyn quote::ToTokens = match &postgresql_type_initialization_try_new {
                                ...
                                PostgresqlTypeInitializationTryNew::SqlxPostgresTypesPgRangeStdPrimitiveI32AsInt4Range => &gen_int_range_type_error_variants_ts(&IntRangeType::SqlxPostgresTypesPgRangeStdPrimitiveI32AsInt4Range),
                                PostgresqlTypeInitializationTryNew::SqlxPostgresTypesPgRangeStdPrimitiveI64AsInt8Range => &gen_int_range_type_error_variants_ts(&IntRangeType::SqlxPostgresTypesPgRangeStdPrimitiveI64AsInt8Range),
                                PostgresqlTypeInitializationTryNew::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateAsDateRange => &gen_start_end_ts(
                                    &sqlx_types_chrono_naive_date_as_not_null_date_origin_try_new_error_named_ucc
                                ),
                                PostgresqlTypeInitializationTryNew::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateTimeAsTimestampRange => &gen_start_end_ts(
                                    &sqlx_types_chrono_naive_date_time_as_not_null_timestamp_origin_try_new_error_named_ucc
                                ),
                                PostgresqlTypeInitializationTryNew::SqlxPostgresTypesPgRangeSqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTzRange => &gen_start_end_ts(
                            ...
                            .build_enum(
                                ...
                                &{
                                    let content_ts: &dyn quote::ToTokens = match &postgresql_type_impl_try_new_for_deserialize {
                                        ...
                                        PostgresqlTypeImplTryNewForDeserialize::SqlxPostgresTypesPgRangeStdPrimitiveI32AsInt4Range => &gen_int_range_type_error_variants_ts(&IntRangeType::SqlxPostgresTypesPgRangeStdPrimitiveI32AsInt4Range),
                                        PostgresqlTypeImplTryNewForDeserialize::SqlxPostgresTypesPgRangeStdPrimitiveI64AsInt8Range => &gen_int_range_type_error_variants_ts(&IntRangeType::SqlxPostgresTypesPgRangeStdPrimitiveI64AsInt8Range),
                                    };
                                    quote!{{#content_ts}}
                                }
                            );
            ...
            let impl_ident_origin_ts = {
                ...
                |()| {
                    let content_ts = {
                        let content_ts = {
                            ...
                            match &postgresql_type_pattern {
                                    ...
                                    NotNullOrNullable::NotNull => {
                                        postgresql_type_range_try_from_postgresql_type.as_ref().map_or_else(
                                            ...
                                            |value_6ed98462| gen_pg_range_conversion_ts(
                                                &ValueSc,
                                                &{
                                                    let range_postgresql_type_ident_origin = SelfOriginUcc::from_display(&gen_ident_str(&PostgresqlType::from(value_6ed98462), not_null_or_nullable, postgresql_type_pattern));
                                ...
                                PostgresqlTypePattern::ArrayDimension1 { dimension1_not_null_or_nullable } => gen_array_dimensions_initialization_ts(&{
                                    let (current_postgresql_type_pattern, current_not_null_or_nullable): (&PostgresqlTypePattern, &NotNullOrNullable) = match &not_null_or_nullable {
                                        NotNullOrNullable::NotNull => (&PostgresqlTypePattern::Standart, dimension1_not_null_or_nullable),
                                        NotNullOrNullable::Nullable => (postgresql_type_pattern, &NotNullOrNullable::NotNull),
                                    };
                                    gen_current_ident_origin_non_wrapping(current_postgresql_type_pattern, current_not_null_or_nullable)
                                }),
                            }
                        };
                    ...
                    match &postgresql_type_pattern {
                        ...
                        PostgresqlTypePattern::ArrayDimension1 { .. } => macros_helpers::gen_new_ts(
                            &must_use_ts,
                            &value_ident_inner_type_ts,
                            &content_ts
                        ),
                            ...
                            PostgresqlTypePattern::Standart => match &not_null_or_nullable {
                                NotNullOrNullable::NotNull => {
                                    ...
                                    match &postgresql_type_initialization_try_new {
                                        ...
                                        PostgresqlTypeInitializationTryNew::SqlxPostgresTypesPgRangeStdPrimitiveI32AsInt4Range => gen_int_range_check_ts(&IntRangeType::SqlxPostgresTypesPgRangeStdPrimitiveI32AsInt4Range),
                                        PostgresqlTypeInitializationTryNew::SqlxPostgresTypesPgRangeStdPrimitiveI64AsInt8Range => gen_int_range_check_ts(&IntRangeType::SqlxPostgresTypesPgRangeStdPrimitiveI64AsInt8Range),
                                        PostgresqlTypeInitializationTryNew::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateAsDateRange => gen_ok_self_sqlx_postgres_types_pg_range_ts(&sqlx_types_chrono_naive_date_as_not_null_date_origin_ucc),
                                        PostgresqlTypeInitializationTryNew::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateTimeAsTimestampRange => gen_ok_self_sqlx_postgres_types_pg_range_ts(&sqlx_types_chrono_naive_date_time_as_not_null_timestamp_origin_ucc),
                                        PostgresqlTypeInitializationTryNew::SqlxPostgresTypesPgRangeSqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTzRange => gen_ok_self_sqlx_postgres_types_pg_range_ts(&sqlx_types_chrono_date_time_sqlx_types_chrono_utc_as_not_null_timestamptz_origin_ucc),
                                    }
                                }
                                NotNullOrNullable::Nullable => gen_match_option_ts(&ident_standart_not_null_origin_ucc),
                            },
                                ...
                                PostgresqlTypeImplNewForDeserializeOrTryNewForDeserialize::NewForDeserialize(postgresql_type_impl_new_for_deserialize) => {
                                    let parameters_ts = {
                                        ...
                                        match &postgresql_type_impl_new_for_deserialize {
                                            PostgresqlTypeImplNewForDeserialize::SsqlxPostgresTypesPgIntervalAsInterval => quote! {
                                                #MonthsSc: #std_primitive_i32_ts,
                                                #DaysSc: #std_primitive_i32_ts,
                                                #MicrosecondsSc: #std_primitive_i64_ts,
                                            },
                                            PostgresqlTypeImplNewForDeserialize::SqlxTypesChronoNaiveDateTimeAsTimestamp => quote! {
                                            ...
                                            PostgresqlTypeImplNewForDeserialize::SqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTz => quote! {
                                                #DateNaiveSc: #sqlx_types_chrono_naive_date_as_not_null_date_origin_ucc,
                                                #TimeSc: #sqlx_types_chrono_naive_time_as_not_null_time_origin_ucc,
                                            },
                                            PostgresqlTypeImplNewForDeserialize::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateAsDateRange => gen_start_end_std_std_ops_bound_ts(&sqlx_types_chrono_naive_date_as_not_null_date_origin_ucc),
                                            PostgresqlTypeImplNewForDeserialize::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateTimeAsTimestampRange => gen_start_end_std_std_ops_bound_ts(&sqlx_types_chrono_naive_date_time_as_not_null_timestamp_origin_ucc),
                                            PostgresqlTypeImplNewForDeserialize::SqlxPostgresTypesPgRangeSqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTzRange => gen_start_end_std_std_ops_bound_ts(&sqlx_types_chrono_date_time_sqlx_types_chrono_utc_as_not_null_timestamptz_origin_ucc),
                                        }
                                    };
                                    let content_ts = {
                                ...
                                PostgresqlTypeImplNewForDeserializeOrTryNewForDeserialize::TryNewForDeserialize(postgresql_type_impl_try_new_for_deserialize) => {
                                    let parameters_ts = {
                                        ...
                                        match &postgresql_type_impl_try_new_for_deserialize {
                                            ...
                                            PostgresqlTypeImplTryNewForDeserialize::SqlxPostgresTypesPgRangeStdPrimitiveI32AsInt4Range => gen_value_pg_range_int_type_ts(&IntRangeType::SqlxPostgresTypesPgRangeStdPrimitiveI32AsInt4Range),
                                            PostgresqlTypeImplTryNewForDeserialize::SqlxPostgresTypesPgRangeStdPrimitiveI64AsInt8Range => gen_value_pg_range_int_type_ts(&IntRangeType::SqlxPostgresTypesPgRangeStdPrimitiveI64AsInt8Range),
                                        }
                                    };
                                    let content_ts = {
                                        ...
                                        match &postgresql_type_impl_try_new_for_deserialize {
                                            ...
                                            PostgresqlTypeImplTryNewForDeserialize::SqlxTypesChronoNaiveDateAsDate => gen_self_match_try_new_ts(
                                                &quote!{value_356f2a0b},
                                                &quote! {
            ...
            let maybe_impl_is_string_empty_for_ident_origin_ts = if matches!(&is_standart_not_null, postgresql_crud_macros_common::IsStandartNotNull::True) {
                match &not_null_or_nullable {
                    NotNullOrNullable::NotNull => match &postgresql_type {
                        ...
                        | PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateTimeAsTimestampRange
                        | PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTzRange => proc_macro2::TokenStream::new(),
                        PostgresqlType::StdStringStringAsText => postgresql_crud_macros_common::gen_impl_crate_is_string_empty_for_ident_content_ts(
                            &ident_origin_ucc,
                            &quote! {self.0.clone().is_empty()},
                        ),
                        PostgresqlType::SqlxTypesUuidUuidAsUuidV4InitializedByPostgresql |
                        PostgresqlType::SqlxTypesUuidUuidAsUuidInitializedByClient |
                        PostgresqlType::SqlxTypesMacAddressMacAddressAsMacAddr => postgresql_crud_macros_common::gen_impl_crate_is_string_empty_for_ident_content_ts(
                            &ident_origin_ucc,
            ...
            let impl_default_option_some_vec_one_el_for_ident_origin_ts = postgresql_crud_macros_common::gen_impl_postgresql_crud_common_default_option_some_vec_one_el_ts(&ident_origin_ucc, &{
                    ...
                    PostgresqlTypePattern::Standart => match &not_null_or_nullable {
                        NotNullOrNullable::NotNull => {
                            ...
                            let initialization_ts: &dyn quote::ToTokens = match &postgresql_type {
                                ...
                                | PostgresqlType::SqlxTypesChronoNaiveTimeAsTime
                                | PostgresqlType::SqlxTypesMacAddressMacAddressAsMacAddr
                                | PostgresqlType::SqlxTypesUuidUuidAsUuidV4InitializedByPostgresql => &quote! {#field_type_handle::default()},
                                PostgresqlType::SqlxTypesUuidUuidAsUuidInitializedByClient => &quote! {#ident_inner_type_ts::default()},
                                PostgresqlType::SqlxPostgresTypesPgMoneyAsMoney => &quote! {#inner_type_standart_not_null_ts(#core_default_default_default_ts)},
                                PostgresqlType::StdVecVecStdPrimitiveU8AsBytea => &quote! {vec![#core_default_default_default_ts]},
                                PostgresqlType::SqlxTypesTimeTimeAsTime => &gen_sqlx_types_time_time_from_hms_micro_unwrap_ts(&quote! {0,0,0,0}),
                                PostgresqlType::SqlxPostgresTypesPgIntervalAsInterval => &quote! {#inner_type_standart_not_null_ts {
                                ...
                                PostgresqlType::SqlxTypesChronoNaiveDateTimeAsTimestamp => &gen_sqlx_types_chrono_naive_date_time_new_ts(&quote! {
                                ...
                                PostgresqlType::SqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTz => &gen_sqlx_types_chrono_date_time_sqlx_types_chrono_utc_from_naive_utc_and_offset_ts(&gen_sqlx_types_chrono_naive_date_time_new_ts(&quote! {
                                    #sqlx_types_chrono_naive_date_as_not_null_date_origin_as_default_option_some_vec_one_el_call_ts.0,
                                    #sqlx_types_chrono_naive_time_as_not_null_time_origin_as_default_option_some_vec_one_el_call_ts.0,
                                })),
                                PostgresqlType::SqlxTypesIpnetworkIpNetworkAsInet => &quote! {
                                ...
                                PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateAsDateRange => &gen_sqlx_postgres_types_pg_range_default_option_some_vec_one_el_ts(&sqlx_types_chrono_naive_date_as_not_null_date_origin_ucc),
                                PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateTimeAsTimestampRange => &gen_sqlx_postgres_types_pg_range_default_option_some_vec_one_el_ts(&sqlx_types_chrono_naive_date_time_as_not_null_timestamp_origin_ucc),
                                PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTzRange => &gen_sqlx_postgres_types_pg_range_default_option_some_vec_one_el_ts(&sqlx_types_chrono_date_time_sqlx_types_chrono_utc_as_not_null_timestamptz_origin_ucc),
                            };
            ...
            let impl_sqlx_type_sqlx_postgres_for_ident_origin_ts = postgresql_crud_macros_common::gen_impl_sqlx_type_sqlx_postgres_for_ident_ts(&ident_origin_ucc, &field_type_handle);
            let impl_sqlx_encode_sqlx_postgres_for_ident_origin_ts = postgresql_crud_macros_common::gen_impl_sqlx_encode_sqlx_postgres_for_ident_ts(&ident_origin_ucc, &quote! {#SelfSc.0});
            let impl_sqlx_decode_sqlx_postgres_for_ident_origin_ts = postgresql_crud_macros_common::gen_impl_sqlx_decode_sqlx_postgres_for_ident_ts(&ident_origin_ucc, &field_type_handle, &{
                ...
                match &postgresql_type_pattern {
                    PostgresqlTypePattern::Standart => match &not_null_or_nullable {
                        NotNullOrNullable::NotNull => match &postgresql_type {
                            ...
                            PostgresqlType::SqlxTypesChronoNaiveDateAsDate | PostgresqlType::SqlxPostgresTypesPgRangeStdPrimitiveI32AsInt4Range | PostgresqlType::SqlxPostgresTypesPgRangeStdPrimitiveI64AsInt8Range => quote! {
            ...
            let maybe_impl_std_convert_from_ident_read_for_ident_origin_ts = match &is_not_null_standart_can_be_primary_key {
                ...
                IsNotNullStandartCanBePrimaryKey::True => macros_helpers::gen_impl_std_convert_from_ts(&ident_standart_not_null_read_ucc, &ident_origin_ucc, &{
                    let ident_standart_not_null_as_crate_postgresql_type_ts = gen_as_postgresql_type_ts(&ident_standart_not_null_ucc);
                    quote! {Self::#NewSc(#ident_standart_not_null_as_crate_postgresql_type_ts::into_inner(#ValueSc))}
                }),
            };
            quote! {
                #ident_origin_ts
        ...
        let ident_table_type_declaration_ts = {
            ...
            let impl_default_option_some_vec_one_el_for_ident_table_type_declaration_ts =
                postgresql_crud_macros_common::gen_impl_postgresql_crud_common_default_option_some_vec_one_el_ts(&ident_table_type_declaration_ucc, &quote! {Self(#postgresql_crud_common_default_option_some_vec_one_el_call_ts)});
            let impl_sqlx_type_sqlx_postgres_for_ident_table_type_declaration_ts = postgresql_crud_macros_common::gen_impl_sqlx_type_sqlx_postgres_for_ident_ts(&ident_table_type_declaration_ucc, &ident_origin_ucc);
            let impl_sqlx_encode_sqlx_postgres_for_ident_table_type_declaration_ts = postgresql_crud_macros_common::gen_impl_sqlx_encode_sqlx_postgres_for_ident_ts(&ident_table_type_declaration_ucc, &quote! {#SelfSc.0});
            let impl_sqlx_decode_sqlx_postgres_for_ident_table_type_declaration_ts = postgresql_crud_macros_common::gen_impl_sqlx_decode_sqlx_postgres_for_ident_ts(&ident_table_type_declaration_ucc, &ident_origin_ucc, &quote! {Ok(Self(value_147c3532))});
        ...
        let ident_create_ts = {
            ...
            let impl_default_option_some_vec_one_el_for_ident_create_ts = postgresql_crud_macros_common::gen_impl_postgresql_crud_common_default_option_some_vec_one_el_ts(&ident_create_ucc, &{
            ...
            let maybe_impl_sqlx_encode_sqlx_postgres_for_ident_create_ts = match &can_be_primary_key {
                CanBePrimaryKey::False => postgresql_crud_macros_common::gen_impl_sqlx_encode_sqlx_postgres_for_ident_ts(&ident_create_ucc, &quote! {#SelfSc.0}),
                CanBePrimaryKey::True => proc_macro2::TokenStream::new(),
            };
            let maybe_impl_sqlx_type_sqlx_postgres_for_ident_create_ts = match &can_be_primary_key {
                CanBePrimaryKey::False => postgresql_crud_macros_common::gen_impl_sqlx_type_sqlx_postgres_for_ident_ts(&ident_create_ucc, &ident_origin_ucc),
                CanBePrimaryKey::True => proc_macro2::TokenStream::new(),
            };
            quote! {
                #ident_create_ts
        ...
        let ident_select_ts = {
            let pub_struct_ident_select_ts = gen_pub_struct_tokens_ts(
                &ident_select_ucc,
                &match &postgresql_type_pattern {
                    PostgresqlTypePattern::Standart => quote! {;},
                    PostgresqlTypePattern::ArrayDimension1 { .. } => {
            ...
            let (impl_default_option_some_vec_one_el_for_ident_select_ts, impl_default_option_some_vec_one_el_max_page_size_for_ident_select_ts) = {
                let gen_default_content_ts = |default_some_one_or_default_some_one_with_max_page_size: &postgresql_crud_macros_common::DefaultSomeOneOrDefaultSomeOneWithMaxPageSize| match &postgresql_type_pattern {
                    PostgresqlTypePattern::Standart => quote! {Self},
                    PostgresqlTypePattern::ArrayDimension1 { .. } => {
                ...
                (
                    postgresql_crud_macros_common::gen_impl_postgresql_crud_common_default_option_some_vec_one_el_ts(&ident_select_ucc, &gen_default_content_ts(&postgresql_crud_macros_common::DefaultSomeOneOrDefaultSomeOneWithMaxPageSize::DefaultSomeOne)),
                    postgresql_crud_macros_common::gen_impl_postgresql_crud_common_default_option_some_vec_one_el_max_page_size_ts(&ident_select_ucc, &gen_default_content_ts(&postgresql_crud_macros_common::DefaultSomeOneOrDefaultSomeOneWithMaxPageSize::DefaultSomeOneWithMaxPageSize)),
                )
            };
            quote! {
        ...
        let ident_where_ts = postgresql_crud_macros_common::gen_postgresql_type_where_ts(
            &allow_clippy_arbitrary_source_item_ordering_ts,
            &{
                let common_postgresql_type_filters = vec![postgresql_crud_macros_common::PostgresqlTypeFilter::Equal { ident: quote! {#ident_table_type_declaration_ucc} }];
                match &postgresql_type_pattern {
                    PostgresqlTypePattern::Standart => {
                        ...
                        let in_handle = postgresql_crud_macros_common::PostgresqlTypeFilter::In { ident: quote! {#ident_table_type_declaration_ucc} };
                    ...
                    PostgresqlTypePattern::ArrayDimension1 { dimension1_not_null_or_nullable } => {
                        let ident_standart_not_null_or_nullable_if_can_be_nullable_table_type_declaration_ucc = {
                            let value = SelfTableTypeDeclarationUcc::from_tokens(&match &postgresql_type.can_be_nullable() {
                                CanBeNullable::False => quote! {#ident_standart_not_null_ucc},
                                CanBeNullable::True => {
                                    let value = gen_ident_ts(postgresql_type, not_null_or_nullable, &PostgresqlTypePattern::Standart);
                                    quote! {#value}
                                }
                            });
                            quote! {#value}
                        ...
                        let (
                            ...
                            let gen_where_sqlx_postgres_types_pg_range_filter_ts = |postgresql_type_range: PostgresqlTypeRange| {
                                let postgresql_type_from_postgresql_type_range = PostgresqlType::from(&postgresql_type_range);
                                let range_el_ident_standart_not_null_ts = gen_ident_standart_not_null_ts(&postgresql_type_from_postgresql_type_range);
                                let mut vec = common_array_dimension1_postgresql_type_filters.clone();
                                let range_el_ident_standart_not_null_as_crate_postgresql_type_read_ts = {
                                    let range_el_ident_standart_not_null_as_crate_postgresql_type_ts = gen_as_postgresql_type_ts(&range_el_ident_standart_not_null_ts);
                                    quote! {#range_el_ident_standart_not_null_as_crate_postgresql_type_ts::Read}
                                };
                            ...
                            (
                                gen_where_sqlx_postgres_types_pg_range_filter_ts(PostgresqlTypeRange::StdPrimitiveI32AsInt4),
                                gen_where_sqlx_postgres_types_pg_range_filter_ts(PostgresqlTypeRange::StdPrimitiveI64AsInt8),
                                gen_where_sqlx_postgres_types_pg_range_filter_ts(PostgresqlTypeRange::SqlxTypesChronoNaiveDateAsDate),
                                gen_where_sqlx_postgres_types_pg_range_filter_ts(PostgresqlTypeRange::SqlxTypesChronoNaiveDateTimeAsTimestamp),
                                gen_where_sqlx_postgres_types_pg_range_filter_ts(PostgresqlTypeRange::SqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTz),
                            )
                        };
                        match &postgresql_type {
                            PostgresqlType::StdPrimitiveI16AsInt2
        ...
        let ident_read_ts = {
            ...
            let impl_crate_default_option_some_vec_one_el_for_ident_read_ts =
                postgresql_crud_macros_common::gen_impl_postgresql_crud_common_default_option_some_vec_one_el_ts(&ident_read_ucc, &quote! {Self(#postgresql_crud_common_default_option_some_vec_one_el_call_ts)});
            let impl_sqlx_encode_sqlx_postgres_for_ident_origin_ts = postgresql_crud_macros_common::gen_impl_sqlx_encode_sqlx_postgres_for_ident_ts(&ident_read_ucc, &quote! {#SelfSc.0});
            let impl_sqlx_decode_sqlx_postgres_for_ident_read_ts = postgresql_crud_macros_common::gen_impl_sqlx_decode_sqlx_postgres_for_ident_ts(
                &ident_read_ucc,
                &ident_origin_ucc,
                &quote! {Ok(Self(value_147c3532))}
            );
            let impl_sqlx_type_sqlx_postgres_for_ident_read_ts = postgresql_crud_macros_common::gen_impl_sqlx_type_sqlx_postgres_for_ident_ts(&ident_read_ucc, &ident_origin_ucc);
        ...
        let ident_read_only_ids_ts = if matches!(&is_not_null_standart_can_be_primary_key, IsNotNullStandartCanBePrimaryKey::True) {
            ...
            let impl_sqlx_decode_sqlx_postgres_for_ident_read_only_ids_ts = postgresql_crud_macros_common::gen_impl_sqlx_decode_sqlx_postgres_for_ident_ts(
                &ident_read_only_ids_ucc,
                &ident_read_ucc,
                &quote! {Ok(Self(value_147c3532))}
            );
            let impl_sqlx_type_sqlx_postgres_for_ident_read_only_ids_ts = postgresql_crud_macros_common::gen_impl_sqlx_type_sqlx_postgres_for_ident_ts(&ident_read_only_ids_ucc, &ident_read_ucc);
            quote! {
        ...
        let ident_update_ts = {
            ...
            let impl_default_option_some_vec_one_el_for_ident_update_ts =
                postgresql_crud_macros_common::gen_impl_postgresql_crud_common_default_option_some_vec_one_el_ts(&ident_update_ucc, &quote! {Self(#postgresql_crud_common_default_option_some_vec_one_el_call_ts)});
        ...
        let ident_update_for_query_ts = {
            ...
            let impl_sqlx_type_sqlx_postgres_for_ident_update_for_query_ts = postgresql_crud_macros_common::gen_impl_sqlx_type_sqlx_postgres_for_ident_ts(&ident_update_for_query_ucc, &ident_origin_ucc);
            let impl_sqlx_encode_sqlx_postgres_for_ident_update_for_query_ts = postgresql_crud_macros_common::gen_impl_sqlx_encode_sqlx_postgres_for_ident_ts(&ident_update_for_query_ucc, &quote! {#SelfSc.0});
        ...
        let impl_postgresql_type_for_ident_ts = {
            ...
            let typical_query_part_ts = {
                let if_write_is_err_ts = macros_helpers::gen_if_write_is_err_ts(
                    ...
                    &postgresql_crud_macros_common::gen_return_err_query_part_error_named_write_into_buffer_ts(import_path)
                );
                quote! {
            ...
            let (query_part_create_ts, bind_value_to_query_create_ts): Handle<'_> = {
                ...
                match &postgresql_type {
                    ...
                    PostgresqlType::SqlxTypesUuidUuidAsUuidV4InitializedByPostgresql => (&ok_std_string_string_from_uuid_gen_v4_ts, &ok_query_ts),
                }
            };
            ...
            postgresql_crud_macros_common::gen_impl_postgresql_type_ts(
                &import_path,
                &ident,
                &ident_table_type_declaration_ucc,
                ...
                &{
                    ...
                    let maybe_array_part = match &postgresql_type_pattern {
                        ...
                        PostgresqlTypePattern::ArrayDimension1 { .. } => repeat_n("[]", array_dimensions_number).collect::<String>(),
                    };
                    ...
                    let maybe_primary_key_is_primary_key_ts = quote! {postgresql_types_common::maybe_primary_key(is_primary_key)};
                    ...
                    match (&not_null_or_nullable, &can_be_primary_key) {
                        (NotNullOrNullable::NotNull, CanBePrimaryKey::False) => {
                            let format_handle_ts = gen_quotes::double_quotes_ts(&column_postgresql_query_type_not_null);
                            quote! {
                                format!(#format_handle_ts)
                            }
                        }
                        (NotNullOrNullable::NotNull, CanBePrimaryKey::True) => {
                            let format_handle_ts = gen_quotes::double_quotes_ts(&format!("{column_postgresql_query_type_not_null}{space_additional_parameter}"));
                            quote! {
                                format!(#format_handle_ts, #maybe_primary_key_is_primary_key_ts)
                            }
                        }
                        (NotNullOrNullable::Nullable, CanBePrimaryKey::False) => {
                            let format_handle_ts = gen_quotes::double_quotes_ts(&column_postgresql_query_type);
                            quote! {
                                format!(#format_handle_ts)
                            }
                        }
                        (NotNullOrNullable::Nullable, CanBePrimaryKey::True) => {
                            let format_handle_ts = gen_quotes::double_quotes_ts(&format!("{column_postgresql_query_type}{space_additional_parameter}"));
                            quote! {
                                format!(#format_handle_ts, #maybe_primary_key_is_primary_key_ts)
                            }
                        }
                ...
                &{
                    let content_ts = match &postgresql_type_pattern {
                        PostgresqlTypePattern::Standart => quote! {#ColumnSc.to_owned()},
                        PostgresqlTypePattern::ArrayDimension1 { .. } => {
                            let format_handle_ts = gen_quotes::double_quotes_ts(&{
                    ...
                    match &postgresql_type_pattern {
                        ...
                        PostgresqlTypePattern::ArrayDimension1 { dimension1_not_null_or_nullable } => match (&not_null_or_nullable, &dimension1_not_null_or_nullable) {
                            ...
                            (NotNullOrNullable::NotNull, NotNullOrNullable::Nullable) => gen_ident_read_ident_origin_ts(&{
                                let current_ident_ts = gen_ident_ts(postgresql_type, &NotNullOrNullable::Nullable, &PostgresqlTypePattern::Standart);
                ...
                &if matches!(&is_not_null_standart_can_be_primary_key, IsNotNullStandartCanBePrimaryKey::True) {
                    ...
                    quote! {#import_path_non_primary_key_postgresql_type_read_only_ids_ts}
                },
        ...
        let impl_postgresql_type_test_cases_for_ident_ts = {
            ...
            let gen_read_or_read_inner_into_update_with_new_or_try_new_unwraped_ts = |read_or_update: &postgresql_crud_macros_common::ReadOrUpdate| {
                ...
                quote! {<#SelfUcc::#PostgresqlTypeUcc
                    as
                #import_path::#PostgresqlTypeUcc>::#read_or_update_ucc:: #content_ts}
            };
            let gen_standart_not_null_test_case_handle_ts = |is_need_to_use_into: &IsNeedToUseInto| {
                ...
                let (
                    ...
                    |gen_ident_standart_not_null_function_ts(
                        &gen_ident_standart_not_null_ts(&PostgresqlType::SqlxTypesChronoNaiveTimeAsTime),
                        &content_ts_fd88ca39
                    );
                    (
                ...
                let (
                    ...
                    |gen_ident_standart_not_null_function_ts(
                        &gen_ident_standart_not_null_ts(&PostgresqlType::SqlxTypesChronoNaiveDateAsDate),
                        &content_ts_7c66f815
                    );
                    (
                ...
                match &postgresql_type {
                    PostgresqlType::StdPrimitiveI16AsInt2 => gen_typical_test_cases_vec_ts(&quote! {std_primitive_i16_test_cases_vec}),
                    PostgresqlType::StdPrimitiveI32AsInt4 => gen_typical_test_cases_vec_ts(&quote! {std_primitive_i32_test_cases_vec}),
                    PostgresqlType::StdPrimitiveI64AsInt8 => gen_typical_test_cases_vec_ts(&quote! {std_primitive_i64_test_cases_vec}),
                    PostgresqlType::StdPrimitiveF32AsFloat4 => gen_typical_test_cases_vec_ts(&quote! {std_primitive_f32_test_cases_vec}),
                    PostgresqlType::StdPrimitiveF64AsFloat8 => gen_typical_test_cases_vec_ts(&quote! {std_primitive_f64_test_cases_vec}),
                    ...
                    PostgresqlType::SqlxPostgresTypesPgMoneyAsMoney => quote! {
                    ...
                    PostgresqlType::StdPrimitiveBoolAsBool => gen_typical_test_cases_vec_ts(&quote! {std_primitive_bool_test_cases_vec}),
                    PostgresqlType::StdStringStringAsText => gen_typical_test_cases_vec_ts(&quote! {std_string_string_test_cases_vec}),
                    PostgresqlType::StdVecVecStdPrimitiveU8AsBytea => quote! {vec![
                        Vec::new(),
                        (0u8..=255).collect(),
                        vec![0; 1024],
                        vec![0; 1024 * 1024 * 2],
                    ...
                    PostgresqlType::SqlxTypesChronoNaiveDateTimeAsTimestamp => quote! {vec![
                    ...
                    PostgresqlType::SqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTz => quote! {vec![
                    ...
                    PostgresqlType::SqlxTypesUuidUuidAsUuidV4InitializedByPostgresql => quote! {Vec::new()},
                    PostgresqlType::SqlxTypesUuidUuidAsUuidInitializedByClient => quote! {vec![
                        sqlx::types::Uuid::new_v4()
                    ]},
                    PostgresqlType::SqlxTypesIpnetworkIpNetworkAsInet => quote! {vec![
                    ...
                    PostgresqlType::SqlxTypesMacAddressMacAddressAsMacAddr => quote! {vec![
                    ...
                    PostgresqlType::SqlxPostgresTypesPgRangeStdPrimitiveI32AsInt4Range => gen_int_pgrange_read_only_ids_to_two_dimensional_vec_read_inner_ts(&IntRangeType::SqlxPostgresTypesPgRangeStdPrimitiveI32AsInt4Range),
                    PostgresqlType::SqlxPostgresTypesPgRangeStdPrimitiveI64AsInt8Range => gen_int_pgrange_read_only_ids_to_two_dimensional_vec_read_inner_ts(&IntRangeType::SqlxPostgresTypesPgRangeStdPrimitiveI64AsInt8Range),
                    PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateAsDateRange => gen_range_read_only_ids_to_two_dimensional_vec_read_inner_ts(
                        &ident_sqlx_types_chrono_naive_date_min_ts,
                        &ident_sqlx_types_chrono_naive_date_negative_less_typical_ts,
                        &ident_sqlx_types_chrono_naive_date_negative_more_typical_ts,
                        &ident_sqlx_types_chrono_naive_date_near_zero_ts,
                    ...
                    PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateTimeAsTimestampRange => gen_range_read_only_ids_to_two_dimensional_vec_read_inner_ts(
                        &sqlx_types_chrono_naive_date_time_min_ts,
                        &sqlx_types_chrono_naive_date_time_negative_less_typical_ts,
                        &sqlx_types_chrono_naive_date_time_negative_more_typical_ts,
                        &sqlx_types_chrono_naive_date_time_near_zero_ts,
                    ...
                    PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTzRange => gen_range_read_only_ids_to_two_dimensional_vec_read_inner_ts(
                        &sqlx_types_chrono_date_time_sqlx_types_chrono_utc_min_ts,
                        &sqlx_types_chrono_date_time_sqlx_types_chrono_utc_negative_less_typical_ts,
                        &sqlx_types_chrono_date_time_sqlx_types_chrono_utc_negative_more_typical_ts,
                        &sqlx_types_chrono_date_time_sqlx_types_chrono_utc_near_zero_ts,
            ...
            let option_vec_create_ts = {
                ...
                | {
                    let (new_or_try_new_content_ts, maybe_acc_push_none_ts) = match (&current_not_null_or_nullable, postgresql_type_initialization_try_new_try_from_postgresql_type.is_ok()) {
                        ...
                        (NotNullOrNullable::Nullable, true) => (
                            ...
                            quote! {acc_0b59a062.push(#self_as_postgresql_type_ts::Create::try_new(None).expect("31878971-17fc-4526-ab01-42c8332e641f"));},
                        ),
                        (NotNullOrNullable::Nullable, false) => (quote! {new(Some(el_0fd5865b.0.into()))}, quote! {acc_0b59a062.push(#self_as_postgresql_type_ts::Create::new(None));}),
                    };
                    let ident_as_postgresql_type_test_cases_ts = gen_as_postgresql_type_test_cases_ts(&current_ident_ts);
                    quote! {Some({
                        let mut acc_0b59a062 = Vec::new();
                        for el_0fd5865b in #ident_as_postgresql_type_test_cases_ts::#OptionVecCreateSc().unwrap_or(Vec::new()) {
                ...
                match &postgresql_type_pattern {
                    PostgresqlTypePattern::Standart => match &not_null_or_nullable {
                        NotNullOrNullable::NotNull => match &can_be_primary_key {
                            CanBePrimaryKey::False => {
                                ...
                                let new_or_try_new_ts = {
                                    let self_as_postgresql_type_create_ts = quote!{#self_as_postgresql_type_ts::Create};
                                    if postgresql_type_initialization_try_new_try_from_postgresql_type.is_ok() {
                                        ...
                                        quote! {#self_as_postgresql_type_create_ts::#NewSc}
                                    }
                                };
                                quote! {Some(
                        ...
                        NotNullOrNullable::Nullable => gen_some_acc_content_ts(not_null_or_nullable, &gen_ident_ts(postgresql_type, &NotNullOrNullable::NotNull, &PostgresqlTypePattern::Standart), &proc_macro2::TokenStream::new()),
                    },
                    PostgresqlTypePattern::ArrayDimension1 { dimension1_not_null_or_nullable } => gen_some_acc_content_ts(
                        not_null_or_nullable,
                        &gen_ident_ts(
                            postgresql_type,
                            &match &not_null_or_nullable {
            ...
            let read_only_ids_to_two_dimensional_vec_read_inner_ts = {
                let gen_star_or_dot_clone_ts = |content_ts|match &postgresql_type {
                    PostgresqlType::StdPrimitiveI16AsInt2 |
                    PostgresqlType::StdPrimitiveI32AsInt4 |
                    PostgresqlType::StdPrimitiveI64AsInt8 |
                    PostgresqlType::StdPrimitiveF32AsFloat4 |
                    ...
                    PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateAsDateRange |
                    PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoNaiveDateTimeAsTimestampRange |
                    PostgresqlType::SqlxPostgresTypesPgRangeSqlxTypesChronoDateTimeSqlxTypesChronoUtcAsTimestampTzRange => quote!{*#content_ts},
                    PostgresqlType::StdVecVecStdPrimitiveU8AsBytea |
                    PostgresqlType::StdStringStringAsText => quote!{#content_ts.clone()}
                };
                match &postgresql_type_pattern {
                    PostgresqlTypePattern::Standart => match &not_null_or_nullable {
                        NotNullOrNullable::NotNull => {
                        ...
                        NotNullOrNullable::Nullable => quote! {
                            #ident_standart_not_null_as_postgresql_type_test_cases_ts::#ReadOnlyIdsToTwoDimensionalVecReadInnerSc(#ReadOnlyIdsSc)
                            .into_iter()
                        ...
                        NotNullOrNullable::NotNull => match &dimension1_not_null_or_nullable {
                            NotNullOrNullable::NotNull => {
                                ...
                                quote! {
                                    ...
                                    let read_only_ids_to_two_dimensional_vec_read_inner = #ident_standart_not_null_as_postgresql_type_test_cases_ts::#ReadOnlyIdsToTwoDimensionalVecReadInnerSc(#ReadOnlyIdsSc);
                            ...
                            NotNullOrNullable::Nullable => {
                                ...
                                quote! {
                                    ...
                                    let read_only_ids_to_two_dimensional_vec_read_inner = #ident_standart_nullable_as_postgresql_type_test_cases_ts::#ReadOnlyIdsToTwoDimensionalVecReadInnerSc(#ReadOnlyIdsSc);
                                    let option_additional = {
                                        let mut option_additional = None;
            ...
            let read_inner_into_read_with_new_or_try_new_unwraped_ts = gen_read_or_read_inner_into_update_with_new_or_try_new_unwraped_ts(&postgresql_crud_macros_common::ReadOrUpdate::Read);
            let read_inner_into_update_with_new_or_try_new_unwraped_ts = gen_read_or_read_inner_into_update_with_new_or_try_new_unwraped_ts(&postgresql_crud_macros_common::ReadOrUpdate::Update);
            ...
            let read_only_ids_to_option_value_read_default_option_some_vec_one_el_ts = {
                //todo that is not correct for array of gend by postgresql primary keys but maybe just need to remove this variants and thats it?
                let value_initialization_ts = gen_import_path_value_initialization_ts(&{
                    ...
                    quote! {#self_postgresql_type_as_postgresql_type_ts::normalize(#content_ts)}
                });
            ...
            let read_only_ids_merged_with_create_into_option_value_read_ts = {
                let value_initialization_ts = gen_import_path_value_initialization_ts(&quote! {
                    <Self as #import_path::PostgresqlTypeTestCases>::#ReadOnlyIdsMergedWithCreateIntoReadSc(
                        #ReadOnlyIdsSc,
                        #CreateSc
                    )
                });
            ...
            let read_only_ids_merged_with_create_into_where_equal_ts = {
                ...
                quote! {
                    #ident_where_ucc::#EqualUcc(where_filters::PostgresqlTypeWhereEqual {
            ...
            let postgresql_type_option_vec_where_greater_than_test_ts = {
                ...
                let gen_greater_than_test_ts = |greater_than_variant_ts: &postgresql_crud_common_and_macros_common::PostgresqlTypeGreaterThanVariant, create_content_ts: &dyn quote::ToTokens, table_type_declaration_content_ts: &dyn quote::ToTokens| {
                    quote! {
                ...
                let gen_greater_than_test_new_new_ts =
                    |greater_than_variant_ts: &postgresql_crud_common_and_macros_common::PostgresqlTypeGreaterThanVariant, create_ts: &dyn quote::ToTokens, greater_than_ts: &dyn quote::ToTokens| gen_greater_than_test_ts(greater_than_variant_ts, &quote! {new(#create_ts)}, &quote! {new(#greater_than_ts)});
                let gen_greater_than_test_try_new_try_new_ts = |greater_than_variant_ts: &postgresql_crud_common_and_macros_common::PostgresqlTypeGreaterThanVariant, create_ts: &dyn quote::ToTokens, greater_than_ts: &dyn quote::ToTokens| {
                ...
                match &postgresql_type_pattern {
                    PostgresqlTypePattern::Standart => match &not_null_or_nullable {
                        NotNullOrNullable::NotNull => {
                            ...
                            match &postgresql_type {
                                PostgresqlType::StdPrimitiveI16AsInt2 => wrap_into_not_empty_unique_vec_ts(&gen_greater_than_test_new_new_vec_ts(
                                ...
                                PostgresqlType::StdPrimitiveI32AsInt4 => wrap_into_not_empty_unique_vec_ts(&gen_greater_than_test_new_new_vec_ts(
                                ...
                                PostgresqlType::StdPrimitiveI64AsInt8 => wrap_into_not_empty_unique_vec_ts(&gen_greater_than_test_new_new_vec_ts(
                                ...
                                PostgresqlType::StdPrimitiveF32AsFloat4 => wrap_into_not_empty_unique_vec_ts(&gen_greater_than_test_new_new_vec_ts(
                                ...
                                PostgresqlType::StdPrimitiveF64AsFloat8 => wrap_into_not_empty_unique_vec_ts(&gen_greater_than_test_new_new_vec_ts(
                                ...
                                PostgresqlType::SqlxTypesChronoNaiveTimeAsTime => wrap_into_not_empty_unique_vec_ts(&gen_greater_than_test_try_new_try_new_vec_ts(
                                ...
                                PostgresqlType::SqlxTypesTimeTimeAsTime => wrap_into_not_empty_unique_vec_ts(&gen_greater_than_test_try_new_try_new_vec_ts(
                                    &quote!{Self::min_inner_type()},
                                    &quote!{Self::slightly_more_than_min_inner_type()},
                                    &quote!{Self::middle_inner_type()},
                                    &quote!{Self::slightly_more_than_middle_inner_type()},
                                ...
                                PostgresqlType::SqlxTypesChronoNaiveDateAsDate => wrap_into_not_empty_unique_vec_ts(&gen_greater_than_test_try_new_try_new_vec_ts(
                                ...
                                PostgresqlType::SqlxTypesChronoNaiveDateTimeAsTimestamp => wrap_into_not_empty_unique_vec_ts(&gen_greater_than_test_try_new_try_new_vec_ts(
                                ...
                                PostgresqlType::StdPrimitiveI16AsSmallSerialInitializedByPostgresql |//todo diffrent test logic for autogend?
                                PostgresqlType::StdPrimitiveI32AsSerialInitializedByPostgresql |//todo diffrent test logic for autogend?
                                PostgresqlType::StdPrimitiveI64AsBigSerialInitializedByPostgresql |//todo diffrent test logic for autogend?
                                PostgresqlType::SqlxPostgresTypesPgMoneyAsMoney |
                                PostgresqlType::StdPrimitiveBoolAsBool |
                                PostgresqlType::StdStringStringAsText |
                                PostgresqlType::StdVecVecStdPrimitiveU8AsBytea |
                        ...
                        NotNullOrNullable::Nullable => quote! {
                            <#ident_standart_not_null_ucc as #import_path::PostgresqlTypeTestCases>::postgresql_type_option_vec_where_greater_than_test().map(
                                |el_e4af7fd9|
                ...
                PostgresqlTypePattern::Standart => {
                    ...
                    let gen_some_ts = |value_476d047b: &CreateReadOnlyIds| match &not_null_or_nullable {
                        ...
                        NotNullOrNullable::Nullable => {
                            ...
                            quote! {
                                #content_ts.map(|el_886032ca| #ident_where_ucc::GreaterThan(where_filters::PostgresqlTypeWhereGreaterThan {
                                    logical_operator: greater_than_variant.logical_operator(),
                    ...
                    match &is_need_to_impl_greater_than_test {
                        IsNeedToImplPostgresqlTypeGreaterThanTest::TrueFromReadOnlyIds => gen_some_ts(&CreateReadOnlyIds::ReadOnlyIds),
                        IsNeedToImplPostgresqlTypeGreaterThanTest::TrueFromCreate => gen_some_ts(&CreateReadOnlyIds::Create),
        ...
        let maybe_impl_postgresql_type_primary_key_for_ident_standart_not_null_if_can_be_primary_key_ts = if matches!(&is_not_null_standart_can_be_primary_key, IsNotNullStandartCanBePrimaryKey::True) {
            let postgresql_type_primary_key_ucc = naming::PostgresqlTypePrimaryKeyUcc;
            let value_as_read_only_ids_ts = quote! {#ValueSc: #self_as_postgresql_type_ts::#ReadOnlyIdsUcc};
            quote! {
                #allow_clippy_arbitrary_source_item_ordering_ts
                impl #import_path::#postgresql_type_primary_key_ucc for #ident_standart_not_null_ucc {
                    type #PostgresqlTypeUcc = Self;
                    type #TableTypeDeclarationUcc = #ident_standart_not_null_table_type_declaration_ucc;
                    fn #ReadOnlyIdsIntoTableTypeDeclarationSc(#value_as_read_only_ids_ts) -> #self_as_postgresql_type_ts::#TableTypeDeclarationUcc {
                        #ident_table_type_declaration_ucc(#ValueSc.0.0)
                    }
                    fn #ReadOnlyIdsIntoReadSc(#value_as_read_only_ids_ts) -> #self_as_postgresql_type_ts::#ReadUcc {
                        #ValueSc.0
                    }
                    fn #ReadOnlyIdsIntoUpdateSc(#value_as_read_only_ids_ts) -> #self_as_postgresql_type_ts::#UpdateUcc {
                        #ident_update_ucc(#ValueSc.0.0)
                    }
                    fn #ReadIntoTableTypeDeclarationSc(
                        #ValueSc: #self_as_postgresql_type_ts::#ReadUcc
                    ) -> #self_as_postgresql_type_ts::#TableTypeDeclarationUcc {
        ...
        let maybe_impl_postgresql_type_not_primary_key_for_ident_ts = if matches!(&is_not_null_standart_can_be_primary_key, IsNotNullStandartCanBePrimaryKey::True) {
            ...
            postgresql_crud_macros_common::gen_impl_postgresql_type_not_primary_key_for_ident_ts(&import_path, &ident)
        };
        let gend = quote! {
            #ident_ts
            #ident_origin_ts
    ...
    macros_helpers::maybe_write_ts_into_file(
        gen_postgresql_json_types_config
            .postgresql_table_columns_content_write_into_postgresql_table_columns_using_postgresql_types,
    ...
    let gend = {
        let gen_postgresql_types_mod_sc = GenPostgresqlTypesModSc;
        ...
        quote! {
            #[allow(unused_qualifications)]
            #[allow(clippy::absolute_paths)]
            mod #gen_postgresql_types_mod_sc {
                #(#content_ts)*
            }
            pub use #gen_postgresql_types_mod_sc::*;
        }
    };
    macros_helpers::maybe_write_ts_into_file(
        gen_postgresql_json_types_config.whole_content_write_into_gen_postgresql_types,
        "gen_postgresql_types",
        &gend,
        &macros_helpers::FormatWithCargofmt::True,
    );
    gend
