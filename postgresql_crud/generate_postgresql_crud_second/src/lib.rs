...
pub fn generate_postgresql_crud_second(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    //     generate_postgresql_query_part_from_vec_error_named_upper_camel_case_stringified: std::string::String,
    //     generate_postgresql_query_part_from_vec_error_named_snake_case_stringified: std::string::String,
    // }
    // #[derive(Debug, Clone)]
...
// }
            ...
            for element in &fields_named.named {
                ...
                let mut is_primary_key = false;
                {
                    for attr in &element.attrs {
                        if attr.path().segments.len() == 1 {
                            let first_segment_ident = &attr.path().segments.first().expect("no first value in punctuated").ident;
                            let generate_postgresql_crud_second_primary_key_snake_case_stringified = naming_conventions::GeneratePostgresqlCrudSecondPrimaryKeySnakeCase.to_string();
    ...
    let primary_key_field_type_to_create_upper_camel_case = naming_conventions::self_constants::SelfToCreateUpperCamelCase::from_syn_type_path_last_segment(&primary_key_field.syn_field.ty);
    let primary_key_field_type_to_read_upper_camel_case = naming_conventions::self_constants::SelfToReadUpperCamelCase::from_syn_type_path_last_segment(&primary_key_field.syn_field.ty);
    let primary_key_field_type_to_update_upper_camel_case = naming_conventions::self_constants::SelfToUpdateUpperCamelCase::from_syn_type_path_last_segment(&primary_key_field.syn_field.ty);
    let primary_key_field_type_to_delete_upper_camel_case = naming_conventions::self_constants::SelfToDeleteUpperCamelCase::from_syn_type_path_last_segment(&primary_key_field.syn_field.ty);
    ...
    let primary_key_field_ident_upper_camel_case_token_stream = naming_conventions::ToTokensToUpperCamelCaseTokenStream::new_or_panic(&primary_key_field_ident);
    ...
    let primary_key_field_type_to_update_token_stream = &naming_conventions::self_constants::SelfToUpdateUpperCamelCase::from_syn_type_path_last_segment(&primary_key_field.syn_field.ty);
    ...
    let impl_ident_table_name_token_stream = {
        let table_name_snake_case = naming_conventions::TableNameSnakeCase;
    ...
    let ident_options_upper_camel_case = naming_conventions::self_constants::SelfOptionsUpperCamelCase::from_dyn_quote_to_tokens(&ident);
    ...
    let ident_options_token_stream = {
        ...
        let field_option_primary_key_token_stream = {
            let postgresql_crud_value_declaration_token_stream = generate_postgresql_crud_value_declaration_token_stream(
                ...
                &naming_conventions::self_constants::SelfToReadUpperCamelCase::from_syn_type_path_last_segment(&primary_key_field.syn_field.ty)
            );
            quote::quote! {
        ...
        let fields_options_excluding_primary_key_token_stream = fields_without_primary_key.iter().map(|element| {
            ...
            let postgresql_crud_value_declaration_token_stream = generate_postgresql_crud_value_declaration_token_stream(
                ...
                &naming_conventions::self_constants::SelfToReadUpperCamelCase::from_syn_type_path_last_segment(&element.syn_field.ty)
            );
            quote::quote! {
                #field_attribute_serde_skip_serializing_if_option_is_none_token_stream
    ...
    let postgresql_crud_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_call_token_stream = quote::quote!{
        postgresql_crud::#std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_upper_camel_case::#std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_snake_case()
    };
    ...
    let ident_column_upper_camel_case = naming_conventions::self_constants::SelfColumnUpperCamelCase::from_dyn_quote_to_tokens(&ident);
    let column_token_stream = {
        let ident_column_token_stream = {
            let variants = fields.iter().map(|element| {
                ...
                let type_path_column_upper_camel_case = naming_conventions::self_constants::SelfColumnUpperCamelCase::from_syn_type_path_last_segment(&element.syn_field.ty);
                quote::quote! {
            ...
            let elements_token_stream = fields.iter().map(|element| {
                ...
                quote::quote! {
                    #ident_column_upper_camel_case::#field_ident_upper_camel_case_token_stream(
                        #postgresql_crud_all_enum_variants_array_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_all_enum_variants_array_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_call_token_stream
                    )
                }
            });
            quote::quote! {
        ...
        let fields_token_stream = if let syn::Fields::Named(value) = &syn_variant_wrapper.variant.fields {
            value.named.iter().enumerate().map(|(index, element)| {
                ...
                if &field_ident.as_ref().unwrap_or_else(|| panic!("{}", naming_conventions::FIELD_IDENT_IS_NONE)).to_string() == &naming_conventions::CodeOccurenceSnakeCase.to_string() {
                    ...
                    let error_increment_snake_case = naming_conventions::self_constants::ErrorSelfSnakeCase::from_dyn_std_fmt_display(&index);
    ...
    let generate_operation_error_initialization_eprintln_response_creation_token_stream = |operation: &Operation, syn_variant_wrapper: &SynVariantWrapper, file: &'static str, line: std::primitive::u32, column: std::primitive::u32| {
        let try_operation_route_logic_error_named_upper_camel_case = naming_conventions::self_constants::TrySelfRouteLogicErrorNamedUpperCamelCase::from_dyn_std_fmt_display(operation);
        let try_operation_route_logic_response_variants_upper_camel_case = naming_conventions::self_constants::TrySelfRouteLogicResponseVariantsUpperCamelCase::from_dyn_std_fmt_display(operation);
    ...
    | {
        let variants_token_stream = fields.iter().map(|element| {
            ...
            //                 macros_helpers::generate_simple_syn_punctuated_punctuated::generate_simple_syn_punctuated_punctuated(&[&value.generate_postgresql_query_part_from_vec_error_named_upper_camel_case_stringified]),
            //             )],
            //         );
    ...
    impl Operation {
        ...
        fn std_vec_vec_self_payload_element_token_stream(&self) -> proc_macro2::TokenStream {
            let operation_payload_element_upper_camel_case = naming_conventions::self_constants::SelfPayloadElementUpperCamelCase::from_dyn_std_fmt_display(self);
    ...
    let generate_options_try_from_sqlx_row_token_stream = |operation: &Operation| {
        ...
        let assignment_variants_excluding_primary_key_token_stream = fields_without_primary_key.iter().map(|element| {
            ...
            let element_field_type_to_read_upper_camel_case_token_stream = &naming_conventions::self_constants::SelfToReadUpperCamelCase::from_syn_type_path_last_segment(&element.syn_field.ty);
            quote::quote! {
    ...
    let ident_column_read_permission_token_stream = {
        ...
        let ident_column_read_permission_upper_camel_case = naming_conventions::self_constants::SelfColumnReadPermissionUpperCamelCase::from_dyn_std_fmt_display(&ident);
                                    ...
                                    segments: {
                                        ...
                                        handle.push_value(syn::PathSegment {
                                            ident: proc_macro2::Ident::new(
                                                &naming_conventions::self_constants::SelfToUpdateUpperCamelCase::from_dyn_quote_to_tokens(&last_path_segment.ident).to_string(),
                            ...
                            handle.push(syn::GenericArgument::Type(syn::Type::Path(syn::TypePath {
                                ...
                                path: syn::Path {
                                    ...
                                    segments: {
                                        ...
                                        handle.push_value(syn::PathSegment {
                                            ident: proc_macro2::Ident::new(
                                                &naming_conventions::self_constants::SelfToDeleteUpperCamelCase::from_dyn_quote_to_tokens(&last_path_segment.ident).to_string(),
                                                proc_macro2::Span::call_site()
                                            ),
                                            arguments: syn::PathArguments::None,
                                        });
    ...
    let limit_and_offset_type_token_stream = quote::quote!{postgresql_crud::postgresql_types::base_wrap::StdPrimitiveI64AsPostgresqlBigSerialNotNull};
    // {
    ...
    //         .unwrap_or_else(|_| panic!("{value} {}", constants::PARSE_PROC_MACRO2_TOKEN_STREAM_FAILED_MESSAGE))
    // };
    let std_vec_vec_primary_key_field_type_to_create_token_stream = quote::quote! {std::vec::Vec::<#primary_key_field_type_to_create_upper_camel_case>};
    ...
    // let json_ident_column_upper_camel_case_stringified = naming_conventions::self_constants::SelfColumnUpperCamelCase::from_dyn_quote_to_tokens(&ident).to_string();// format!("{ident}{}", naming_conventions::ColumnUpperCamelCase);
    ...
    let not_unique_primary_key_syn_variant_wrapper = new_syn_variant_wrapper(
        ...
        Some(macros_helpers::status_code::StatusCode::BadRequest400),
        vec![(macros_helpers::error_occurence::ErrorOccurenceFieldAttribute::EoToStdStringString, &naming_conventions::NotUniquePrimaryKeySnakeCase, {
            // if let syn::Type::Path(value) = &primary_key_field.syn_field.ty {
            //     if let Some(last_path_segment) = value.path.segments.last() {
            //         let mut handle = syn::punctuated::Punctuated::<syn::PathSegment, syn::token::PathSep>::new();
            //         for element in value.path.segments.iter().rev().skip(1).rev() {
    ...
    let no_payload_fields_primary_key_syn_variant_wrapper = new_syn_variant_wrapper(
        ...
        Some(macros_helpers::status_code::StatusCode::InternalServerError500),
        vec![(macros_helpers::error_occurence::ErrorOccurenceFieldAttribute::EoToStdStringString, &naming_conventions::NoPayloadFieldsPrimaryKeySnakeCase, {
            if let syn::Type::Path(value) = &primary_key_field.syn_field.ty {
                if let Some(last_path_segment) = value.path.segments.last() {
                    let mut handle = syn::punctuated::Punctuated::<syn::PathSegment, syn::token::PathSep>::new();
                    for element in value.path.segments.iter().rev().skip(1).rev() {
    ...
    let pub_fields_idents_std_option_option_std_vec_vec_where_inner_type_token_stream = generate_fields_named_excluding_primary_key_token_stream(&|element: &SynFieldWrapper| -> proc_macro2::TokenStream {
        ...
        let field_type_where_upper_camel_case = &naming_conventions::self_constants::SelfWhereUpperCamelCase::from_syn_type_path_last_segment(&element.syn_field.ty);
        quote::quote! {
            pub #field_ident: std::option::Option<std::vec::Vec<#field_type_where_upper_camel_case>>
        }
    });
    ...
    let generate_filter_not_unique_column_route_logic_token_stream = |operation: &Operation| {
        ...
        let generics_generate_postgresql_query_part_from_vec_error_named_syn_variants_wrappers_token_stream = {
        ...
        quote::quote! {
            ...
            for #element_snake_case in &#value_snake_case.#select_snake_case {
                if #acc_snake_case.contains(&#element_snake_case) {
                    ...
                    #generics_generate_postgresql_query_part_from_vec_error_named_syn_variants_wrappers_token_stream
    ...
    let generate_filter_not_unique_column_http_request_token_stream = |operation: &Operation| {
        let try_operation_error_named_upper_camel_case = naming_conventions::self_constants::TrySelfErrorNamedUpperCamelCase::from_dyn_std_fmt_display(operation);
    ...
    let update_fields_token_stream = {
        let fields_named_excluding_primary_key_token_stream = generate_fields_named_excluding_primary_key_token_stream(&|element: &SynFieldWrapper| {
            ...
            let path_value_token_stream = {
                let value = format!("{}::{}", naming_conventions::PostgresqlCrudSnakeCase, naming_conventions::ValueUpperCamelCase);
                value.parse::<proc_macro2::TokenStream>()
                .unwrap_or_else(|_| panic!("{value} {}", constants::PARSE_PROC_MACRO2_TOKEN_STREAM_FAILED_MESSAGE))
            };
            // let inner_type_token_stream = &element.inner_type_with_generic_token_stream;
            let field_ident_to_update_upper_camel_case = &naming_conventions::self_constants::SelfToUpdateUpperCamelCase::from_syn_type_path_last_segment(&element.syn_field.ty);
            quote::quote! {
    ...
    let generate_filter_no_payload_fields_token_stream = |operation: &Operation, source_token_stream: &dyn quote::ToTokens| {
        ...
        let none_fields_named_excluding_primary_key_token_stream = fields_without_primary_key.iter().map(|_| naming_conventions::NoneUpperCamelCase);
    ...
    let not_unique_fields_syn_variants_wrappers = fields_without_primary_key.iter().map(|element| {
        new_syn_variant_wrapper(
            &naming_conventions::self_constants::NotUniqueSelfUpperCamelCase::from_dyn_quote_to_tokens(&element.field_ident),
            ...
            vec![(
                ...
                &element.field_ident.to_string(),
                {
                    ...
                    //         // &naming_conventions::self_constants::SelfToReadUpperCamelCase::from_syn_type_path_last_segment(&element.syn_field.ty).to_string()
                    //         "todo"
                    //         ,
                    //         proc_macro2::Span::call_site(),
                    //     ),
    ...
    let generate_error_occurence_variant_token_stream = |error_variant: &syn::Variant| -> proc_macro2::TokenStream {
        ...
        let fields_mapped_into_token_stream = fields_named.named.iter().map(|field| {
            let field_ident = field.ident.as_ref().unwrap_or_else(|| panic!("{}", naming_conventions::FIELD_IDENT_IS_NONE));
    ...
    | -> proc_macro2::TokenStream {
        ...
        let impl_std_convert_from_try_operation_route_logic_error_named_for_try_operation_route_logic_response_variants_token_stream = {
            let variants_token_stream = type_variants_from_request_response_syn_variants.iter().map(|element| {
                ...
                let try_operation_route_logic_error_named_with_serialize_deserialize_upper_camel_case = naming_conventions::self_constants::TrySelfRouteLogicErrorNamedWithSerializeDeserializeUpperCamelCase::from_dyn_std_fmt_display(operation);
                quote::quote! {
    ...
    let generate_parameters_pattern_token_stream = |operation: &Operation, payload_token_stream: proc_macro2::TokenStream| -> proc_macro2::TokenStream {
        let parameters_token_stream = {
            let operation_parameters_upper_camel_case = naming_conventions::self_constants::SelfParametersUpperCamelCase::from_dyn_std_fmt_display(operation);
            let operation_payload_upper_camel_case = naming_conventions::self_constants::SelfPayloadUpperCamelCase::from_dyn_std_fmt_display(operation);
            quote::quote! {
                #derive_debug
    ...
    let generate_payload_and_payload_element_token_stream = |operation: &Operation, fields_token_stream: &dyn quote::ToTokens| -> proc_macro2::TokenStream {
        let operation_payload_element_token_stream = {
            let operation_payload_element_upper_camel_case = naming_conventions::self_constants::SelfPayloadElementUpperCamelCase::from_dyn_std_fmt_display(operation);
            quote::quote! {
                #derive_debug_serde_serialize_serde_deserialize_utoipa_to_schema
        ...
        let syn_variants = {
            ...
            value.push({
                ...
                new_syn_variant_wrapper(
                    ...
                    vec![(
                        macros_helpers::error_occurence::ErrorOccurenceFieldAttribute::EoToStdStringString,
                        &naming_conventions::self_constants::TrySelfRouteLogicErrorNamedWithSerializeDeserializeSnakeCase::from_dyn_std_fmt_display(operation),
    ...
    | -> proc_macro2::TokenStream {
        let try_operation_route_logic_snake_case = naming_conventions::self_constants::TrySelfRouteLogicSnakeCase::from_dyn_std_fmt_display(operation);
    ...
    let pub_field_ident_field_type_fields_named_excluding_primary_key_token_stream = generate_fields_named_excluding_primary_key_token_stream(&|element: &SynFieldWrapper| {
        ...
        let field_type_to_create = naming_conventions::self_constants::SelfToCreateUpperCamelCase::from_syn_type_path_last_segment(&element.syn_field.ty);
        // let inner_type_token_stream = &element.inner_type_with_generic_token_stream;
        // let field_type_token_stream = match &element.option_generic {
        //     Some(value) => {
        //         let value_token_stream = naming_conventions::self_constants::SelfToCreateUpperCamelCase::from_dyn_quote_to_tokens(&{
    ...
    | -> proc_macro2::TokenStream {
        let try_operation_snake_case = naming_conventions::self_constants::TrySelfSnakeCase::from_dyn_std_fmt_display(operation);
        let try_operation_error_named_upper_camel_case = naming_conventions::self_constants::TrySelfErrorNamedUpperCamelCase::from_dyn_std_fmt_display(operation);
        let operation_parameters_upper_camel_case = naming_conventions::self_constants::SelfParametersUpperCamelCase::from_dyn_std_fmt_display(operation);
        ...
        let future_token_stream = {
            let operation_http_method_snake_case_token_stream = naming_conventions::AsRefStrToSnakeCaseTokenStream::new_or_panic(&operation.http_method());
        ...
        let try_operation_route_logic_error_named_with_serialize_deserialize_upper_camel_case = naming_conventions::self_constants::TrySelfRouteLogicErrorNamedWithSerializeDeserializeUpperCamelCase::from_dyn_std_fmt_display(operation);
        let try_operation_route_logic_error_named_with_serialize_deserialize_snake_case = naming_conventions::self_constants::TrySelfRouteLogicErrorNamedWithSerializeDeserializeSnakeCase::from_dyn_std_fmt_display(operation);
    ...
    let generate_create_update_delete_many_fetch_token_stream = |operation: &Operation| {
        generate_fetch_token_stream(
            &generate_sqlx_row_try_get_primary_key_token_stream(
                &match operation {
                    Operation::CreateMany => quote::quote!{#primary_key_field_type_to_create_upper_camel_case},
    ...
    let generate_create_update_delete_one_fetch_token_stream = |operation: &Operation| {
        generate_fetch_one_token_stream(
            &generate_sqlx_row_try_get_primary_key_token_stream(
                &match operation {
                    Operation::CreateOne => quote::quote!{#primary_key_field_type_to_create_upper_camel_case},
    ...
    let generate_operation_payload_example_route_logic_token_stream = |operation: &Operation| {
        let operation_payload_example_route_logic_snake_case = naming_conventions::self_constants::SelfPayloadExampleRouteLogicSnakeCase::from_dyn_std_fmt_display(operation);
        let wraped_into_axum_response_token_stream = wrap_into_axum_response_token_stream(
            &{
                let operation_payload_upper_camel_case = naming_conventions::self_constants::SelfPayloadUpperCamelCase::from_dyn_std_fmt_display(operation);
    ...
    | {
        quote::quote! {
            impl postgresql_crud::#std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_upper_camel_case for #for_token_stream {
            ...
            let try_operation_route_logic_token_stream = {
                ...
                let query_string_token_stream = {
                    ...
                    let column_increments_token_stream = fields_without_primary_key.iter().map(|element| {
                        ...
                        let bind_query_syn_variant_error_initialization_eprintln_response_creation_token_stream = generate_operation_error_initialization_eprintln_response_creation_token_stream(&operation, &bind_query_syn_variant_wrapper, file!(), line!(), column!());
                        quote::quote! {
                            match postgresql_crud::BindQuerySecond::try_generate_bind_increments(&#element_snake_case.#element_field_ident, &mut #increment_snake_case) {
                                Ok(#value_snake_case) => {
                                    #acc_snake_case.push_str(&format!("{value},"));
                                },
                                Err(#error_0_token_stream) => {
                ...
                let binded_query_token_stream = {
                    ...
                    let query_bind_token_stream = fields_without_primary_key.iter().map(|element| {
                        let field_ident = &element.field_ident;
                        quote::quote! {
                            #query_snake_case = #postgresql_crud_snake_case::BindQuerySecond::bind_value_to_query(#element_snake_case.#field_ident, #query_snake_case);
                        }
                    });
                    quote::quote! {
        ...
        let impl_postgresql_crud_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_for_operation_payload_token_stream = {
            let impl_postgresql_crud_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_for_operation_payload_element_token_stream =
                wrap_into_impl_postgresql_crud_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_for_value_token_stream(
                    &naming_conventions::self_constants::SelfPayloadElementUpperCamelCase::from_dyn_std_fmt_display(&operation),
            ...
            let impl_postgresql_crud_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_for_operation_payload_token_stream =
                wrap_into_impl_postgresql_crud_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_for_value_token_stream(
                    &naming_conventions::self_constants::SelfPayloadUpperCamelCase::from_dyn_std_fmt_display(&operation),
                    &quote::quote! {(vec![
            ...
            let try_operation_route_logic_token_stream = {
                ...
                let query_string_token_stream = {
                    ...
                    let try_generate_bind_increments = fields_without_primary_key.iter().map(|element| {
                        ...
                        let bind_query_syn_variant_error_initialization_eprintln_response_creation_token_stream = generate_operation_error_initialization_eprintln_response_creation_token_stream(&operation, &bind_query_syn_variant_wrapper, file!(), line!(), column!());
                        quote::quote! {
                            match postgresql_crud::BindQuerySecond::try_generate_bind_increments(&#parameters_snake_case.#payload_snake_case.#element_field_ident, &mut #increment_snake_case) {
                                Ok(#value_snake_case) => #value_snake_case,
                                Err(#error_0_token_stream) => {
                                    #bind_query_syn_variant_error_initialization_eprintln_response_creation_token_stream
                                }
                    ...
                    let filter_not_unique_fields_token_stream = {
                        ...
                        let filter_not_unique_fields_named_excluding_primary_key_token_stream = fields_without_primary_key.iter().map(|element| {
                            ...
                            let not_unique_fields_syn_variant_initialization_token_stream = {
                                let not_unique_field_ident_upper_camel_case = naming_conventions::self_constants::NotUniqueSelfUpperCamelCase::from_dyn_quote_to_tokens(&element_field_ident);
                ...
                quote::quote!{
                    {
                        ...
                        #order_by_snake_case: postgresql_crud::OrderBy {
                            #column_snake_case: #ident_column_upper_camel_case::#primary_key_field_ident_upper_camel_case_token_stream(vec![]),
    ...
    let (read_one_token_stream, read_one_http_request_test_expect_success_token_stream, read_one_http_request_test_expect_fail_token_stream) = {
        ...
        let impl_postgresql_crud_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_for_operation_payload_token_stream = wrap_into_impl_postgresql_crud_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_for_value_token_stream(
            ...
            &quote::quote! {
                {
                    #primary_key_field_ident: postgresql_crud::#std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_upper_camel_case::#std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_snake_case(),
                    #select_snake_case: #postgresql_crud_all_enum_variants_array_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_all_enum_variants_array_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_call_token_stream
                }
            },
        );
            ...
            let try_operation_route_logic_token_stream = {
                ...
                let query_string_token_stream = {
                    ...
                    let fields_named_excluding_primary_key_update_assignment_token_stream = fields_without_primary_key.iter().map(|element| {
                        let field_ident = &element.field_ident;
                        let is_field_ident_update_exists_snake_case = naming_conventions::self_constants::IsSelfUpdateExistSnakeCase::from_dyn_quote_to_tokens(&field_ident);
                    ...
                    let where_primary_key_field_ident_in_primary_keys_returning_primary_key_field_ident_token_stream = {
                        let where_primary_key_field_ident_in_primary_keys_double_quotes_token_stream = generate_quotes::double_quotes_token_stream(
                            &format!(" {where_snake_case} {primary_key_field_ident} {} ({{}}) {returning_snake_case} {primary_key_field_ident};", naming_conventions::InSnakeCase)
                        );
                        quote::quote! {
                            #query_snake_case.push_str(&format!(
                ...
                wrap_into_impl_postgresql_crud_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_for_value_token_stream(
                    ...
                    &{
                        let primary_key_field_with_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_token_stream = {
                            quote::quote! {
                                #primary_key_field_ident: #postgresql_crud_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_call_token_stream
                            }
                        };
                        let fields_without_primary_key_with_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_token_stream = fields_without_primary_key.iter().map(|element| {
                            ...
                            quote::quote! {
                                #field_ident: Some(postgresql_crud::Value{
                                    #value_snake_case: #postgresql_crud_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_call_token_stream
                                })
                            }
                        });
                        quote::quote! {{
            ...
            let try_operation_route_logic_token_stream = {
                ...
                let binded_query_token_stream = {
                    let binded_query_modifications_token_stream = fields_without_primary_key.iter().map(|element| {
                        ...
                        quote::quote! {
                            if let Some(#value_snake_case) = #parameters_snake_case.#payload_snake_case.#field_ident {
                                #query_snake_case = #postgresql_crud_snake_case::BindQuerySecond::bind_value_to_query(
                                    #value_snake_case.#value_snake_case,
                                    #query_snake_case,
                                );
                            }
        ...
        let parameters_token_stream = generate_parameters_pattern_token_stream(
            ...
            generate_operation_payload_token_stream(
                ...
                &{
                    let primary_key_field_ident_std_option_option_std_vec_vec_primary_key_inner_type_handle_token_stream = {
                        // let primary_key_inner_type_handle_token_stream = generate_primary_key_inner_type_handle_token_stream(true);
                        quote::quote! {pub #primary_key_field_ident: std::option::Option<std::vec::Vec<#primary_key_field_type_to_delete_upper_camel_case>>}
                    };
                    quote::quote! {
                        #primary_key_field_ident_std_option_option_std_vec_vec_primary_key_inner_type_handle_token_stream,
                        #pub_fields_idents_std_option_option_std_vec_vec_where_inner_type_token_stream
    ...
    let (delete_one_token_stream, delete_one_test_token_stream) = {
        ...
        let parameters_token_stream = generate_parameters_pattern_token_stream(&operation, generate_operation_payload_token_stream(
            ...
            &generate_pub_handle_primary_key_field_ident_primary_key_inner_type_handle_token_stream(
                &naming_conventions::self_constants::SelfToDeleteUpperCamelCase::from_syn_type_path_last_segment(&primary_key_field.syn_field.ty)
            )
        ));
        // println!("{parameters_token_stream}");
        let try_operation_route_logic_token_stream = {
...
// #[derive(Debug, Clone, Copy, naming_conventions::AsRefStrEnumWithUnitFieldsToSnakeCaseStringified)]
...
//             naming_conventions::AsRefStrToSnakeCaseStringified::new(test_operation_print_in_info),
//             slashes,
//         )
//     }
// }
