...
// }
    ...
    let primary_key_field_type_to_update_upper_camel_case = naming_conventions::SelfToUpdateUpperCamelCase::from_syn_type_path_last_segment(&primary_key_field.syn_field.ty);
    let primary_key_field_type_to_delete_upper_camel_case = naming_conventions::SelfToDeleteUpperCamelCase::from_syn_type_path_last_segment(&primary_key_field.syn_field.ty);
    ...
    let primary_key_field_type_to_update_token_stream = &naming_conventions::SelfToUpdateUpperCamelCase::from_syn_type_path_last_segment(&primary_key_field.syn_field.ty);
    ...
    let postgresql_crud_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_call_token_stream = quote::quote!{
        postgresql_crud::#std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_upper_camel_case::#default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_snake_case()
    };
    ...
    let generate_operation_error_initialization_eprintln_response_creation_token_stream = |operation: &Operation, syn_variant_wrapper: &SynVariantWrapper, file: &'static str, line: std::primitive::u32, column: std::primitive::u32| {
        let try_operation_route_logic_error_named_upper_camel_case = naming_conventions::TrySelfRouteLogicErrorNamedUpperCamelCase::from_dyn_std_fmt_display(operation);
        let try_operation_route_logic_response_variants_upper_camel_case = naming_conventions::TrySelfRouteLogicResponseVariantsUpperCamelCase::from_dyn_std_fmt_display(operation);
    ...
    //         .unwrap_or_else(|_| panic!("primary key functionality is not implemented for {primary_key_rust_sqlx_map_to_postgres_type_variant} in logic"))
    //         .to_sqlx_postgres_type()
    //         .to_string();
    ...
    //                                         std_vec_vec_generic_type.push_value(syn::PathSegment {
    ...
    let limit_and_offset_type_token_stream = quote::quote!{postgresql_crud::postgresql_types::StdPrimitiveI64};
    // {
    ...
    let json_ident_column_upper_camel_case_stringified = naming_conventions::SelfColumnUpperCamelCase::from_dyn_quote_to_tokens(&ident).to_string();// format!("{ident}{}", naming_conventions::ColumnUpperCamelCase);
    ...
    let not_unique_primary_key_syn_variant_wrapper = new_syn_variant_wrapper(
        ...
        vec![(macros_helpers::error_occurence::ErrorOccurenceFieldAttribute::EoToStdStringString, &naming_conventions::NotUniquePrimaryKeySnakeCase, {
            ...
            //                 &naming_conventions::SelfToUpdateUpperCamelCase::from_dyn_quote_to_tokens(&last_path_segment.ident).to_string(),
    ...
    let f = &primary_key_field.syn_field.ty;
    // println!("{f:#?}");
    ...
    let no_payload_fields_primary_key_syn_variant_wrapper = new_syn_variant_wrapper(
        ...
        vec![(macros_helpers::error_occurence::ErrorOccurenceFieldAttribute::EoToStdStringString, &naming_conventions::NoPayloadFieldsPrimaryKeySnakeCase, {
            ...
            // // value.push_value(syn::PathSegment {
            // //     ident: proc_macro2::Ident::new(
            // //         &postgresql_crud_common::SqlxPostgresType::from_supported_sqlx_postgres_type_removing_option(&postgresql_crud_common::SupportedSqlxPostgresType::from(primary_key_rust_sqlx_map_to_postgres_type_variant)).to_string(),
            // //         proc_macro2::Span::call_site(),
            // //     ),
            // //     arguments: syn::PathArguments::None,
            // // });
    ...
    let pub_fields_idents_std_option_option_std_vec_vec_where_inner_type_token_stream = generate_fields_named_excluding_primary_key_token_stream(&|element: &SynFieldWrapper| -> proc_macro2::TokenStream {
        ...
        let field_type_where_upper_camel_case = &naming_conventions::SelfWhereUpperCamelCase::from_syn_type_path_last_segment(&element.syn_field.ty);
        quote::quote! {
    ...
    let update_fields_token_stream = {
        let fields_named_excluding_primary_key_token_stream = generate_fields_named_excluding_primary_key_token_stream(&|element: &SynFieldWrapper| {
            ...
            let field_ident_to_update_upper_camel_case = &naming_conventions::SelfToUpdateUpperCamelCase::from_syn_type_path_last_segment(&element.syn_field.ty);
            quote::quote! {
    ...
    //     let non_existing_primary_keys_syn_variant_error_initialization_eprintln_response_creation_token_stream = generate_operation_error_initialization_eprintln_response_creation_token_stream(&operation, &non_existing_primary_keys_syn_variant_wrapper, file!(), line!(), column!());
    //     let non_existing_primary_keys_and_rollback_syn_variant_error_initialization_eprintln_response_creation_token_stream =
    //         generate_operation_error_initialization_eprintln_response_creation_token_stream(&operation, &non_existing_primary_keys_and_rollback_syn_variant_wrapper, file!(), line!(), column!());
    //     quote::quote! {
    ...
    let generate_payload_and_payload_element_token_stream = |operation: &Operation, fields_token_stream: &dyn quote::ToTokens| -> proc_macro2::TokenStream {
        let operation_payload_element_token_stream = {
            let operation_payload_element_upper_camel_case = naming_conventions::SelfPayloadElementUpperCamelCase::from_dyn_std_fmt_display(operation);
            quote::quote! {
                #derive_debug_serde_serialize_serde_deserialize_utoipa_to_schema
    ...
    | -> proc_macro2::TokenStream {
        let try_operation_route_logic_snake_case = naming_conventions::TrySelfRouteLogicSnakeCase::from_dyn_std_fmt_display(operation);
    ...
    let pub_field_ident_field_type_fields_named_excluding_primary_key_token_stream = generate_fields_named_excluding_primary_key_token_stream(&|element: &SynFieldWrapper| {
        ...
        let field_type_to_create = naming_conventions::SelfToCreateUpperCamelCase::from_syn_type_path_last_segment(&element.syn_field.ty);
        // let inner_type_token_stream = &element.inner_type_with_generic_token_stream;
    ...
    | -> proc_macro2::TokenStream {
        let try_operation_snake_case = naming_conventions::TrySelfSnakeCase::from_dyn_std_fmt_display(operation);
        let try_operation_error_named_upper_camel_case = naming_conventions::TrySelfErrorNamedUpperCamelCase::from_dyn_std_fmt_display(operation);
        let operation_parameters_upper_camel_case = naming_conventions::SelfParametersUpperCamelCase::from_dyn_std_fmt_display(operation);
        ...
        let try_operation_route_logic_error_named_with_serialize_deserialize_upper_camel_case = naming_conventions::TrySelfRouteLogicErrorNamedWithSerializeDeserializeUpperCamelCase::from_dyn_std_fmt_display(operation);
        let try_operation_route_logic_error_named_with_serialize_deserialize_snake_case = naming_conventions::TrySelfRouteLogicErrorNamedWithSerializeDeserializeSnakeCase::from_dyn_std_fmt_display(operation);
    ...
    let generate_operation_payload_example_route_logic_token_stream = |operation: &Operation| {
        ...
        let wraped_into_axum_response_token_stream = wrap_into_axum_response_token_stream(
            &{
                let operation_payload_upper_camel_case = naming_conventions::SelfPayloadUpperCamelCase::from_dyn_std_fmt_display(operation);
                quote::quote! {<#operation_payload_upper_camel_case as postgresql_crud::#std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_upper_camel_case>::#default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_snake_case()}
            },
    ...
    let (read_one_token_stream, read_one_http_request_test_expect_success_token_stream, read_one_http_request_test_expect_fail_token_stream) = {
        ...
        let impl_postgresql_crud_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_for_operation_payload_token_stream = wrap_into_impl_postgresql_crud_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_for_value_token_stream(
            ...
            &quote::quote! {
                {
                    #primary_key_field_ident: postgresql_crud::#std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_upper_camel_case::#default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_snake_case(),
                    #select_snake_case: #postgresql_crud_all_enum_variants_array_std_default_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_all_enum_variants_array_default_but_std_option_option_is_always_some_and_std_vec_vec_always_contains_one_element_call_token_stream
                }
            },
        );
    ...
    //                         &format!(" {} {primary_key_field_ident} {} ({{}}) {returning_snake_case} {primary_key_field_ident};", naming_conventions::WhereSnakeCase, naming_conventions::InSnakeCase)
    //                     );
    //                     quote::quote! {
    //                         #query_snake_case.push_str(&format!(
    ...
    //             let postgresql_logic_token_stream = wrap_content_into_postgresql_transaction_begin_commit_value_token_stream(&operation, &{
    //                 let fetch_token_stream = generate_create_update_delete_many_fetch_token_stream(&operation);
    //                 let non_existing_primary_keys_check_token_stream = generate_non_existing_primary_keys_check_token_stream(&operation, &expected_primary_keys_snake_case);
    //                 quote::quote! {
    //                     #fetch_token_stream
    //                     {
    ...
    //                     }
    //                 };
    //                 let filter_not_unique_fields_token_stream = generate_filter_not_unique_fields_token_stream(&operation, &syn_field_with_additional_info_fields_named_excluding_primary_key);
    //                 quote::quote! {
    //                     #filter_no_payload_fields_token_stream
    ...
    //     let parameters_token_stream = generate_parameters_pattern_token_stream(&operation, generate_operation_payload_token_stream(&operation, &pub_handle_primary_key_field_ident_primary_key_inner_type_handle_token_stream));
    //     // println!("{parameters_token_stream}");
    //     let try_operation_route_logic_token_stream = {
    //         let try_operation_route_logic_response_variants_impl_std_convert_from_try_operation_route_logic_error_named_for_try_operation_route_logic_response_variants_try_operation_route_logic_error_named_token_stream =
    //             generate_try_operation_route_logic_response_variants_impl_std_convert_from_try_operation_route_logic_error_named_for_try_operation_route_logic_response_variants_try_operation_route_logic_error_named_token_stream(
