...
pub fn generate_postgresql_crud(input: proc_macro::TokenStream) -> proc_macro::TokenStream {//todo in few cases rows affected is usefull. (update delete for example). if 0 afftected -maybe its error? or maybe use select then update\delete?(rewrite query)
    ...
    let primary_key_field_type = &primary_key_syn_field_with_additional_info.field.ty;
    ...
    //     }
    // }
    let primary_key_field_ident = primary_key_syn_field_with_additional_info.field.ident.as_ref()
        .unwrap_or_else(|| {
    ...
    let fields_named_wrappers_excluding_primary_key = fields_named.clone().into_iter().filter(|field|*field != primary_key_syn_field_with_additional_info.field).map(|element|{
    ...
    let struct_options_token_stream = {
        ...
        let field_option_primary_key_token_stream = {
            ...
            let (
                rust_sqlx_map_to_postgres_type_variant,
                maybe_generic_token_stream
            ) = match &primary_key_syn_field_with_additional_info.field.ty {
                syn::Type::Path(value) => match value.path.segments.len() == 2 {
                    true => {
                        if value.path.segments[0].ident != postgresql_crud_common::POSTGRESQL_CRUD_SNAKE_CASE {
                            panic!("{proc_macro_name_upper_camel_case_ident_stringified} field_type is not syn::Type::Path");
        ...
        let fields_options_excluding_primary_key_token_stream = fields_named_wrappers_excluding_primary_key.iter().map(|element| {
            ...
            //todo json generic
            let inner_type_with_serialize_deserialize_token_stream = {
                let inner_type_with_serialize_deserialize_stringified = &element.rust_sqlx_map_to_postgres_type_variant.get_inner_type_with_serialize_deserialize_stringified("");
                inner_type_with_serialize_deserialize_stringified.parse::<proc_macro2::TokenStream>()
                .unwrap_or_else(|_| panic!("{proc_macro_name_upper_camel_case_ident_stringified} {inner_type_with_serialize_deserialize_stringified} {}", proc_macro_common::global_variables::hardcode::PARSE_PROC_MACRO2_TOKEN_STREAM_FAILED_MESSAGE))
            };
            quote::quote!{
                #serde_skip_serializing_if_value_attribute_token_stream
                #field_vis #field_ident: std::option::Option<#inner_type_with_serialize_deserialize_token_stream>//#field_type_path
            }
        });
        quote::quote! {
    ...
    let from_ident_for_ident_options_token_stream = {
        // postgresql_crud::SqlxTypesUuidUuidTryParseWithSerializeDeserialize::from(value.id.0)
        // #primary_key_field
        //todo reusage
        let (
            rust_sqlx_map_to_postgres_type_variant,
    ...
    let column_select_token_stream = {
        ...
        let options_try_from_sqlx_row_token_stream = {
            ...
            let declaration_excluding_primary_key_token_stream = fields_named_wrappers_excluding_primary_key.iter().map(|element|{
                ...
                //
                quote::quote! {
                    let mut #field_ident: std::option::Option<#inner_type_with_serialize_deserialize_token_stream> = None;//#field_type
                }
            });
            let assignment_token_stream = column_variants.iter().map(|column_variant|{
                ...
                //             //
                //             Err(#error_value_snake_case_token_stream) => {
                //                 return Err(#error_value_snake_case_token_stream); //todo custom type
                //             }
                //         };
                //     }
                // };
                let write_ident_token_stream = column_variant.iter().filter_map(|field|match field == &primary_key_syn_field_with_additional_info.field {
                    true => None,
                    false => {
            ...
            let sqlx_decode_decode_and_sqlx_types_type_primary_key_token_stream = {
                ...
                // 
                let inner_type_token_stream = {
                    let inner_type_stringified = &rust_sqlx_map_to_postgres_type_variant.get_original_type_stringified("");//todo generic for json
                    inner_type_stringified.parse::<proc_macro2::TokenStream>()
                    .unwrap_or_else(|_| panic!("{proc_macro_name_upper_camel_case_ident_stringified} {inner_type_stringified} {}", proc_macro_common::global_variables::hardcode::PARSE_PROC_MACRO2_TOKEN_STREAM_FAILED_MESSAGE))
                };
                //
...
fn generate_pub_field_ident_field_type_token_stream(
    ...
    quote::quote!{
        pub #field_ident: #inner_type_token_stream//#field_type
    }
}
