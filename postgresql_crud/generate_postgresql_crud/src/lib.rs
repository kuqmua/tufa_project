...
pub fn generate_postgresql_crud(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let vec_of_primary_keys_returned_from_create_many_snake_case =
        naming::VecOfPrimaryKeysReturnedFromCreateManySnakeCase;
    ...
    let some_value_read_only_ids_returned_from_create_one_snake_case =
        naming::SomeValueReadOnlyIdsReturnedFromCreateOneSnakeCase;
    let sort_vec_of_ident_read_with_primary_key_by_primary_key_snake_case =
        naming::SortVecOfIdentReadWithPrimaryKeyByPrimaryKeySnakeCase; //here
    let read_only_ids_returned_from_create_one_snake_case =
        naming::ReadOnlyIdsReturnedFromCreateOneSnakeCase;
    ...
    let (primary_key_field, fields, fields_without_primary_key) = if let syn::Data::Struct(
        data_struct,
    ) = &syn_derive_input.data
    {
        if let syn::Fields::Named(fields_named) = &data_struct.fields {
            ...
            for element in &fields_named.named {
                ...
                assert!(
                    ...
                    "Postgresql truncates column names to {max_postgresql_column_length} characters, this is more: {field_ident} ({field_ident_len} characters)"
                );
                ...
                let mut is_primary_key = false;
                {
                    for attr in &element.attrs {
                        if attr.path().segments.len() == 1 {
                            let first_segment_ident = &attr
                                .path()
                                .segments
                                .first()
                                .expect("no first value in punctuated")
                                .ident;
                            let generate_postgresql_crud_primary_key_snake_case_stringified =
                                naming::GeneratePostgresqlCrudPrimaryKeySnakeCase.to_string();
                            if first_segment_ident
                                == &generate_postgresql_crud_primary_key_snake_case_stringified
                            {
                                if option_primary_key_field.is_some() {
                                    panic!(
                                        "two or more supported {generate_postgresql_crud_primary_key_snake_case_stringified} attributes!"
                                    );
                                } else {
    ...
    let generate_as_postgresql_type_table_type_declaration_token_stream =
    ...
    let primary_key_field_type_as_postgresql_type_table_type_declaration_token_stream =
        generate_as_postgresql_type_table_type_declaration_token_stream(&primary_key_field_type);
    ...
    let generate_as_postgresql_type_where_element_token_stream =
        |field_type: &dyn quote::ToTokens| {
            generate_as_postgresql_type_tokens_token_stream(
                &field_type,
    ...
    let primary_key_field_type_as_postgresql_type_where_element_token_stream =
        generate_as_postgresql_type_where_element_token_stream(&primary_key_field_type);
    ...
    let primary_key_field_type_as_postgresql_type_update_token_stream =
        generate_as_postgresql_type_update_token_stream(&primary_key_field_type);
    ...
    let std_vec_vec_primary_key_field_type_read_token_stream =
        quote::quote! {std::vec::Vec::<#primary_key_field_type_as_primary_key_upper_camel_case>};
        ...
        let pub_async_fn_prepare_postgresql_token_stream = {
            let prepare_postgresql_double_quotes_token_stream = {
                ...
                generate_quotes::double_quotes_token_stream(&format!(
                    "create table if not exists {ident_snake_case_stringified} ({acc})"
                ))
            };
                ...
                let generate_field_type_as_postgresql_crud_create_table_column_query_part_create_table_query_part_token_stream =
                    |field_type: &syn::Type,
                     ...
                     is_primary_key: std::primitive::bool| {
                        ...
                        let field_type_postgresql_type_table_type_declaration_token_stream =
                            generate_as_postgresql_type_table_type_declaration_token_stream(
                                &field_type,
                            );
                        quote::quote! {
                ...
                &|element: &SynFieldWrapper| {
                    ...
                    let initialization_token_stream = {
                        ...
                        let as_postgresql_crud_postgresql_type_postgresql_type_token_stream =
                            generate_as_postgresql_type_token_stream(&element.syn_field.ty);
                        quote::quote! {
    ...
    let generate_postgresql_crud_value_declaration_token_stream =
        |content_token_stream: &dyn quote::ToTokens| {
            quote::quote! {#postgresql_crud_snake_case::#value_upper_camel_case<#content_token_stream>}
        };
    let generate_postgresql_crud_value_initialization_token_stream =
        |content_token_stream: &dyn quote::ToTokens| {
    ...
    let generate_impl_postgresql_crud_default_but_option_is_always_some_and_vec_always_contains_one_element_for_tokens_no_lifetime_token_stream =
        |ident: &dyn quote::ToTokens, content_token_stream: &dyn quote::ToTokens| {
            postgresql_crud_macros_common::generate_impl_postgresql_crud_default_but_option_is_always_some_and_vec_always_contains_one_element_for_tokens_token_stream(&ident, &proc_macro2::TokenStream::new(), &content_token_stream)
        };
    #[derive(Debug)]
    ...
    impl SynVariantWrapper {
        ...
        const fn get_option_status_code(
            &self,
        ) -> &std::option::Option<macros_helpers::status_code::StatusCode> {
            &self.status_code
        }
    }
    let new_syn_variant_wrapper = |variant_name: &dyn std::fmt::Display,
                                   status_code: std::option::Option<
            ...
            variant: syn::Variant {
                ...
                fields: syn::Fields::Named(syn::FieldsNamed {
                    ...
                    named: {
                        ...
                        handle.push_value(
                            macros_helpers::code_occurence_syn_field::code_occurence_syn_field(),
                        );
                        handle
                    },
                }),
    ...
    impl Operation {
        ...
        const fn desirable_status_code(self) -> macros_helpers::status_code::StatusCode {
            match self {
                ...
                | Self::UpdateOne
                | Self::DeleteMany
                | Self::DeleteOne => macros_helpers::status_code::StatusCode::Ok200,
            }
        }
        const fn generate_postgresql_crud_attribute_additional_error_variants(
            self,
        ) -> GeneratePostgresqlCrudAttribute {
            match self {
                Self::CreateMany => {
                    GeneratePostgresqlCrudAttribute::CreateManyAdditionalErrorVariants
                }
                Self::CreateOne => {
                    GeneratePostgresqlCrudAttribute::CreateOneAdditionalErrorVariants
                }
                ...
                Self::UpdateMany => {
                    GeneratePostgresqlCrudAttribute::UpdateManyAdditionalErrorVariants
                }
                Self::UpdateOne => {
                    GeneratePostgresqlCrudAttribute::UpdateOneAdditionalErrorVariants
                }
                Self::DeleteMany => {
                    GeneratePostgresqlCrudAttribute::DeleteManyAdditionalErrorVariants
                }
                Self::DeleteOne => {
                    GeneratePostgresqlCrudAttribute::DeleteOneAdditionalErrorVariants
                }
            }
        }
        const fn generate_postgresql_crud_attribute_additional_logic(
            self,
        ) -> GeneratePostgresqlCrudAttribute {
            match self {
    ...
    let generate_initialization_token_stream = |syn_variant_wrapper: &SynVariantWrapper,
     ...
     -> proc_macro2::TokenStream {
        ...
        let fields_token_stream = if let syn::Fields::Named(value) =
            &syn_variant_wrapper.variant.fields
        {
        ...
        |operation: &Operation,
         ...
         column: std::primitive::u32| {
            ...
            let wraped_into_axum_response_token_stream = wrap_into_axum_response_token_stream(
                &quote::quote! {#ident_operation_response_variants_upper_camel_case::#from_snake_case(#error_snake_case)},
    ...
    let ident_create_token_stream = {
            ...
            let content_token_stream =
                generate_fields_named_without_primary_key_with_comma_token_stream(
                    &|element: &SynFieldWrapper| {
                        ...
                        let element_syn_field_ty_as_postgresql_type_create_token_stream =
                            generate_as_postgresql_type_create_token_stream(&element.syn_field.ty);
                        quote::quote! {
        ...
        let impl_postgresql_crud_default_but_option_is_always_some_and_vec_always_contains_one_element_for_ident_create_token_stream =
            generate_impl_postgresql_crud_default_but_option_is_always_some_and_vec_always_contains_one_element_for_tokens_no_lifetime_token_stream(&ident_create_upper_camel_case, &{
    ...
    let ident_where_many_token_stream = {
        let fields_declaration_token_stream = generate_fields_named_with_comma_token_stream(
            &|element: &SynFieldWrapper| -> proc_macro2::TokenStream {
                let field_ident = &element.field_ident;
                let element_syn_field_ty_as_postgresql_type_where_element_token_stream =
                    generate_as_postgresql_type_where_element_token_stream(&element.syn_field.ty);
        ...
        let impl_ident_where_many_token_stream = {
            let pub_fn_try_new_token_stream = {
                let none_token_stream = generate_fields_named_with_comma_token_stream(
                    &|_: &SynFieldWrapper| -> proc_macro2::TokenStream {
                        quote::quote! {None}
                    },
                );
        ...
        let impl_serde_deserialize_for_ident_where_many_token_stream =
            postgresql_crud_macros_common::generate_impl_serde_deserialize_for_struct_token_stream(
                ...
                fields
                    .iter()
                    .map(|element| (&element.field_ident, &element.syn_field.ty))
                    .collect::<std::vec::Vec<(&syn::Ident, &syn::Type)>>(),
                fields_len,
                &|_: &syn::Ident, syn_type: &syn::Type| {
                    let syn_type_as_postgresql_type_where_element_token_stream =
                        generate_as_postgresql_type_where_element_token_stream(&syn_type);
                    quote::quote! {std::option::Option<postgresql_crud::PostgresqlTypeWhere<#syn_type_as_postgresql_type_where_element_token_stream>>}
                },
            );
        let impl_postgresql_crud_default_but_option_is_always_some_and_vec_always_contains_one_element_for_ident_where_many_token_stream =
            generate_impl_postgresql_crud_default_but_option_is_always_some_and_vec_always_contains_one_element_for_tokens_no_lifetime_token_stream(&ident_where_many_upper_camel_case, &{
    ...
    let select_token_stream = {
        let ident_select_token_stream = {
            let variants = generate_fields_named_with_comma_token_stream(
                &|element: &SynFieldWrapper| {
                    ...
                    let element_syn_field_ty_as_postgresql_type_select_token_stream =
                        generate_as_postgresql_type_select_token_stream(&element.syn_field.ty);
                    quote::quote! {
        ...
        let ident_read_token_stream = {
            let field_option_primary_key_token_stream = {
                let postgresql_crud_value_declaration_token_stream =
                    generate_postgresql_crud_value_declaration_token_stream(
                        &generate_as_postgresql_type_read_token_stream(&primary_key_field_type),
                    );
                quote::quote! {
            ...
            let fields_options_without_primary_key_token_stream =
                generate_fields_named_without_primary_key_with_comma_token_stream(
                    &|element: &SynFieldWrapper| -> proc_macro2::TokenStream {
                        ...
                        let postgresql_crud_value_declaration_token_stream =
                            generate_postgresql_crud_value_declaration_token_stream(
                                &generate_as_postgresql_type_read_token_stream(
            ...
            let fn_try_from_sqlx_postgres_pg_row_with_not_empty_unique_enum_vec_ident_select_token_stream = {
                ...
                let assignment_variant_primary_key_token_stream = {
                    let primary_key_field_ident_string_double_quotes_token_stream =
                        generate_quotes::double_quotes_token_stream(&primary_key_field_ident);
                    let postgresql_crud_value_initialization_token_stream =
                        generate_postgresql_crud_value_initialization_token_stream(
                            &value_snake_case,
                        );
                    quote::quote! {
    ...
    let ident_read_only_ids_token_stream = {
            ...
            let content_token_stream = generate_fields_named_with_comma_token_stream(
                &|element: &SynFieldWrapper| {
                    ...
                    let element_syn_field_ty_as_postgresql_type_read_only_ids_token_stream =
                        generate_as_postgresql_type_read_only_ids_token_stream(
                            &element.syn_field.ty,
                        );
                    quote::quote! {
        ...
        let impl_try_from_pg_row_for_ident_read_only_ids_token_stream = {
            let fields_initialization_token_stream =
                generate_fields_named_without_comma_token_stream(&|element: &SynFieldWrapper| {
                    let field_ident = &element.field_ident;
                    let field_ident_double_quotes_token_stream =
                        generate_quotes::double_quotes_token_stream(&field_ident);
                    let element_syn_field_ty_as_postgresql_type_read_only_ids_token_stream =
    ...
    let ident_column_read_permission_token_stream = {
        let ident_column_read_permission_upper_camel_case =
            naming::parameter::SelfColumnReadPermissionUpperCamelCase::from_display(&ident);
        let fields_permission_token_stream =
            generate_fields_named_with_comma_token_stream(&|element: &SynFieldWrapper| {
                let field_ident = &element.field_ident;
    ...
    let ident_update_token_stream = {
        ...
        let impl_postgresql_crud_default_but_option_is_always_some_and_vec_always_contains_one_element_for_ident_update_token_stream =
            generate_impl_postgresql_crud_default_but_option_is_always_some_and_vec_always_contains_one_element_for_tokens_no_lifetime_token_stream(&ident_update_upper_camel_case, &{
    ...
    let use_postgresql_crud_try_stream_ext_token_stream =
        quote::quote! {use #postgresql_crud_snake_case::TryStreamExt};
    ...
    let std_vec_vec_struct_options_ident_token_stream =
        quote::quote! {std::vec::Vec::<#ident_read_upper_camel_case>};
    ...
    let generate_additional_error_variants =
        |syn_derive_input: &syn::DeriveInput,
         ...
         -> std::vec::Vec<syn::Variant> {
            let generate_postgresql_crud_attribute_stringified =
                generate_postgresql_crud_attribute.to_string();
            ...
            let value: syn::DeriveInput = syn::parse(
                (*common_additional_error_variants_attribute_token_stream)
                    .clone()
                    .into(),
            )
            .unwrap_or_else(|error| panic!("{}: {error}", constants::AST_PARSE_FAILED));
    ...
    let common_additional_logic_token_stream =
        macros_helpers::get_macro_attribute::get_macro_attribute_meta_list_token_stream(
            &syn_derive_input.attrs,
            &GeneratePostgresqlCrudAttribute::CommonAdditionalLogic.generate_path_to_attribute(),
        );
    ...
    let generate_parameters_payload_and_default_token_stream =
        |operation: &Operation,
         ...
         default_init_content_token_stream: &dyn quote::ToTokens| {
            ...
            generate_impl_postgresql_crud_default_but_option_is_always_some_and_vec_always_contains_one_element_for_tokens_no_lifetime_token_stream(&ident_operation_payload_upper_camel_case, &quote::quote! {Self #default_init_content_token_stream});
            quote::quote! {
        ...
        |syn_variants: &std::vec::Vec<&syn::Variant>,
         ...
         -> std::vec::Vec<syn::Variant> {
            ...
            let operation_additional_error_variants = generate_additional_error_variants(
                &syn_derive_input,
                operation.generate_postgresql_crud_attribute_additional_error_variants(),
            );
    ...
    let generate_ident_try_operation_error_named_token_stream =
        |operation: &Operation,
         ...
         -> proc_macro2::TokenStream {
            ...
            let variants_token_stream = syn_variants
                .iter()
                .map(generate_error_occurence_variant_token_stream);
            quote::quote! {
            ...
            let additional_validators_token_stream = {
                let operation_additional_logic_token_stream =
                    macros_helpers::get_macro_attribute::get_macro_attribute_meta_list_token_stream(
                        &syn_derive_input.attrs,
                        &operation
                            .generate_postgresql_crud_attribute_additional_logic()
    ...
    let generate_create_update_delete_many_fetch_token_stream =
        |create_many_or_update_many_or_delete_many: &CreateManyOrUpdateManyOrDeleteMany| {
    ...
    let generate_create_update_delete_one_fetch_token_stream =
        |create_one_or_update_one_or_delete_one: &CreateOneOrUpdateOneOrDeleteOne| {
    ...
    let increment_initialization_token_stream =
        quote::quote! {let mut #increment_snake_case: std::primitive::u64 = 0;};
    let column_names = {
    ...
    let create_many_token_stream = {
        ...
        let operation_payload_example_token_stream =
            generate_operation_payload_example_token_stream(&operation);
        quote::quote! {
            #parameters_token_stream
            #operation_token_stream
            #try_operation_token_stream
            ...
            let operation_token_stream = {
                ...
                let postgresql_logic_token_stream =
                    wrap_content_into_postgresql_transaction_begin_commit_value_token_stream(
                        ...
                        &{
                            let create_one_or_update_one_or_delete_one =
                                CreateOneOrUpdateOneOrDeleteOne::CreateOne;
    ...
    let update_many_token_stream = {
        ...
        let parameters_token_stream = generate_parameters_pattern_token_stream(&operation, {
            ...
            let std_vec_vec_ident_update_token_stream =
                quote::quote! {std::vec::Vec<#ident_update_upper_camel_case>};
            ...
            let operation_token_stream = {
                    ...
                    let fields_named_without_primary_key_update_assignment_token_stream =
                        generate_fields_named_without_primary_key_without_comma_token_stream(
                            &|element: &SynFieldWrapper| {
                                ...
                                let update_query_part_primary_key_snake_case =
                                    naming::UpdateQueryPartPrimaryKeySnakeCase;
                                quote::quote! {
                                    {
                ...
                let postgresql_logic_token_stream =
                    wrap_content_into_postgresql_transaction_begin_commit_value_token_stream(
                        &operation,
                        &generate_create_update_delete_many_fetch_token_stream(
                            &CreateManyOrUpdateManyOrDeleteMany::UpdateMany,
                        ),
                    );
                generate_operation_token_stream(
                    &operation,
        ...
        let try_operation_token_stream = {
            let try_operation_error_named_token_stream =
                generate_ident_try_operation_error_named_token_stream(
                    &operation,
                    &common_http_request_syn_variants.clone(),
                );
            let try_operation_token_stream = generate_try_operation_token_stream(
                &operation,
                &type_variants_from_request_response_syn_variants,
            ...
            let operation_token_stream = {
                ...
                let postgresql_logic_token_stream =
                    wrap_content_into_postgresql_transaction_begin_commit_value_token_stream(
                        ...
                        &{
                            let create_one_or_update_one_or_delete_one =
                                CreateOneOrUpdateOneOrDeleteOne::UpdateOne;
                            let current_operation = match &create_one_or_update_one_or_delete_one {
                                CreateOneOrUpdateOneOrDeleteOne::CreateOne => Operation::CreateOne,
                                CreateOneOrUpdateOneOrDeleteOne::UpdateOne => Operation::UpdateOne,
                                CreateOneOrUpdateOneOrDeleteOne::DeleteOne => Operation::DeleteOne,
            ...
            let operation_token_stream = {
                ...
                let postgresql_logic_token_stream =
                    wrap_content_into_postgresql_transaction_begin_commit_value_token_stream(
                        &operation,
                        &generate_create_update_delete_many_fetch_token_stream(
                            &CreateManyOrUpdateManyOrDeleteMany::DeleteMany,
                        ),
                    );
                generate_operation_token_stream(
                    &operation,
                ...
                let postgresql_logic_token_stream =
                    wrap_content_into_postgresql_transaction_begin_commit_value_token_stream(
                        ...
                        &generate_create_update_delete_one_fetch_token_stream(
                            &CreateOneOrUpdateOneOrDeleteOne::DeleteOne,
                        ),
                    );
                generate_operation_token_stream(
                    &operation,
    ...
    let routes_token_stream = {
        ...
        let slash_create_many_double_quotes_token_stream =
            generate_slash_route_double_quotes_token_stream(&create_many.snake_case_stringified());
        let slash_create_one_double_quotes_token_stream =
            generate_slash_route_double_quotes_token_stream(&create_one.snake_case_stringified());
        let slash_read_many_double_quotes_token_stream =
            generate_slash_route_double_quotes_token_stream(&read_many.snake_case_stringified());
        let slash_read_one_double_quotes_token_stream =
            generate_slash_route_double_quotes_token_stream(&read_one.snake_case_stringified());
        let slash_update_many_double_quotes_token_stream =
            generate_slash_route_double_quotes_token_stream(&update_many.snake_case_stringified());
        let slash_update_one_double_quotes_token_stream =
            generate_slash_route_double_quotes_token_stream(&update_one.snake_case_stringified());
        let slash_delete_many_double_quotes_token_stream =
            generate_slash_route_double_quotes_token_stream(&delete_many.snake_case_stringified());
        let slash_delete_one_double_quotes_token_stream =
            generate_slash_route_double_quotes_token_stream(&delete_one.snake_case_stringified());
    ...
    let ident_tests_token_stream = {
        ...
        let drop_table_if_exists_ident_double_quotes_token_stream =
            generate_quotes::double_quotes_token_stream(&format!(
        ...
        let ident_update_many_parameters_upper_camel_case =
            generate_ident_operation_parameters_upper_camel_case(&Operation::UpdateMany);
        let ident_update_many_payload_upper_camel_case =
            generate_ident_operation_payload_upper_camel_case(&Operation::UpdateMany);
        ...
        let ident_delete_many_parameters_upper_camel_case =
            generate_ident_operation_parameters_upper_camel_case(&Operation::DeleteMany);
        let ident_delete_many_payload_upper_camel_case =
            generate_ident_operation_payload_upper_camel_case(&Operation::DeleteMany);
        let ident_delete_one_parameters_upper_camel_case =
            generate_ident_operation_parameters_upper_camel_case(&Operation::DeleteOne);
        let ident_delete_one_payload_upper_camel_case =
            generate_ident_operation_payload_upper_camel_case(&Operation::DeleteOne);
        let ident_try_read_one_error_named_upper_camel_case =
            generate_ident_try_operation_error_named_upper_camel_case(&Operation::ReadOne);
        let ident_read_one_error_named_with_serialize_deserialize_upper_camel_case =
            generate_ident_operation_error_named_with_serialize_deserialize_upper_camel_case(
                &Operation::ReadOne,
            );
        let std_option_option_ident_where_many_upper_camel_case =
            naming::parameter::StdOptionOptionSelfWhereManyUpperCamelCase::from_tokens(&ident);
        let config_path_token_stream =
            quote::quote! {crate::repositories_types::server::config::Config};
        let config_upper_case_token_stream = quote::quote! {CONFIG};
        let underscore_unused_token_stream = quote::quote! {_unused};
        let where_many_1_and_2_primary_keys_token_stream =
            quote::quote! {where_many_1_and_2_primary_keys};
        ...
        let primary_key_read_returned_from_create_many1_token_stream =
            quote::quote! {primary_key_read_returned_from_create_many1};
        let primary_key_read_returned_from_create_many2_token_stream =
            quote::quote! {primary_key_read_returned_from_create_many2};
        let some_value_primary_key_read_returned_from_create_many1_token_stream =
            quote::quote! {some_value_primary_key_read_returned_from_create_many1};
        let some_value_primary_key_read_returned_from_create_many2_token_stream =
            quote::quote! {some_value_primary_key_read_returned_from_create_many2};
        let (equal_filter_primary_key1_token_stream, equal_filter_primary_key2_token_stream) = {
            ...
            (
                generate_equal_filter_primary_key_token_stream(
        ...
        let columns_test_cases_declaration_token_stream =
            generate_fields_named_without_primary_key_without_comma_token_stream(
                &|element: &SynFieldWrapper| {
        ...
        let test_cases_max_len_token_stream = {
            ...
            for element in &fields_without_primary_key {
                let field_ident_test_cases_snake_case =
                    naming::parameter::SelfTestCasesSnakeCase::from_tokens(&element.field_ident);
                if content_token_stream.to_string() == "".to_string() {
                    content_token_stream =
                        quote::quote! {0..#field_ident_test_cases_snake_case.len()};
                } else {
        ...
        let update_try_new_parameters_declaration_token_stream =
            generate_fields_named_without_primary_key_without_comma_token_stream(
                &|element: &SynFieldWrapper| {
                    let field_ident_test_cases_snake_case =
                        naming::parameter::SelfTestCasesSnakeCase::from_tokens(
                    ...
                    let field_ident_some_value_update_snake_case =
                        naming::parameter::SelfSomeValueUpdateSnakeCase::from_tokens(
                            &element.field_ident,
                        );
                    let field_type = &element.syn_field.ty;
                    quote::quote! {
        ...
        let update_try_new_parameters_cloned_token_stream =
            generate_fields_named_without_primary_key_with_comma_token_stream(
                &|element: &SynFieldWrapper| {
                    let field_ident_some_value_update_snake_case =
                        naming::parameter::SelfSomeValueUpdateSnakeCase::from_tokens(
                    ...
                    quote::quote! {#field_ident_some_value_update_snake_case.clone()}
                },
            );
        let update_try_new_parameters_token_stream =
            generate_fields_named_without_primary_key_with_comma_token_stream(
                &|element: &SynFieldWrapper| {
                    let field_ident_some_value_update_snake_case =
                        naming::parameter::SelfSomeValueUpdateSnakeCase::from_tokens(
                    ...
                    quote::quote! {#field_ident_some_value_update_snake_case}
                },
            );
        let ident_select_columns_token_stream = generate_fields_named_with_comma_token_stream(
            &|element: &SynFieldWrapper| {
                let field_ident_upper_camel_case =
                    naming::ToTokensToUpperCamelCaseTokenStream::case_or_panic(
                        &element.field_ident,
        ...
        let ident_read_fields_declaration_token_stream =
            generate_fields_named_without_primary_key_without_comma_token_stream(
                &|element: &SynFieldWrapper| {
                    let field_ident = &element.field_ident;
                    let field_ident_test_cases_snake_case =
                        naming::parameter::SelfTestCasesSnakeCase::from_tokens(&field_ident);
                    let field_ident_some_value_read_snake_case =
                        naming::parameter::SelfSomeValueReadSnakeCase::from_tokens(&field_ident);
                    let field_type = &element.syn_field.ty;
                    quote::quote! {
        ...
        let ident_read_fields_cloned_token_stream =
            generate_fields_named_without_primary_key_with_comma_token_stream(
                &|element: &SynFieldWrapper| {
                    let field_ident = &element.field_ident;
                    let field_ident_some_value_read_snake_case =
                        naming::parameter::SelfSomeValueReadSnakeCase::from_tokens(&field_ident);
                    quote::quote! {#field_ident: #field_ident_some_value_read_snake_case.clone()}
                },
            );
        let ident_read_fields_token_stream =
            generate_fields_named_without_primary_key_with_comma_token_stream(
                &|element: &SynFieldWrapper| {
                    let field_ident = &element.field_ident;
                    let field_ident_some_value_read_snake_case =
                        naming::parameter::SelfSomeValueReadSnakeCase::from_tokens(&field_ident);
                    quote::quote! {#field_ident: #field_ident_some_value_read_snake_case}
                },
            );
        quote::quote! {
            #[cfg(test)]
                    ...
                    std::thread::Builder::new()
                        ...
                        .spawn(|| {
                            tokio::runtime::Builder::new_multi_thread().worker_threads(num_cpus::get()).enable_all().build().expect("error 38823c21-1879-449c-9b60-ce7293709959").block_on(async {
                                ...
                                let #some_value_read_only_ids_returned_from_create_one_snake_case = Some(postgresql_crud::Value { #value_snake_case: #read_only_ids_returned_from_create_one_snake_case.#primary_key_field_ident.clone() });
                                assert_eq!(
                                    super::#ident_read_upper_camel_case {
                                ...
                                for #index_snake_case in #test_cases_max_len_token_stream {
                                    ...
                                    // );
                                    assert_eq!(
                                        #read_only_ids_returned_from_create_one_snake_case.clone(),
                                        {
                                            let f = super::#ident::try_update_one(
                                                &#url_snake_case,
