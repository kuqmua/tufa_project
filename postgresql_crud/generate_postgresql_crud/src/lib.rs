...
pub fn generate_postgresql_crud(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let primary_key_try_from_sqlx_row_name_token_stream = {
        let value = format!(
            "{}_{}_{}_{}_{}_{}",
            proc_macro_helpers::naming_conventions::primary_snake_case_stringified(),
            proc_macro_helpers::naming_conventions::key_snake_case_stringified(),
            proc_macro_helpers::naming_conventions::try_snake_case_stringified(),
            proc_macro_helpers::naming_conventions::from_snake_case_stringified(),
            proc_macro_helpers::naming_conventions::sqlx_snake_case_stringified(),
            proc_macro_helpers::naming_conventions::row_snake_case_stringified(),
        );
        value.parse::<proc_macro2::TokenStream>()
        .unwrap_or_else(|_| panic!("{proc_macro_name_upper_camel_case_ident_stringified} {value} {}", proc_macro_common::global_variables::hardcode::PARSE_PROC_MACRO2_TOKEN_STREAM_FAILED_MESSAGE))
    };
    ...
    let deserialize_ident_order_by_token_stream = {
        ...
        let ivalid_ident_order_by_handle_token_stream = {
            let ivalid_ident_order_by_handle = format!(
                "\"Invalid {ident}{}:\"",
                proc_macro_helpers::naming_conventions::order_by_upper_camel_case_stringified()
            );
        ...
        let deserialize_ident_order_by_snake_case_name_token_stream = {
            let deserialize_ident_order_by_snake_case_name =
                format!("deserialize_{ident_snake_case_stringified}_{}", proc_macro_helpers::naming_conventions::order_by_snake_case_stringified());
        ...
        quote::quote! {
            fn #deserialize_ident_order_by_snake_case_name_token_stream<'de, D>(
                deserializer: D,
            ) -> Result<#postgresql_crud_order_by_token_stream<#ident_column_upper_camel_case_token_stream>, D::Error>
            where
                D: serde::de::Deserializer<'de>,
            {
                                ...
                                Some(offset_slice_next_comma_index) => {
                                    match offset_slice.get(0..offset_slice_next_comma_index) {
                                        Some(possible_order) => match {
                                            use #std_str_from_str_token_stream;
                                            #postgresql_crud_order_token_stream::#from_str_snake_case_token_stream(possible_order)
                                        } {
                                            Ok(order) => Some(order),
                                            Err(#error_value_snake_case_token_stream) => {
                                                return Err(serde::de::Error::custom(&format!(
                ...
                Ok(#postgresql_crud_order_by_token_stream { column, order })
            }
        }
    };
        ...
        let struct_token_stream = {
            ...
            quote::quote! {
                ...
                pub struct #ident_order_by_wrapper_name_token_stream(
                    ...
                    pub #postgresql_crud_order_by_token_stream<#ident_column_upper_camel_case_token_stream>,
                );
            }
        };
            ...
            quote::quote! {
                impl #std_str_from_str_token_stream for #ident_order_by_wrapper_name_token_stream {
                    ...
                    fn from_str(value: #str_ref_token_stream) -> Result<Self, Self::Err> {
                            ...
                            Some(index) => match index.checked_add(order_equal_str.len()) {
                                Some(offset) => match string_deserialized.get(offset..) {
                                    Some(offset_slice) => match offset_slice.find(split_inner_url_parameters_symbol) {
                                        Some(offset_slice_next_comma_index) => {
                                            match offset_slice.get(0..offset_slice_next_comma_index) {
                                                Some(possible_order) => match #postgresql_crud_order_token_stream::#from_str_snake_case_token_stream(possible_order) {
                                                    Ok(order) => Some(order),
                                                    Err(#error_value_snake_case_token_stream) => {
                        ...
                        Ok(Self(#postgresql_crud_order_by_token_stream { column, order }))
                    }
                }
            }
        };
    ...
    let commit_failed_syn_variant = {
        let variant_name_upper_camel_case_stringified = format!(
            "{}{}",
            proc_macro_helpers::naming_conventions::commit_upper_camel_case_stringified(),
            proc_macro_helpers::naming_conventions::failed_upper_camel_case_stringified(),
        );
        let variant_name_snake_case_stringified = proc_macro_common::naming_conventions::ToSnakeCaseStringified::to_snake_case_stringified(&variant_name_upper_camel_case_stringified);
        crate::type_variants_from_request_response_generator::construct_syn_variant(
            proc_macro_helpers::status_code::StatusCode::Tvfrr500InternalServerError,
...
impl<'a> std::convert::From<&'a syn::Field> for SynFieldWithAdditionalInfo<'a> {
    fn from(value: &'a syn::Field) -> Self {
        ...
        //     value.parse::<proc_macro2::TokenStream>()
        //     .unwrap_or_else(|_| panic!("{name} {value} {}", proc_macro_common::global_variables::hardcode::PARSE_PROC_MACRO2_TOKEN_STREAM_FAILED_MESSAGE))
        // };
