...
pub fn generate_postgresql_crud(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let ident_tests_token_stream = {
        ...
        let read_fields_initialization_token_stream = generate_fields_named_without_primary_key_with_comma_token_stream(&|element: &SynFieldWrapper| {
            let field_ident = &element.field_ident;
            let field_type_as_postgresql_type_read_token_stream = generate_as_postgresql_type_read_token_stream(&element.syn_field.ty);
            quote::quote! {
                #field_ident: Some(postgresql_crud::Value {
                    value: #field_type_as_postgresql_type_read_token_stream::new(modification),
                })
            }
        });
                    ...
                    std::thread::Builder::new()
                        ...
                        .spawn(|| {
                            tokio::runtime::Builder::new_multi_thread().worker_threads(num_cpus::get()).enable_all().build().unwrap().block_on(async {
                                ...
                                let vec_of_ident_read_with_primary_key_sort_by_primary_key = |mut vec: std::vec::Vec<super::#ident_read_upper_camel_case>|{
                                ...
                                assert_eq!(
                                    ...
                                    vec_of_primary_keys_returned_from_delete_many.sort(),
