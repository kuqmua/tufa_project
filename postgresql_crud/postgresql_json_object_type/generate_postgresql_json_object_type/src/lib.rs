...
pub fn generate_postgresql_json_object_type(input_token_stream: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let (fields_token_stream, postgresql_json_object_type_array) = postgresql_json_object_type_record_vec
        ...
        .map(|(index, element)| {
            ...
            let postgresql_crud_path_postgresql_json_type_uuid_uuid_update_for_query_token_stream = generate_type_as_postgresql_json_type_update_for_query_token_stream(&import_path_postgresql_json_type_uuid_uuid_as_not_null_jsonb_string_token_stream);
                    ...
                    &match &postgresql_json_object_type_pattern {
                        PostgresqlJsonObjectTypePattern::Standart => match &not_null_or_nullable {
                            ...
                            postgresql_crud_macros_common::NotNullOrNullable::Nullable => {
                                quote::quote!{
                                    if let Some(#value_snake_case) = &#value_snake_case.0 {
                                        match <#ident_standart_not_null_upper_camel_case as postgresql_crud::PostgresqlJsonType>::#select_only_created_ids_query_bind_snake_case(
                                            &#value_snake_case,
                                            #query_snake_case
                                        ) {
                                            Ok(#value_snake_case) => {
