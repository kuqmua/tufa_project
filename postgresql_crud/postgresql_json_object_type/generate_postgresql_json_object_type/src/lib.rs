        ...
        .map(|(index, element)| {
            ...
            let ident_select_token_stream = {
                ...
                let impl_ident_select_token_stream = {
                    ...
                    //todo why need this as separate function?
            ...
            let (maybe_impl_postgresql_crud_postgresql_json_type_for_ident_token_stream, maybe_impl_postgresql_crud_postgresql_types_postgresql_type_postgresql_type_token_stream) = {
                let generate_update_query_part_standart_not_null_token_stream = |postgresql_type_or_postgresql_json_type: &postgresql_crud_macros_common::PostgresqlTypeOrPostgresqlJsonType|{
                    let none_format_handle_token_stream = generate_quotes::double_quotes_token_stream(&match &postgresql_type_or_postgresql_json_type {
                ...
                let impl_postgresql_crud_postgresql_json_type_for_ident_token_stream = postgresql_crud_macros_common::generate_impl_postgresql_json_type_token_stream(
                    ...
                    &match &postgresql_json_object_type_pattern {
                        PostgresqlJsonObjectTypePattern::Standart => match &not_null_or_nullable {
                            ...
                            postgresql_crud_macros_common::NotNullOrNullable::Nullable => generate_update_query_part_standart_not_null_token_stream(
                                ...
                                quote::quote! {
                                    match &value.0 {
                                        Some(value) => {
                                            ...
                                            let maybe_where = if value.delete.is_empty() { std::string::String::default() } else { format!(" where {delete_query_part_acc}") };
                                            let maybe_jsonb_build_array = if value.create.is_empty() { std::string::String::default() } else { format!(" || jsonb_build_array({create_query_part_acc})") };
                                            Ok(format!("(case when jsonb_typeof({jsonb_set_target}) = 'null' then '[]'::jsonb else (select coalesce((select jsonb_agg({update_query_part_acc}) from jsonb_array_elements({jsonb_set_target}) as elem {maybe_where}),'[]'::jsonb)) end {maybe_jsonb_build_array})"
                                            ))
                                        },
                                        None => match increment.checked_add(1) {
                                            Some(value) => {
