            ...
            quote::quote!{
                impl postgresql_crud::GeneratePostgresqlQueryPartToUpdate<#std_option_option_generic_ident_option_to_update_try_generate_bind_increments_error_named_upper_camel_case_token_stream> for #std_option_option_generic_ident_option_to_update_upper_camel_case_token_stream {
                    fn try_generate_bind_increments(
                        ...
                        match &self.0 {
                            Some(value) => {
                                let mut #std_option_option_generic_acc_snake_case = format!("case when jsonb_typeof({jsonb_set_target}) = 'null' then '{{}}'::jsonb else ({jsonb_set_target})::jsonb end");
                                for element in value {
                                    match element {
                                        #(#try_generate_bind_increments_variants_token_stream),*
                                    }
                        ...
                        let delete_query_part_acc = {
                            ...
                            for _ in &self.0.delete {
                                match increment.checked_add(1) {
                                    ...
                                    None => {
                                        return Err(#std_vec_vec_generic_with_id_ident_option_to_update_try_generate_bind_increments_error_named_upper_camel_case_token_stream::CheckedAdd { 
                                            code_occurence: error_occurence_lib::code_occurence!()
                                        });
                                    }
                                }
