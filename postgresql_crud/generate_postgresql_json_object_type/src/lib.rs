...
pub fn generate_postgresql_json_object_type(input_token_stream: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    .map(|(index, element)|{
        ...
        let generate_select_only_ids_query_part_token_stream = |is_standart_with_id: &IsStandartWithId|{
            let format_handle_token_stream = generate_quotes::double_quotes_token_stream(&{
                let mut acc = get_vec_syn_field(&is_standart_with_id).iter().fold(
        ...
        let impl_postgresql_json_type_test_cases_for_ident_with_id_not_null_token_stream = if is_standart_not_null {
            ...
            postgresql_crud_macros_common::generate_impl_postgresql_json_type_test_cases_for_ident_token_stream(
                ...
                &ident_with_id_standart_not_null_upper_camel_case,
                &{
                    let generate_default_field_ident_some_value_token_stream = |
                        default_element_field_ident: &dyn quote::ToTokens,
                        default_element_type: &dyn quote::ToTokens
                    |{
                        quote::quote! {
                            #default_element_field_ident: Some(postgresql_crud::Value {
                                #value_snake_case: <
                                    #default_element_type
                                    as
                                    postgresql_crud::PostgresqlJsonType
                    ...
                    let id_default_field_ident_some_value_token_stream = generate_default_field_ident_some_value_token_stream(
                        &id_snake_case,
                        &uuid_path_type_token_stream,
                    );
                    let content_token_stream = vec_syn_field.iter().map(|element| {
                        let element_field_ident = element.ident.as_ref().unwrap_or_else(|| {
                            panic!("{}", naming::FIELD_IDENT_IS_NONE);
                        ...
                        let default_token_stream = vec_syn_field.iter().map(|element| {
                            ...
                            if element_field_ident == default_element_field_ident {
                                ...
                                generate_default_field_ident_some_value_token_stream(
                                    &default_element_field_ident,
                                    &default_element_type
                                )
                            }
                        });
                ...
                &{
                    let generate_match_some_value_test_cases_token_stream = |
                        element_field_ident: &dyn quote::ToTokens,
                    ...
                    let id_match_some_value_test_cases_token_stream = generate_match_some_value_test_cases_token_stream(
                        &id_snake_case,
                        &uuid_path_type_token_stream,
                    );
                    let parameters_token_stream = vec_syn_field.iter().map(|element| {
                        ...
                        let element_type = &element.ty;
                        generate_match_some_value_test_cases_token_stream(
                            &element_field_ident,
                            &element_type
                        )
                    });
                    quote::quote!{<Self::Element as postgresql_crud::PostgresqlJsonType>::Read::try_new(
                        #id_match_some_value_test_cases_token_stream,
                        #(#parameters_token_stream),*
                    ).unwrap()}
                },
                &{
                    ...
                    quote::quote!{
                        <Self::Element as postgresql_crud::PostgresqlJsonType>::Update::new(
                            postgresql_crud::NotEmptyUniqueEnumVec::try_new({
                                let mut #acc_snake_case = vec![];
                                #(#parameters_token_stream)*
                                #acc_snake_case
