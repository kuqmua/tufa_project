            ...
            &{
                ...
                let format_handle = match &postgresql_json_type_pattern {
                    ...
                    PostgresqlJsonTypePattern::ArrayDimension4 {
                        ...
                        let not_null_nullable_nullable_nullable = format!("
select 
  jsonb_agg(
          ...
          jsonb_agg(
            case when jsonb_typeof(d2_elem.value)= 'array' then (
              (
                select 
                  jsonb_agg(
                    case when jsonb_typeof(d3_elem.value)= 'array' then (
                    
                    (
                      select 
                        jsonb_agg(d4_elem.value) 
                      from 
                        jsonb_array_elements(d3_elem.value) with ordinality as d4_elem(value, d4_ord) 
                      where 
                        d4_ord between {{dimension4_start}} 
                        and {{dimension4_end}}
                    )
