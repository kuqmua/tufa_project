                        ...
                        let not_null_nullable_not_null_not_null = generate_jsonb_agg_d1(
                            &generate_case_when_jsonb_typeof_array_then_else_null_end_d1(
                                &generate_jsonb_agg_d2(
                                    ...
                                    // &format!("(select jsonb_agg() from jsonb_array_elements(d2_elem.value) with ordinality as d3_elem(value, d3_ord) where d3_ord between {{dimension3_start}} and {{dimension3_end}})")
                                )
                            )
                        );
