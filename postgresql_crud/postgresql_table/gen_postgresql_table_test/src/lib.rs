...
mod tests {
    ...
    fn clippy() {
        ...
        macro_clippy_check_common::clippy_check(
            "gen_postgresql_table_test_content",
...
server_config = {path = "../../../server_config"}"#,
            &{
                enum ShouldAddGenPostgresqlTablePrimaryKey {
                    False,
                    True,
                }
                let gen_table_example_ts = |
                    should_add_gen_postgresql_table_primary_key: ShouldAddGenPostgresqlTablePrimaryKey
                |{
                    let allow_clippy_arbitrary_source_item_ordering_ts = token_patterns::AllowClippyArbitrarySourceItemOrdering;
                    let maybe_gen_postgresql_table_primary_key_ts = match should_add_gen_postgresql_table_primary_key {
                        ShouldAddGenPostgresqlTablePrimaryKey::False => proc_macro2::TokenStream::new(),
                        ShouldAddGenPostgresqlTablePrimaryKey::True => quote!{#[gen_postgresql_table_primary_key]},
                    };
                    quote!{
                        ...
                        pub struct TableExample {
                            #maybe_gen_postgresql_table_primary_key_ts
                ...
                let ts =
                    gen_postgresql_table_source::gen_postgresql_table(
                        gen_table_example_ts(
                            ShouldAddGenPostgresqlTablePrimaryKey::True
                        )
                    );
                let table_struct_ts = gen_table_example_ts(
                    ShouldAddGenPostgresqlTablePrimaryKey::False
                );
                quote! {
                    #ts
                    #table_struct_ts
